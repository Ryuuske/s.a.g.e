"""Tests for mine_nook_lock — the per-nook non-blocking mine guard.

Covers the fix for the runaway mine fan-out described alongside issues
#974 and #965: if N copies of `sage mine` are spawned concurrently
against the same nook, they must collapse to a single runner rather
than queue as waiters that will drive parallel HNSW inserts. Mines
against *different* nooks must still be free to run in parallel.
"""
...
def _hold_lock(nook_path: str, ready_flag: str, release_flag: str) -> int:
    """Acquire mine_nook_lock, signal readiness, wait for release flag.
...
def test_single_acquire_succeeds(tmp_path, monkeypatch):
    ...
    with mine_nook_lock(str(tmp_path / "nook")):
...
def test_same_nook_serializes_across_processes(tmp_path, monkeypatch):
    """Two processes contending for the same nook: second must be rejected."""
    monkeypatch.setenv("HOME", str(tmp_path))
    nook = str(tmp_path / "nook")
    ...
    ctx = _get_mp_context()
    holder = ctx.Process(target=_hold_lock, args=(nook, ready, release))
    holder.start()
    try:
        ...
        # From the parent, we must not be able to acquire the same nook lock
        with pytest.raises(MineAlreadyRunning):
            with mine_nook_lock(nook):
                pytest.fail("second acquire of same nook should have raised")
    finally:
...
def test_different_nooks_dont_conflict(tmp_path, monkeypatch):
    """Mines against different nooks must NOT block each other."""
    monkeypatch.setenv("HOME", str(tmp_path))
    ...
    ctx = _get_mp_context()
    holder = ctx.Process(target=_hold_lock, args=(nook_a, ready, release))
    holder.start()
    try:
        for _ in range(500):
            if os.path.exists(ready):
        ...
        # Different nook — must succeed even while nook_a is held
        with mine_nook_lock(nook_b):
...
def test_nook_path_is_normalized(tmp_path, monkeypatch):
    ...
    os.makedirs(tmp_path / "nook", exist_ok=True)
    absolute = str(tmp_path / "nook")
    ready = str(tmp_path / "ready")
    release = str(tmp_path / "release")
    ...
    try:
        ...
        # Parent holds CWD = tmp_path so "nook" is the same on-disk dir as
...
def test_reentrant_same_thread_passes_through(tmp_path, monkeypatch):
    """Same thread re-acquiring the same nook lock must not deadlock or raise.

    This is the invariant that makes ``ChromaCollection`` write methods
    (which take ``mine_nook_lock`` for MCP/direct-writer protection)
    ...
    with mine_nook_lock(nook):
        ...
        ctx = _get_mp_context()
        result_q = ctx.Queue()
        child = ctx.Process(target=_try_acquire_expect_busy, args=(nook, result_q))
        try:
            child.start()
...
def _try_acquire_expect_busy(nook_path, result_q):
...
def _hold_lock_send_pid(nook_path: str, ready_flag: str, release_flag: str, pid_q) -> None:
...
def test_lock_failure_message_names_holder(tmp_path, monkeypatch):
    ...
    Before this fix, a `sage mine` colliding with another writer
    (mine, MCP server, anything taking mine_nook_lock) saw a generic
    "another `sage mine` is already running" message and exited
    ...
    ctx = _get_mp_context()
    pid_q = ctx.Queue()
    holder = ctx.Process(target=_hold_lock_send_pid, args=(nook, ready, release, pid_q))
    holder.start()
    try:
        for _ in range(500):
            if os.path.exists(ready):
...
def test_write_lock_holder_is_best_effort_on_unicode_error(monkeypatch):
    ...
    monkeypatch.setattr(sys, "argv", ["sage", "mine", "北"])
...
def test_lock_holder_identity_persists_across_release(tmp_path, monkeypatch):
    ...
    # Windows; setting both makes the ``~/.sage/locks`` lookup land
    ...
    # it by scanning the sage locks dir for mine_nook_*.lock entries.
    lock_dir = tmp_path / ".sage" / "locks"
    lock_files = list(lock_dir.glob("mine_nook_*.lock"))
    assert lock_files, "expected the nook lock file to exist after acquire/release"
...
def test_mine_global_lock_is_alias_for_back_compat(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("HOME", str(tmp_path))
    assert mine_global_lock is mine_nook_lock
    with mine_global_lock(str(tmp_path / "nook")):
        pass  # the alias accepts the same nook_path argument
