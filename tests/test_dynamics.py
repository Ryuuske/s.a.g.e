"""Tests for sage.dynamics — living-connection math for halls + tunnels.
...
def _fresh_connection(created_at=None, **overrides):
    ...
    or nook_graph.py — no dynamics fields yet."""
    base = {
        "id": "test_conn_001",
        "wing": "test_wing",
        "entity_a": "Alpha",
...
class TestApplyDecay:
    ...
    def test_floors_at_strength_floor(self):
        """Strength never decays below ``STRENGTH_FLOOR`` — the nook
...
# ── Additional dynamics unit tests (relocated from test_bootstrap.py) ─────────


def test_dynamics_parse_iso_edge_cases():
    """_parse_iso handles None, datetime, non-string, and naive datetime inputs."""
    from datetime import datetime, timezone
    from sage_mcp.dynamics import _parse_iso

    # None → None (line 223)
    assert _parse_iso(None) is None

    # Already a datetime with tzinfo (line 224-225)
    dt_aware = datetime(2024, 1, 1, tzinfo=timezone.utc)
    assert _parse_iso(dt_aware) is dt_aware

    # datetime without tzinfo → gets UTC added (line 224-225 else branch, 239)
    dt_naive = datetime(2024, 1, 1)
    result = _parse_iso(dt_naive)
    assert result.tzinfo is not None

    # Non-string, non-datetime → None (line 226-227)
    assert _parse_iso(42) is None
    assert _parse_iso([]) is None

    # Empty string → None (line 226-227)
    assert _parse_iso("") is None

    # Valid ISO string with Z suffix → parsed OK (line 233-234)
    result2 = _parse_iso("2024-01-01T00:00:00Z")
    assert result2 is not None
    assert result2.tzinfo is not None

    # Naive ISO string → UTC added (line 239)
    result3 = _parse_iso("2024-01-01T12:00:00")
    assert result3 is not None
    assert result3.tzinfo is not None


def test_dynamics_potentiate_and_decay_edge_cases():
    """potentiate and apply_decay handle None now and edge inputs."""
    from datetime import datetime, timezone

    from sage_mcp.dynamics import apply_decay, initialize_dynamics_fields, potentiate

    now = datetime(2024, 1, 1, tzinfo=timezone.utc)

    # potentiate with now=None — exercises line 128 (datetime.now default)
    conn = {"created_at": "2024-01-01T00:00:00+00:00"}
    initialize_dynamics_fields(conn, now=now)
    result = potentiate(conn)  # no now kwarg → uses datetime.now()
    assert "strength" in result

    # apply_decay with bad last_activated timestamp → returns unchanged (line 191)
    conn2 = {"last_activated": "not-a-date", "strength": 1.0}
    initialize_dynamics_fields(conn2, now=now)
    conn2["last_activated"] = "not-a-date"  # re-set to bad value after init
    result2 = apply_decay(conn2, now=now)
    assert result2 is conn2  # returns same object unchanged

    # apply_decay with stability=0 → uses DEFAULT_STABILITY for the calc (line 200)
    conn3 = {"last_activated": "2024-01-01T00:00:00+00:00", "strength": 0.5, "stability": 0.0}
    initialize_dynamics_fields(conn3, now=now)
    conn3["stability"] = 0.0  # force it after init
    later = datetime(2024, 6, 1, tzinfo=timezone.utc)
    result3 = apply_decay(conn3, now=later)
    # strength should have decayed (stability reset to DEFAULT prevented /0 error)
    assert result3["strength"] <= 0.5

    # apply_decay with now=None → uses datetime.now() (line 179-180)
    conn4 = {"last_activated": "2024-01-01T00:00:00+00:00", "strength": 0.8}
    initialize_dynamics_fields(conn4, now=now)
    result4 = apply_decay(conn4)  # no now kwarg
    assert "strength" in result4

    # potentiate with unparseable timestamp → hours_since = 0.0 (line 140)
    conn5 = {}
    initialize_dynamics_fields(conn5, now=now)
    conn5["last_activated"] = "not-a-timestamp"
    conn5["created_at"] = "also-not-a-timestamp"
    result5 = potentiate(conn5, now=now)
    assert "strength" in result5
