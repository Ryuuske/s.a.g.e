...
from sage_mcp.closet_llm import (
    LLMConfig,
    _call_llm,
...
class TestCallLLM:
    ...
    def test_returns_none_on_invalid_json(self):
        ...
        with (
            ...
            patch("sage_mcp.closet_llm.time.sleep"),
        ):
            parsed, _ = _call_llm(cfg, "/tmp/x", "w", "r", "c")
...
class TestRegenerateClosets:
    def test_missing_config_returns_error(self, monkeypatch):
        monkeypatch.delenv("LLM_ENDPOINT", raising=False)
        monkeypatch.delenv("LLM_MODEL", raising=False)
        with tempfile.TemporaryDirectory() as nook:
            result = regenerate_closets(nook)
    ...
    def test_regen_purges_regex_closets_and_stamps_normalize_version(self, tmp_path):
        ...
        drawers = get_collection(nook, create=True)
        drawers.upsert(
            ids=["drawer_01"],
        ...
        closets = get_closets_collection(nook)
        upsert_closet_lines(
            closets,
        ...
        with patch("urllib.request.urlopen", side_effect=fake_urlopen):
            result = regenerate_closets(nook, cfg=cfg)
    ...
    def test_regen_paginates_drawer_fetch(self, tmp_path):
        ...
        batch_size=5000. A single get(limit=total, ...) on a nook with
        ...
        from sage_mcp import closet_llm as closet_llm_mod
        ...
        with (
            ...
            result = regenerate_closets(nook, cfg=cfg, dry_run=True)

        # Four paginated calls: (0), (5000), (10000), (12000 — empty terminator).
        # The loop now runs while True and breaks on an empty page, so the final
        # fetch at offset=n_drawers is the sentinel that ends the loop.
        assert len(get_calls) == 4, (
            f"expected 4 batched fetches (3 data + 1 empty), got {len(get_calls)}"
        )
        ...
        assert [c["offset"] for c in get_calls] == [0, 5000, 10_000, 12_000]
    ...
    def test_regen_by_source_aggregates_across_batches(self, tmp_path):
        ...
        with (
            ...
            regenerate_closets(nook, cfg=cfg)
    ...
    def test_regen_uses_basename_not_split_slash(self, tmp_path, monkeypatch):
        ...
        from sage_mcp.nook import get_collection, get_closets_collection
