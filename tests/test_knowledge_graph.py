...
import pytest
import sqlite3
from sage_mcp.knowledge_graph import KnowledgeGraph
...
class TestTemporalDateTimeCompatibility:
    def test_datetime_query_matches_date_only_fact(self, kg):
        kg.add_triple(
            "Alice",
            "ate_at",
            "Cafe",
    ...
    def test_datetime_query_before_date_only_fact_does_not_match(self, kg):
        kg.add_triple(
            "Alice",
            "ate_at",
            "Cafe",
    ...
    def test_datetime_query_after_date_only_fact_does_not_match(self, kg):
        kg.add_triple(
            "Alice",
            "ate_at",
            "Cafe",
...
# ── Additional knowledge_graph unit tests (relocated from test_bootstrap.py) ──


def test_knowledge_graph_temporal_none_inputs():
    """_temporal_start_key and _temporal_end_key return None for None input."""
    from sage_mcp.knowledge_graph import _temporal_start_key, _temporal_end_key

    assert _temporal_start_key(None) is None
    assert _temporal_end_key(None) is None

    # Date-only values get expanded
    assert _temporal_start_key("2024-01-15") == "2024-01-15T00:00:00Z"
    assert _temporal_end_key("2024-01-15") == "2024-01-15T23:59:59Z"

    # ISO datetime passthrough
    assert _temporal_start_key("2024-01-15T12:00:00Z") == "2024-01-15T12:00:00Z"


def test_kg_db_file_is_owner_only(tmp_path):
    """ADR-0077 / E2E round-C: the KG db holds plaintext facts — must be 0600.

    DEFAULT_KG_PATH was created at the umask default (0644); only the config-dir
    kg.sqlite was chmod'd. KnowledgeGraph.__init__ now hardens the db file."""
    from sage_mcp.knowledge_graph import KnowledgeGraph
    ...
    try:
        assert db.exists()
        assert (db.stat().st_mode & 0o777) == 0o600, f"KG db is {oct(db.stat().st_mode & 0o777)}"
    finally:
...
# ── read_counts_readonly tests ────────────────────────────────────────────────
# Findings [60]/[48] from pycore audit cluster: the fail-soft exception path
# and no-mutation invariant (mode=ro never creates -wal/-shm) were uncovered.


class TestReadCountsReadonly:
    """Direct unit tests for read_counts_readonly (ADR-0003 read-only contract)."""

    def test_nonexistent_path_returns_zero_zero(self, tmp_path):
        """Non-existent path returns (0, 0) without creating any file."""
        from sage_mcp.knowledge_graph import read_counts_readonly

        db_path = str(tmp_path / "no_such.sqlite3")
        result = read_counts_readonly(db_path)
        assert result == (0, 0)
        assert not (tmp_path / "no_such.sqlite3").exists(), (
            "read_counts_readonly must NOT create the file"
        )

    def test_garbage_file_fails_soft(self, tmp_path):
        """A schema-less/garbage file returns (0, 0) without raising."""
        from sage_mcp.knowledge_graph import read_counts_readonly

        garbage = tmp_path / "garbage.sqlite3"
        garbage.write_bytes(b"not a sqlite database at all")
        result = read_counts_readonly(str(garbage))
        assert result == (0, 0), f"expected (0, 0) for garbage file, got {result}"

    def test_no_mutation_on_real_kg(self, tmp_path):
        """read_counts_readonly does not create -wal or -shm sidecars."""
        from sage_mcp.knowledge_graph import KnowledgeGraph, read_counts_readonly
        ...
        # Record directory listing before read.
        before = set(tmp_path.iterdir())
        entities, triples = read_counts_readonly(str(db))
        after = set(tmp_path.iterdir())

        assert entities >= 1, "should count the entity we added"
        new_files = after - before
        sidecar_names = {p.name for p in new_files}
        assert not any(".wal" in n or ".shm" in n for n in sidecar_names), (
            f"read_counts_readonly created unexpected sidecars: {sidecar_names}"
        )
