...
from sage_mcp.searcher import _bm25_only_via_sqlite


COLLECTION = "nook_drawers"
...
def _seed_chroma_db(
    ...
    accordingly. Omit to model an older nook that pre-dates PR #1191.
    """
...
def _write_pickle(nook: str, segment_id: str, hnsw_count: int) -> None:
    ...
    production.
    """
    seg_dir = os.path.join(nook, segment_id)
...
def test_vector_segment_id_no_nook(tmp_path):
...
def test_refresh_preserves_sticky_flag_on_probe_failure(tmp_path, monkeypatch):
    """A transient capacity-probe failure must NOT clear a previously-detected
    divergence (ADR-0074 review M-62 / #1222 fail-safe): the sticky
    _vector_disabled flag is preserved, not flipped back to enabled."""
    from sage_mcp import mcp_server
    ...
    monkeypatch.setattr(mcp_server, "_config", _Cfg())
    monkeypatch.setattr(mcp_server, "_vector_disabled", True)
    monkeypatch.setattr(mcp_server, "_vector_disabled_reason", "known divergence")
    ...
    assert mcp_server._vector_disabled_reason == "known divergence"


def test_capacity_status_quiet_for_empty_nook(tmp_path):
    info = hnsw_capacity_status(str(tmp_path), COLLECTION)
    assert info["diverged"] is False
    assert info["status"] == "unknown"
...
def test_capacity_status_tolerates_lag_under_large_sync_threshold(tmp_path):
    ...
    Nooks created via sage's _HNSW_BLOAT_GUARD (sync_threshold=
    ...
    write cycle on any actively-mined ≥100K nook.
    """
    seg = "seg-bloat-guard"
...
def test_capacity_status_default_threshold_when_no_sync_metadata(tmp_path):
    """Older nooks without ``hnsw:sync_threshold`` fall back to 2000 floor.
...
def _seed_drawers(nook: str, segment_id: str, drawers: list[tuple[str, dict, str]]) -> None:
    """Insert (text, metadata, embedding_id) tuples into a seeded nook.
...
def _set_drawer_created_at(nook: str, timestamps: dict[int, str]) -> None:
    db_path = os.path.join(nook, "chroma.sqlite3")
    conn = sqlite3.connect(db_path)
    try:
...
@pytest.fixture
def nook_with_drawers(tmp_path):
    seg = "seg-bm25"
    ...
    drawers = [
        ...
        (
            "Memory nook technique using rooms and drawers for recall",
...
def test_bm25_fallback_returns_matches(nook_with_drawers):
    out = _bm25_only_via_sqlite("segfault chromadb", str(nook_with_drawers), n_results=5)
...
def test_bm25_fallback_filters_by_wing(nook_with_drawers):
    out = _bm25_only_via_sqlite(
        "memory nook recall", str(nook_with_drawers), wing="design", n_results=5
    )
...
def test_bm25_fallback_no_nook(tmp_path):
    out = _bm25_only_via_sqlite("anything", str(tmp_path))
    assert "error" in out


def test_bm25_fallback_handles_short_query(nook_with_drawers):
    """Single-character tokens are unmatchable in trigram FTS5 — must
    not crash, must fall back to the recency window."""
    out = _bm25_only_via_sqlite("a", str(nook_with_drawers), n_results=5)
...
def test_repair_status_reports_diverged(tmp_path, capsys):
    ...
    from sage_mcp.repair import status as repair_status

    seg = "seg-status"
    _seed_chroma_db(str(tmp_path), sqlite_count=20_000, segment_id=seg)
    _write_pickle(str(tmp_path), seg, hnsw_count=2_000)
    out = repair_status(nook_path=str(tmp_path))
    captured = capsys.readouterr().out
    assert "DIVERGED" in captured
    assert "sage repair`" in captured
    assert out["drawers"]["diverged"] is True


def test_repair_status_quiet_on_healthy_nook(tmp_path, capsys):
    from sage_mcp.repair import status as repair_status

    seg = "seg-status-ok"
    _seed_chroma_db(str(tmp_path), sqlite_count=500, segment_id=seg)
    _write_pickle(str(tmp_path), seg, hnsw_count=480)
    repair_status(nook_path=str(tmp_path))
    captured = capsys.readouterr().out
...
def test_tool_status_via_sqlite_returns_breakdown(nook_with_drawers, monkeypatch):
    """When _vector_disabled is set, tool_status reads counts from sqlite
    instead of opening a chromadb client."""
    from sage_mcp import mcp_server

    # _config.nook_path is a read-only property; swap the whole object
    # for a tiny stand-in so we don't have to monkey with the real
    # SageConfig.
    class _Cfg:
        nook_path = str(nook_with_drawers)
        collection_name = "nook_drawers"
    ...
    # Wing breakdown comes from the seeded nook_with_drawers fixture:
