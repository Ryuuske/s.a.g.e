"""Tests for sage.entity_detector."""
...
from sage_mcp.entity_detector import (
    PROSE_EXTENSIONS,
    _print_entity_list,
    classify_entity,
    confirm_entities,
...
def test_extract_candidates_filters_code_as_common_content_word():
    ...
    content word, not a proper noun. The current nook's known_entities.json
...
def test_extract_candidates_filters_all_known_false_positives():
    """Every known false-positive entity in a real test nook must be
...
def test_extract_candidates_does_not_filter_real_names():
    ...
    text = "Riley said hi. Riley laughed. Riley smiled. Alice watched. Alice laughed. Alice smiled."
    result = extract_candidates(text)
    assert "Riley" in result, (
        f"Real name 'Riley' should NOT be filtered; got: {list(result.keys())!r}"
    )
    assert "Alice" in result, (
        f"Real name 'Alice' should NOT be filtered; got: {list(result.keys())!r}"
    )
...
def test_coca_wordlist_file_loads_with_expected_shape():
    ...
    pkg_dir = Path(sage_mcp.__file__).parent
...
def test_coca_wordlist_contains_all_known_false_positives():
    """Direct data-level assertion: the wordlist must contain every known
    false positive from a real test nook, lowercased."""
    import json
    from pathlib import Path
...
def test_extract_candidates_real_single_word_name_still_detected():
    ...
    'Alice' appearing as a standalone name 3+ times still gets detected."""
    text = "Alice wrote the spec. Alice reviewed Cedar's PR. Alice shipped to main."
    result = extract_candidates(text)
    assert "Alice" in result, (
        f"Single-word real name 'Alice' should still be detected; got: {list(result.keys())!r}"
    )
...
def test_stopwords_contains_common_words():
    stopwords = get_entity_patterns(("en",))["stopwords"]
...
def _temp_locale(locale_code: str, entity_section: dict):
    """Context manager that drops a locale JSON into sage/i18n/ for the test body.
    ...
    Note: writes into the real sage/i18n/ directory. If a test process is
    ...
    Recover with `rm sage/i18n/zz-test-*.json`.
    """
    from sage_mcp import i18n
    from sage_mcp import entity_detector
...
def test_build_patterns_cache_is_keyed_by_language():
    ...
    from sage_mcp.entity_detector import _build_patterns
...
def test_normalize_langs_handles_string_input():
    ...
    from sage_mcp.entity_detector import _normalize_langs
...
def test_config_entity_languages_defaults_to_english(tmp_path, monkeypatch):
    """SageConfig.entity_languages defaults to ['en'] with no config file."""
    from sage_mcp.config import SageConfig

    monkeypatch.delenv("SAGE_ENTITY_LANGUAGES", raising=False)
    monkeypatch.delenv("SAGE_ENTITY_LANGUAGES", raising=False)
...
def test_config_entity_languages_from_env(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("SAGE_ENTITY_LANGUAGES", "en,pt-br,ru")
    cfg = SageConfig(config_dir=str(tmp_path))
...
def test_config_set_entity_languages_persists(tmp_path, monkeypatch):
    ...
    cfg2 = SageConfig(config_dir=str(tmp_path))
