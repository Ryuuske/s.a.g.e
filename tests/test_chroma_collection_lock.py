"""Tests for ChromaCollection's nook-write-lock integration.

Closes the gap left by ``mine_nook_lock`` only protecting the
``sage mine`` pipeline: MCP/direct writers that call
...
* ``ChromaCollection(c, nook_path=p)`` wraps every write with
  ``mine_nook_lock(p)``.
...
* Re-entrant composition with ``miner.mine()`` does not self-deadlock:
  ``with mine_nook_lock(p): col.upsert(...)`` runs to completion.
* ``ChromaCollection(c)`` (no nook_path) preserves the no-lock
  behaviour for tests/callers that build the adapter directly without
  going through ``ChromaBackend``.

POSIX-only: ``mine_nook_lock`` uses ``fcntl`` on Unix and ``msvcrt`` on
...
from sage_mcp.nook import MineAlreadyRunning, mine_nook_lock


def _get_mp_context():
    """Same start-method picker as test_nook_locks.py — ``spawn`` everywhere.
...
def _hold_lock(nook_path: str, ready_flag: str, release_flag: str) -> int:
    """Acquire ``mine_nook_lock``, signal readiness, wait for release.

    Mirrors the helper in ``test_nook_locks.py`` so the contention
...
def test_nook_path_none_skips_lock(tmp_path, monkeypatch):
    """Older callers (``ChromaCollection(c)``) keep no-lock behaviour.

    A ``ChromaCollection`` built without ``nook_path`` must not touch the
    ...
    col = ChromaCollection(fake)  # no nook_path -> no lock

    # Hold the lock in a child process. Without nook_path, the parent
...
def test_writer_blocks_during_mine(tmp_path, monkeypatch):
    """A held ``mine_nook_lock`` causes ``ChromaCollection`` writes to raise.
    ...
    try:
        ...
        fake = _FakeChromaCollection()
        col = ChromaCollection(fake, nook_path=nook)
...
def test_reentrant_inside_mine_passes_through(tmp_path, monkeypatch):
    """``ChromaCollection.upsert`` inside ``mine_nook_lock`` does not deadlock.

    ``miner.mine()`` already holds ``mine_nook_lock(nook_path)`` for the
    full mine pipeline; ``_mine_body`` then calls
    ``collection.upsert(...)``. With the per-thread re-entrant guard in
    ``mine_nook_lock``, the inner acquire is a pass-through and the
...
class _SlowFakeChromaCollection(_FakeChromaCollection):
    ...
    Used to keep ``mine_nook_lock`` acquired long enough for a sibling
...
def _slow_writer_target(nook_path, tmp_path_str, pid, result_q):
    ...
    from sage_mcp.backends.chroma import ChromaCollection as _CC
    from sage_mcp.nook import MineAlreadyRunning as _MAR

    fake = _SlowFakeChromaCollection(hold_seconds=0.3)
    col = _CC(fake, nook_path=nook_path)
    try:
...
def test_concurrent_writers_serialize(tmp_path, monkeypatch):
    ...
    nook must be serialized: at most one enters chromadb at a time, the
    ...
    p1 = ctx.Process(target=_slow_writer_target, args=(nook, str(tmp_path), 1, result_q))
    p2 = ctx.Process(target=_slow_writer_target, args=(nook, str(tmp_path), 2, result_q))
    p1.start()
...
def test_read_path_does_not_acquire_lock(tmp_path, monkeypatch):
    ...
    try:
        ...
        # need to confirm the wrapper does not call into mine_nook_lock
