...
from sage_mcp.exporter import export_nook
...
def _setup_nook(tmpdir):
    """Create a small nook with drawers across two wings for testing."""
    project_a = Path(tmpdir) / "project_a"
    project_b = Path(tmpdir) / "project_b"
    nook_path = str(Path(tmpdir) / "nook")
    ...
    with open(project_a / "sage.yaml", "w") as f:
        yaml.dump(
            {
                "wing": "alpha",
                "rooms": [
    ...
    with open(project_b / "sage.yaml", "w") as f:
        yaml.dump(
            {
                "wing": "beta",
    ...
    mine(str(project_a), nook_path)
    mine(str(project_b), nook_path)
...
def test_export_creates_structure():
    ...
    try:
        nook_path = _setup_nook(tmpdir)
        output_dir = os.path.join(tmpdir, "export")

        stats = export_nook(nook_path, output_dir)
...
def test_export_markdown_content():
    ...
    try:
        ...
        export_nook(nook_path, output_dir)
...
def test_export_empty_nook():
    tmpdir = tempfile.mkdtemp()
    try:
        nook_path = os.path.join(tmpdir, "empty_nook")
        output_dir = os.path.join(tmpdir, "export")
