...
The project miner (sage.miner.scan_project) walks a directory and
...
from sage_mcp.miner import MAX_FILE_SIZE, READABLE_EXTENSIONS, scan_project
...
class TestJsonlNotSilentlySkipped:
    def test_jsonl_in_readable_extensions(self):
        ...
        `.json` is already there (see sage/miner.py:30). `.jsonl`
        ...
        assert ".jsonl" in READABLE_EXTENSIONS, (
            "sage/miner.py:READABLE_EXTENSIONS contains `.json` "
            ...
            "reported by users — the hook fires `sage mine`, the "
    ...
    def test_scan_project_picks_up_jsonl_file(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            jsonl_path.write_text(
                ...
                '{"role": "assistant", "content": "pip install.*sage"}\n'
            )
    ...
    def test_large_jsonl_not_silently_dropped_by_size_cap(self):
        ...
        The previous cap was 10 MB, which is smaller than a long Claude Code
