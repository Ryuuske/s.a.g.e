"""Tests for sage.convo_scanner."""

import json
from pathlib import Path

from sage_mcp.convo_scanner import (
...
# ── Additional convo_scanner unit tests (relocated from test_bootstrap.py) ────


def test_convo_scanner_extract_cwd_edge_cases(tmp_path):
    """_extract_cwd_from_session handles empty lines and MAX_HEADER_LINES limit."""
    from sage_mcp.convo_scanner import _extract_cwd_from_session, MAX_HEADER_LINES

    # File with empty lines (exercises line 68-69 continue)
    session_file = tmp_path / "session_empty_lines.jsonl"
    lines = ["\n", "\n", json.dumps({"cwd": "/my/project"})]
    session_file.write_text("".join(line + "\n" for line in lines))
    assert _extract_cwd_from_session(session_file) == "/my/project"

    # File with more than MAX_HEADER_LINES before cwd (exercises line 65-66 break)
    session_over = tmp_path / "session_over_limit.jsonl"
    # Write MAX_HEADER_LINES+1 lines with no cwd, then a line with cwd
    over_content = "\n".join(json.dumps({"text": f"line {i}"}) for i in range(MAX_HEADER_LINES + 2))
    over_content += "\n" + json.dumps({"cwd": "/unreachable"})
    session_over.write_text(over_content)
    # cwd is past the limit; should return None
    result = _extract_cwd_from_session(session_over)
    assert result is None  # cwd was past header limit
