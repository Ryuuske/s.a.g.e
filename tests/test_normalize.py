...
from sage_mcp.normalize import (
    _extract_content,
    _format_tool_result,
    _format_tool_use,
...
def test_format_tool_use_unknown_tool():
    block = {
        "type": "tool_use",
        "id": "t1",
        "name": "mcp__sage__search",
        "input": {"query": "firmware probe", "limit": 5},
    }
    result = _format_tool_use(block)
    assert result.startswith("[mcp__sage__search]")
...
def test_format_tool_result_read_omitted():
    """Read results are omitted (content already in nook from project mining)."""
...
def test_format_tool_result_unknown_short():
    ...
    result = _format_tool_result("some output", "mcp__sage__search")
...
def test_messages_to_transcript_basic():
    ...
    with patch("sage_mcp.normalize.spellcheck_user_text", side_effect=lambda x: x, create=True):
...
def test_normalize_rejects_large_file():
    ...
    with patch("sage_mcp.normalize.os.path.getsize", return_value=600 * 1024 * 1024):
        try:
