...
from sage_mcp.miner import detect_room, load_config, mine, scan_project, status
from sage_mcp.nook import (
    NORMALIZE_VERSION,
    bulk_check_mined,
    file_already_mined,
    prefetch_mined_set,
...
def test_project_mining():
    ...
    try:
        ...
        with open(project_root / "sage.yaml", "w") as f:
            yaml.dump(
                {
                    "wing": "test_project",
                    "rooms": [
        ...
        nook_path = project_root / "nook"
        mine(str(project_root), str(nook_path))
...
def test_mine_computes_hallways_for_wing_post_mine(monkeypatch):
    ...
    from sage_mcp import miner as miner_mod
    ...
    # Patch at the call site (sage.miner.compute_hallways_for_wing) so
...
def test_mine_computes_entity_tunnels_for_wing_post_mine(monkeypatch):
    ...
    must be called once with the wing name from sage_mcp.yaml.

    Without this call the entity-tunnel feature is dead code — no miner
    triggers it, no entity tunnels ever land in ~/.sage/tunnels.json.
    ...
    # Patch at the call site (sage.miner._compute_entity_tunnels_for_wing)
...
def test_scan_project_skips_sage_generated_files():
    with tempfile.TemporaryDirectory() as tmpdir:
        project_root = Path(tmpdir).resolve()
        write_file(project_root / "entities.json", '{"people": [], "projects": []}')
        write_file(project_root / "sage.yaml", "wing: test\nrooms: []\n")
...
def test_entity_metadata_finds_cyrillic_names(monkeypatch):
    ...
    import sage_mcp.nook as nook_mod
    from sage_mcp.miner import _extract_entities_for_metadata

    # Reset cached patterns so they reload with the monkeypatched languages
    monkeypatch.setattr(nook_mod, "_CANDIDATE_RX_CACHE", None)
    monkeypatch.setattr(
        "sage_mcp.config.SageConfig.entity_languages",
...
def test_entity_metadata_matches_known_names_case_insensitively(monkeypatch):
    ...
    updated to use the same flag — so an entry like ``"Alice"`` in
    known_entities.json fails to match the lowercase mention ``alice`` that
    ...
    """
    from sage_mcp import miner

    # Stub the known-entity registry to a controlled set
    monkeypatch.setattr(miner, "_load_known_entities", lambda: frozenset({"Alice", "Bob"}))

    # Lowercase mentions of seeded names must still be tagged.
    result = miner._extract_entities_for_metadata("alice talked to bob today about the nook.")
    matched = set(result.split(";")) if result else set()
    assert "Alice" in matched, (
        f"lowercase 'alice' must match the seeded 'Alice' (case-insensitive). Got: {matched!r}"
    )
    assert "Bob" in matched, (
        f"lowercase 'bob' must match the seeded 'Bob' (case-insensitive). Got: {matched!r}"
    )

    # Mixed case must also match
    result_mixed = miner._extract_entities_for_metadata("Alice saw bob. ALICE waved.")
    matched_mixed = set(result_mixed.split(";")) if result_mixed else set()
    assert "Alice" in matched_mixed
    assert "Bob" in matched_mixed


def test_file_already_mined_check_mtime():
    tmpdir = tempfile.mkdtemp()
    try:
        nook_path = os.path.join(tmpdir, "nook")
...
def test_mine_dry_run_with_tiny_file_no_crash():
    ...
    try:
        ...
        mine(str(project_root), str(nook_path), dry_run=True)
    finally:
        shutil.rmtree(tmpdir, ignore_errors=True)


def test_status_missing_nook_does_not_create_empty_collection(tmp_path, capsys):
    nook_path = tmp_path / "missing-nook"

    status(str(nook_path))
...
def test_status_initialized_but_empty_nook_reports_empty(tmp_path, capsys):
    """State C from #1498: nook dir + chroma.sqlite3 exist but no drawers
    have been mined yet. status() must print the 'initialized but empty'
    message and suggest `sage mine`, not the misleading 'No nook
    found' / 'Run init' message."""
    import chromadb

    nook_path = tmp_path / "empty-nook"
    nook_path.mkdir()
    chromadb.PersistentClient(path=str(nook_path))  # creates chroma.sqlite3
    ...
    assert "sage mine" in out
    assert "No nook found" not in out


def test_status_nook_dir_without_db_reports_uninitialized(tmp_path, capsys):
    """State B from #1498: nook dir exists but chroma.sqlite3 is absent.
...
def test_status_handles_none_metadata_without_crash(tmp_path, capsys):
    ...
    Nooks can contain drawers whose metadata was never set (older mining
    ...
    with patch("sage_mcp.miner._open_collection_or_explain", return_value=FakeCol()):
        status(str(tmp_path))
...
def test_file_already_mined_returns_false_for_stale_normalize_version():
    ...
    try:
        ...
        os.makedirs(nook_path)
        client = chromadb.PersistentClient(path=nook_path)
        col = client.get_or_create_collection("nook_drawers")
...
def test_add_drawer_stamps_normalize_version(tmp_path):
    ...
    from sage_mcp.miner import add_drawer
...
def test_mine_creates_topic_tunnels_for_shared_topics(tmp_path, monkeypatch):
    ...
    Issue #1180.
    """
    from sage_mcp import miner, nook_graph

    # Redirect both the registry and tunnel-storage paths into tmp_path
    # so we never touch the developer's real ~/.sage directory.
    ...
    monkeypatch.setattr(nook_graph, "_get_tunnel_file", lambda *a, **kw: str(tunnels_file))
...
def test_mine_no_tunnel_when_threshold_blocks_overlap(tmp_path, monkeypatch):
    """Bumping ``SAGE_TOPIC_TUNNEL_MIN_COUNT`` above the actual overlap
    ...
    tunnels_file = tmp_path / "tunnels.json"
    monkeypatch.setattr(nook_graph, "_get_tunnel_file", lambda *a, **kw: str(tunnels_file))
    monkeypatch.setenv("SAGE_TOPIC_TUNNEL_MIN_COUNT", "2")
...
def test_mine_keyboard_interrupt_prints_summary_and_exits_130(tmp_path, capsys):
    ...
    with patch("sage_mcp.miner.process_file", side_effect=fake_process_file):
...
def test_mine_keyboard_interrupt_quotes_path_with_spaces_in_resume_hint(tmp_path, capsys):
    ...
    spaces / metacharacters yields a copy-paste-safe `sage mine ...`
    ...
    assert f"sage mine {shlex.quote(str(project_root))}" in out
...
def test_process_file_skips_when_chunks_exceed_max(tmp_path, monkeypatch, capsys):
    ...
    monkeypatch.delenv("SAGE_MAX_CHUNKS_PER_FILE", raising=False)
    ...
    assert "SAGE_MAX_CHUNKS_PER_FILE" in captured.err
...
def test_resolve_max_chunks_env_var_wins_over_default(monkeypatch):
    ...
    monkeypatch.setenv("SAGE_MAX_CHUNKS_PER_FILE", "777")
...
def test_resolve_max_chunks_sentinel_zero_disables(monkeypatch):
    ...
    monkeypatch.setenv("SAGE_MAX_CHUNKS_PER_FILE", "0")
...
def test_resolve_max_chunks_invalid_env_falls_back_to_default(monkeypatch, capsys):
    ...
    monkeypatch.setenv("SAGE_MAX_CHUNKS_PER_FILE", "banana")
    assert miner._resolve_max_chunks_per_file(None) == miner.MAX_CHUNKS_PER_FILE
    err = capsys.readouterr().err
    assert "SAGE_MAX_CHUNKS_PER_FILE" in err
...
def test_resolve_max_chunks_negative_env_falls_back_to_default(monkeypatch, capsys):
    ...
    monkeypatch.setenv("SAGE_MAX_CHUNKS_PER_FILE", "-5")
    assert miner._resolve_max_chunks_per_file(None) == miner.MAX_CHUNKS_PER_FILE
    err = capsys.readouterr().err
    assert "SAGE_MAX_CHUNKS_PER_FILE" in err
    assert "-5" in err
...
def test_mine_plumbs_max_chunks_per_file_to_process_file(tmp_path):
    ...
    with patch("sage_mcp.miner.process_file", side_effect=fake_process_file):
        mine(str(project_root), str(nook_path), max_chunks_per_file=0)
...
def test_mine_cleans_up_pid_file_on_interrupt(tmp_path):
    ...
    with (
        patch.dict(os.environ, {"SAGE_MINE_PID_FILE": str(pid_file)}),
        patch("sage_mcp.miner.process_file", side_effect=fake_process_file),
    ):
        with pytest.raises(SystemExit):
            mine(str(project_root), str(nook_path))
...
def test_mine_cleans_up_pid_file_on_clean_exit(tmp_path):
    ...
    with patch.dict(os.environ, {"SAGE_MINE_PID_FILE": str(pid_file)}):
        mine(str(project_root), str(nook_path))
...
class TestBuildDrawerMetadataLineRange:
    ...
    too. When it doesn't (older miners that omitted it), the function omits
    the keys entirely — backward compatible.
    """

    def test_includes_line_range_when_provided(self):
        from sage_mcp.miner import _build_drawer_metadata
...
class TestExtractContentDate:
    ...
    def test_filename_iso_extracts(self, tmp_path):
        from sage_mcp.miner import _extract_content_date
    ...
    # ── Negative cases — date-extraction regression cases ──────────────
    # Per code review: dateutil.parser.parse(fuzzy=True)
    ...
    def test_no_hallucination_content_with_version_number(self):
        """Content 'Version 7.2.1 released' returned '2006-03-03' before fix."""
        from sage_mcp.miner import _extract_content_date

        content = "Version 7.2.1 released today with new features.\n"
    ...
    # ── Two-digit year boundary edge case ─────────────────
...
# =============================================================================
# K1 (ADR-0070): batch prefetch on the project-miner pre-lock skip path.
# bulk_check_mined returns a version-gated source_file -> source_mtime map so
# `sage bootstrap` does ONE paginated scan instead of one ChromaDB query per
# file; process_file uses it as an O(1) local skip.
# =============================================================================


def test_bulk_check_mined_returns_versioned_mtime_map():
    """bulk_check_mined returns source->mtime only for current-version drawers
    with an mtime; stale-version or mtime-less rows are excluded so the caller
    re-mines them (mirrors file_already_mined(check_mtime=True))."""
    tmpdir = tempfile.mkdtemp()
    try:
        nook_path = os.path.join(tmpdir, "nook")
        os.makedirs(nook_path)
        ...
        col = client.get_or_create_collection("nook_drawers", metadata={"hnsw:space": "cosine"})
        col.add(
            ids=["cur", "stale", "nomtime"],
            documents=["a", "b", "c"],
            metadatas=[
                # current version + mtime → included
                {
                    "source_file": "/repo/cur.py",
                    "source_mtime": "123.5",
                    "normalize_version": NORMALIZE_VERSION,
                },
                # stale version → excluded (must re-mine after a normalize upgrade)
                {
                    "source_file": "/repo/stale.py",
                    "source_mtime": "200.0",
                    "normalize_version": NORMALIZE_VERSION - 1,
                },
                # no mtime → excluded (check_mtime can't verify it)
                {
                    "source_file": "/repo/nomtime.py",
                    "normalize_version": NORMALIZE_VERSION,
                },
            ],
        )
        mined = bulk_check_mined(col)
        assert mined == {"/repo/cur.py": 123.5}
    finally:
        del col, client
        shutil.rmtree(tmpdir, ignore_errors=True)


def test_bulk_check_mined_tolerates_none_metadata():
    """A None metadata row (partial flush) must not crash the bulk scan."""

    class _FakeCol:
        def count(self):
            return 2

        def get(self, limit, offset, include=None):
            rows = [
                {
                    "source_file": "/x.py",
                    "source_mtime": "1.0",
            ...
            return {"ids": [f"i{offset}"], "metadatas": rows}

    mined = bulk_check_mined(_FakeCol())
    assert mined == {"/x.py": 1.0}


def test_process_file_prefetched_skip_avoids_db_query(tmp_path):
    """When prefetched_mtimes carries the file at its current mtime, process_file
    skips WITHOUT touching the collection (the pre-lock query is bypassed)."""

    from sage_mcp import miner

    class _ExplodingCol:
        def get(self, *a, **k):  # any DB touch on the skip path is a regression
            raise AssertionError("process_file queried the collection despite a prefetch hit")

    f = tmp_path / "mined.py"
    f.write_text("print('x')\n")
    prefetched = {str(f): os.path.getmtime(f)}
    drawers, room, skip = miner.process_file(
        filepath=f,
        project_path=tmp_path,
        collection=_ExplodingCol(),
        wing="sage",
        rooms=[{"name": "general", "keywords": []}],
        agent="t",
        dry_run=False,
        prefetched_mtimes=prefetched,
    )
    assert (drawers, skip) == (0, None)


def test_process_file_prefetched_miss_does_not_skip(tmp_path):
    """A changed mtime (prefetch entry stale) must NOT skip — the file falls
    through to mining (reaches collection.upsert), never silently treated as
    up-to-date. The authoritative post-lock query reports 'not mined' so the
    fresh content is filed."""
    from sage_mcp import miner

    f = tmp_path / "changed.py"
    f.write_text("fresh content here, long enough to chunk and file as a drawer\n" * 5)
    # Prefetched mtime is deliberately wrong (file changed since the scan).
    prefetched = {str(f): os.path.getmtime(f) - 999.0}

    filed = {}

    class _Col:
        def get(self, *a, **k):  # post-lock re-check: report "not mined"
    ...
    assert filed.get("mined") is True
