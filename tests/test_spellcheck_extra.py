...
class TestLoadKnownNames:
    def test_returns_names_from_registry(self):
        ...
        with patch("sage_mcp.entity_registry.EntityRegistry") as MockER:
    ...
    def test_returns_empty_on_exception(self):
        with patch(
            "sage_mcp.entity_registry.EntityRegistry.load",
...
class TestSpellerEdgeCases:
    ...
    def test_system_word_not_corrected(self):
        ...
        with patch("sage_mcp.spellcheck._get_speller", return_value=fake_speller):
            with patch("sage_mcp.spellcheck._get_system_words", return_value={"coherently"}):
                with patch("sage_mcp.spellcheck._load_known_names", return_value=set()):
