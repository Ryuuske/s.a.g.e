...
import sage_mcp.embedding as embedding  # noqa: E402  (after importorskip)
...
def test_missing_deps_raise_helpful_error(monkeypatch):
    ...
    ef = embedding.EmbeddinggemmaONNX()
    with pytest.raises(ImportError, match=r"pip install --force-reinstall -e"):
        ef(["anything"])


def test_config_embedding_model_env_override(monkeypatch):
    """SAGE_EMBEDDING_MODEL env var must override the config file default."""
    from sage_mcp.config import SageConfig

    monkeypatch.setenv("SAGE_EMBEDDING_MODEL", "embeddinggemma")
    assert SageConfig().embedding_model == "embeddinggemma"

    monkeypatch.setenv("SAGE_EMBEDDING_MODEL", "MiniLM")  # case-insensitive
    assert SageConfig().embedding_model == "minilm"
...
def test_config_embedding_model_default_is_minilm(monkeypatch):
    ...
    monkeypatch.delenv("SAGE_EMBEDDING_MODEL", raising=False)
    assert SageConfig().embedding_model == "minilm"
