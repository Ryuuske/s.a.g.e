...
from sage_mcp.nook_graph import (
    create_tunnel,
    delete_tunnel,
    follow_tunnels,
    list_tunnels,
)
from sage_mcp.searcher import (
    _bm25_scores,
...
def _lock_worker(target: str, name: str, hold_seconds: float, log_path: str) -> None:
    ...
    from sage_mcp.nook import mine_lock as _mine_lock
...
class TestMineLock:
    def test_lock_acquires_and_releases(self, tmp_path):
        ...
        with mine_lock(target):
            lock_dir = os.path.expanduser("~/.sage/locks")
...
class TestBuildClosetLines:
    ...
    def test_real_proper_nouns_survive_stoplist(self):
        content = (
            "Bob reviewed the diff. Alice wrote the spec. "
            "Bob pushed the fix. Alice approved the PR. "
            "Bob and Alice shipped together."
        )
        lines = build_closet_lines("/x.md", ["d1"], content, "w", "r")
        joined_entities = ";".join(line.split("|")[1] for line in lines)
        assert "Bob" in joined_entities
        assert "Alice" in joined_entities
    ...
    def test_falls_back_to_3_segment_format_when_metas_missing(self):
        ...
        lines = build_closet_lines("/x.md", ["d1"], content, "w", "r")
        for line in lines:
            assert len(line.split("|")) == 3, f"expected 3-segment format, got {line!r}"
    ...
    def test_falls_back_to_3_segment_format_when_metas_lack_line_keys(self):
        ...
        built from those drawers must emit the 3-segment form, not a
    ...
    def test_content_date_preferred_over_filed_at(self):
        ...
        ingestion-time (the date the chunk was mined). Critical for older
...
class TestUpsertClosetLines:
    def test_overwrites_existing_closet_does_not_append(self, nook_path):
        col = get_closets_collection(nook_path)
    ...
    def test_packs_into_multiple_closets_without_splitting_lines(self, nook_path):
        col = get_closets_collection(nook_path)
...
class TestPurgeFileClosets:
    def test_deletes_only_the_targeted_source(self, nook_path):
        col = get_closets_collection(nook_path)
        col.upsert(
...
class TestMinerClosetRebuild:
    def test_remine_replaces_closets_completely(self, tmp_path):
        ...
        nook = tmp_path / "nook"
        mine(str(project), str(nook), wing_override="proj", agent="test")

        col = get_closets_collection(str(nook))
    ...
    def test_production_miner_emits_4_segment_pointers_with_content_date(self, tmp_path):
        """Regression for date-extraction Issue #1.
...
class TestSearchMemoriesHybrid:
    def test_pure_drawer_when_no_closets(self, nook_path, seeded_collection):
        """Nooks without closets return results via direct drawer search —
    ...
    def test_closet_boost_marks_hit_as_drawer_plus_closet(self, nook_path, seeded_collection):
        ...
        closets = get_closets_collection(nook_path)
    ...
    def test_max_distance_filters_hybrid_hits(self, nook_path, seeded_collection):
        closets = get_closets_collection(nook_path)
        closets.upsert(
            ids=["closet_proj_backend_aaa_01"],
            documents=["JWT auth tokens|;|→drawer_proj_backend_aaa"],
...
class TestEntityMetadata:
    def test_extracts_capitalized_names(self):
        text = "Ben reviewed the code. Ben approved it. Carol flagged two issues. Carol fixed them."
...
class TestDiaryIngest:
    def test_ingest_creates_drawers_and_closets(self, tmp_path):
        ...
        result = ingest_diaries(str(diary_dir), str(nook_dir), force=True)
        assert result["days_updated"] >= 1
        assert get_collection(str(nook_dir)).count() >= 1
    ...
    def test_ingest_skips_unchanged_on_second_run(self, tmp_path):
        ...
        ingest_diaries(str(diary_dir), str(nook_dir), force=True)
        result = ingest_diaries(str(diary_dir), str(nook_dir))
    ...
    def test_ingest_detects_same_size_content_edit(self, tmp_path):
        ...
        drawers = get_collection(str(nook_dir)).get(where={"source_file": str(diary_file)})
        ...
        closets = get_closets_collection(str(nook_dir)).get(where={"source_file": str(diary_file)})
        joined_closets = "\n".join(closets["documents"])
        assert "Teh elaborate" not in joined_closets, "closet index still holds stale content"

    def test_earlier_state_backfills_content_hash(self, tmp_path):
        # Upgraded users can carry earlier-state entries without ``content_hash``.
        ...
        from sage_mcp.diary_ingest import _state_file_for, ingest_diaries

        # Simulate a earlier-state file: only size + entry_count, no content_hash.
        state_file = _state_file_for(str(nook_dir), diary_dir.resolve())
        ...
        assert "content_hash" in entry, "the older skip path must record the hash"
    ...
    def test_state_file_lives_outside_diary_dir(self, tmp_path):
        ...
        # State file does exist under ~/.sage/state/.
        state_path = _state_file_for(str(nook_dir), diary_dir.resolve())
        assert state_path.exists()
        # Platform-neutral path check: compare parents rather than a hardcoded
        # separator string that would fail on Windows (``\.sage\state\``).
        assert state_path.parent.name == "state"
        assert state_path.parent.parent.name == ".sage"
    ...
    def test_wing_prefixed_drawer_id_prevents_cross_diary_collision(self, tmp_path):
        ...
        # Two separate diary dirs, ingested into the same nook under
        ...
        from sage_mcp.diary_ingest import _diary_drawer_id_entry, ingest_diaries

        ingest_diaries(str(personal_dir), str(nook_dir), wing="personal", force=True)
        ingest_diaries(str(work_dir), str(nook_dir), wing="work", force=True)

        col = get_collection(str(nook_dir))
    ...
    def test_diary_multiple_entries_one_drawer_each(self, tmp_path):
        ...
        ingest_diaries(str(diary_dir), str(nook_dir), force=True)
        count = get_collection(str(nook_dir)).count()
    ...
    def test_large_diary_with_oversized_entry_chunks_within_entry(self, tmp_path):
        ...
        # assertion without crashing the test infrastructure.
        big_body = "X" * 1500
        (diary_dir / "2026-04-13.md").write_text(
            f"# 2026-04-13\n\n"
            f"## 10:00 — first\n\nfirst body content here, well above min size.\n\n"
    ...
    def test_incremental_appends_new_entry_only(self, tmp_path):
        ...
        ingest_diaries(str(diary_dir), str(nook_dir), force=True)
        initial = get_collection(str(nook_dir)).count()
        ...
        ingest_diaries(str(diary_dir), str(nook_dir))
        final = get_collection(str(nook_dir)).count()
    ...
    def test_entry_count_watermark_matches_drawer_count(self, tmp_path):
        ...
        from sage_mcp.diary_ingest import _state_file_for, _split_entries, ingest_diaries

        ingest_diaries(str(diary_dir), str(nook_dir), force=True)

        state = json.loads(_state_file_for(str(nook_dir), diary_dir.resolve()).read_text())
    ...
    def test_diary_atomic_batched_upsert_on_failure(self, tmp_path, monkeypatch):
        ...
        from sage_mcp import diary_ingest
        ...
        with pytest.raises(RuntimeError, match="simulated embedding model failure"):
            diary_ingest.ingest_diaries(str(diary_dir), str(nook_dir), force=True)
        ...
        real_col = real_get_collection(str(nook_dir))
...
class TestTunnels:
    ...
    ``<nook_path_parent>/tunnels.json``. Each test points the resolver at
    a fresh tmp file so tests don't cross-contaminate or touch the user's
    real tunnels."""

    def setup_method(self):
        import sage_mcp.nook_graph as pg
        ...
        # Neutralize the endpoint-existence check so these tests
        # don't get rejected when an earlier test module has bound
        # _get_collection to a real backend.
...
class TestDrawerGrepExpansion:
    def _seed_source_file(self, nook_path, source: str, n_chunks: int):
        """Helper: put N sequential drawers for a single source file into
        the nook and return the drawer IDs keyed by chunk_index."""
        col = get_collection(nook_path)
    ...
    def test_expand_returns_matched_plus_neighbors(self, nook_path):
        col, by_idx = self._seed_source_file(nook_path, "/proj/doc.md", n_chunks=5)
    ...
    def test_expand_at_start_of_file_only_has_next_neighbor(self, nook_path):
        col, _ = self._seed_source_file(nook_path, "/proj/edge_start.md", n_chunks=3)
    ...
    def test_expand_at_end_of_file_only_has_prev_neighbor(self, nook_path):
        col, _ = self._seed_source_file(nook_path, "/proj/edge_end.md", n_chunks=3)
        out = _expand_with_neighbors(
            col,
            "chunk_2 content",
    ...
    def test_expand_single_drawer_file_returns_just_matched(self, nook_path):
        col, _ = self._seed_source_file(nook_path, "/proj/lone.md", n_chunks=1)
        out = _expand_with_neighbors(
            col,
            "chunk_0 content",
    ...
    def test_expand_falls_back_when_metadata_missing(self, nook_path):
        col = get_collection(nook_path)
    ...
    def test_hybrid_search_enrichment_populates_drawer_index_and_total(self, nook_path):
