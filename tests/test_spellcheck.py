"""Tests for sage.spellcheck — spell-correction utilities."""

from unittest.mock import patch

from sage_mcp.spellcheck import (
    _edit_distance,
    _get_system_words,
    _should_skip,
...
class TestShouldSkip:
    ...
    def test_camelcase_skipped(self):
        ...
        assert _should_skip("sage-mcp", set()) is True
    ...
    def test_known_name_skipped(self):
        assert _should_skip("sage", {"sage"}) is True
...
def test_spellcheck_user_text_passthrough_no_autocorrect():
    ...
    with patch("sage_mcp.spellcheck._get_speller", return_value=None):
...
def test_spellcheck_user_text_with_speller():
    ...
    with patch("sage_mcp.spellcheck._get_speller", return_value=fake_speller):
        with patch("sage_mcp.spellcheck._get_system_words", return_value=set()):
            with patch("sage_mcp.spellcheck._load_known_names", return_value=set()):
...
def test_transcript_line_user_turn():
    ...
    with patch("sage_mcp.spellcheck.spellcheck_user_text", return_value="corrected"):
...
def test_spellcheck_transcript_processes_content():
    ...
    with patch("sage_mcp.spellcheck.spellcheck_user_text", return_value="fixed"):
