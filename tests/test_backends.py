...
from sage_mcp.backends.chroma import (
    ChromaBackend,
    ChromaCollection,
...
def test_resolve_backend_priority_order(tmp_path):
    from sage_mcp.backends import resolve_backend_for_nook

    # explicit kwarg wins over everything
    assert resolve_backend_for_nook(explicit="pg", config_value="lance") == "pg"
    # config value wins over env / default
    assert resolve_backend_for_nook(config_value="lance", env_value="qdrant") == "lance"
    # env wins over default
    assert resolve_backend_for_nook(env_value="qdrant", default="chroma") == "qdrant"
    # falls back to default
    assert resolve_backend_for_nook() == "chroma"


def test_chroma_detect_matches_nook_with_chroma_sqlite(tmp_path):
...
def test_chroma_close_nook_releases_sqlite_lock_for_reopen(tmp_path):
    """close_nook must release chromadb's rust-side SQLite file lock so
    a fresh PersistentClient on the same path after shutil.rmtree can
    write without hitting SQLITE_READONLY_DBMOVED."""
    backend = ChromaBackend()
    nook_path = tmp_path / "nook-a"
    ref = NookRef(id=str(nook_path), local_path=str(nook_path))

    col = backend.get_collection(nook=ref, collection_name="nook_drawers", create=True)
    col.upsert(documents=["hello"], ids=["a"], metadatas=[{"k": "v"}])

    backend.close_nook(ref)
...
def test_chroma_close_releases_all_cached_clients(tmp_path):
    ...
    of their nook paths can be reopened by a fresh backend in the same
    process."""
    backend = ChromaBackend()
    nook_a = tmp_path / "nook-a"
    nook_b = tmp_path / "nook-b"
    ref_a = NookRef(id=str(nook_a), local_path=str(nook_a))
    ref_b = NookRef(id=str(nook_b), local_path=str(nook_b))

    for ref in (ref_a, ref_b):
        backend.get_collection(nook=ref, collection_name="nook_drawers", create=True).upsert(
            documents=["x"], ids=["x"], metadatas=[{"k": "v"}]
        )

    backend.close()

    for path in (nook_a, nook_b):
        shutil.rmtree(path)
        ref = NookRef(id=str(path), local_path=str(path))
        fresh = ChromaBackend()
        col = fresh.get_collection(nook=ref, collection_name="nook_drawers", create=True)
...
def test_chroma_cache_invalidates_when_db_file_missing(tmp_path):
    """A nook rebuild that removes chroma.sqlite3 must drop the stale cache.
    ...
    nook_path.mkdir()
    db_file = nook_path / "chroma.sqlite3"
    db_file.write_bytes(b"")  # any file is enough for _db_stat to see it
    st = db_file.stat()

    sentinel = object()
    backend._clients[str(nook_path)] = sentinel
    backend._freshness[str(nook_path)] = (st.st_ino, st.st_mtime)
    ...
    new_client = backend._client(str(nook_path))
    ...
    assert backend._freshness[str(nook_path)] != prior_freshness
...
def test_chroma_cache_picks_up_db_created_after_first_open(tmp_path):
    ...
    # nook when chroma.sqlite3 did not exist yet. Freshness (0, 0.0) is the
    # signal that the DB was absent at cache time.
    sentinel = object()
    backend._clients[str(nook_path)] = sentinel
    backend._freshness[str(nook_path)] = (0, 0.0)
    ...
    _chromadb.PersistentClient(path=str(nook_path)).get_or_create_collection("seed")
    assert (nook_path / "chroma.sqlite3").is_file()

    # Next _client() call must detect the 0 → nonzero transition and rebuild.
    refreshed = backend._client(str(nook_path))
    assert refreshed is not sentinel
    assert backend._freshness[str(nook_path)] != (0, 0.0)
...
def test_base_collection_update_default_rejects_mismatched_lengths():
    ...
    from sage_mcp.backends.base import BaseCollection
...
def test_base_collection_update_merge_preserves_document(tmp_path):
    """update() with documents=None preserves the existing document (None-fallback branch).

    Covers the previously-uncovered merge path at base.py:304-310.
    Findings [42],[40] from pycore audit cluster.
    """
    from sage_mcp.backends.base import BaseCollection
    ...
    ref = NookRef(id=str(nook_path), local_path=str(nook_path))
    col = backend.get_collection(nook=ref, collection_name="nook_drawers", create=True)
    col.upsert(ids=["row1"], documents=["original doc"], metadatas=[{"k": "v1"}])

    # update with documents=None must preserve the existing document.
    BaseCollection.update(col, ids=["row1"], metadatas=[{"k": "v2", "extra": "new"}])

    result = col.get(ids=["row1"], include=["documents", "metadatas"])
    assert result["documents"][0] == "original doc", (
        "document must be preserved when documents=None"
    )
    assert result["metadatas"][0].get("k") == "v2", "metadata key k must be updated"
    assert result["metadatas"][0].get("extra") == "new", "new metadata key must be present"


def test_base_collection_update_merge_with_none_metadata_entry(tmp_path):
    """update() with metadatas[i]=None treats it as {} (no-op merge).

    Covers the 'metadatas[i] or {}' branch at base.py:309.
    """
    from sage_mcp.backends.base import BaseCollection

    nook_path = tmp_path / "nook2"
    backend = ChromaBackend()
    ref = NookRef(id=str(nook_path), local_path=str(nook_path))
    col = backend.get_collection(nook=ref, collection_name="nook_drawers", create=True)
    col.upsert(ids=["row1"], documents=["doc"], metadatas=[{"k": "original"}])

    # metadatas[i]=None should be treated as empty dict — no metadata change.
    BaseCollection.update(col, ids=["row1"], metadatas=[None])

    result = col.get(ids=["row1"], include=["documents", "metadatas"])
    assert result["documents"][0] == "doc"
    assert result["metadatas"][0].get("k") == "original", "metadata must be preserved on None entry"


def test_chroma_backend_accepts_nook_ref_kwarg(tmp_path):
    nook_path = tmp_path / "nook"
    backend = ChromaBackend()
    collection = backend.get_collection(
        nook=NookRef(id=str(nook_path), local_path=str(nook_path)),
...
def test_chroma_backend_create_false_raises_without_creating_directory(tmp_path):
    nook_path = tmp_path / "missing-nook"
...
def test_chroma_backend_create_true_creates_directory_and_collection(tmp_path):
    ...
    client = chromadb.PersistentClient(path=str(nook_path))
    client.get_collection("nook_drawers")
...
def test_chroma_backend_sets_hnsw_bloat_guard_on_creation(tmp_path):
    ...
    collection-creation time so every fresh nook gets it without needing
...
def test_chroma_backend_create_collection_sets_hnsw_bloat_guard(tmp_path):
    """Same guard must apply via the older create_collection() path."""
    nook_path = tmp_path / "nook"

    ChromaBackend().create_collection(str(nook_path), "nook_drawers")
...
def test_get_collection_create_true_is_idempotent(tmp_path):
    ...
    """
    nook = str(tmp_path / "nook")
    backend = ChromaBackend()
    backend.get_collection(nook, collection_name="nook_drawers", create=True)
    col2 = backend.get_collection(nook, collection_name="nook_drawers", create=True)
...
def test_get_collection_create_true_preserves_existing_metadata(tmp_path):
    ...
    backend = ChromaBackend()
    backend.get_collection(nook, collection_name="nook_drawers", create=True)
    col = backend.get_collection(nook, collection_name="nook_drawers", create=True)
...
def test_fix_blob_seq_ids_noop_without_database(tmp_path):
    """No error when nook has no chroma.sqlite3."""
...
def test_fix_blob_seq_ids_still_converts_blobs_in_embeddings(tmp_path):
...
def test_fix_blob_seq_ids_writes_marker_after_blob_path(tmp_path):
    ...
    from sage_mcp.backends.chroma import _BLOB_FIX_MARKER
...
def test_fix_blob_seq_ids_writes_marker_when_already_integer(tmp_path):
    ...
    not to record that a conversion happened. So a clean nook gets the
...
def test_fix_blob_seq_ids_skips_sqlite_when_marker_present(tmp_path):
    ...
    PersistentClient call (#1090). Once a nook has been migrated, we
    ...
    with patch("sage_mcp.backends.chroma.sqlite3.connect") as mock_connect:
        _fix_blob_seq_ids(str(tmp_path))
...
def _make_nook_with_segment(tmp_path, hnsw_mtime, sqlite_mtime, meta_bytes=_HEALTHY_META):
    """Helper: build a nook dir with one HNSW segment + sqlite at given
    ...
    os.utime(nook / "chroma.sqlite3", (sqlite_mtime, sqlite_mtime))
    return nook, seg
...
def test_quarantine_stale_hnsw_renames_corrupt_segment(tmp_path):
    ...
    now = 1_700_000_000.0
    nook, seg = _make_nook_with_segment(
        tmp_path,
        hnsw_mtime=now - 7200,
        sqlite_mtime=now,
        meta_bytes=_CORRUPT_META,
    )
    moved = quarantine_stale_hnsw(str(nook), stale_seconds=3600.0)
    assert len(moved) == 1
    assert ".drift-" in moved[0]
    assert not seg.exists()
    renamed = list(nook.iterdir())
...
def test_quarantine_stale_hnsw_leaves_healthy_segment_with_drift_alone(tmp_path):
    ...
    leaving 151K-drawer nook with vector_ranked=0."""
    now = 1_700_000_000.0
    nook, seg = _make_nook_with_segment(
        tmp_path,
        hnsw_mtime=now - 7200,
...
def test_quarantine_stale_hnsw_renames_missing_metadata_with_nontrivial_data(tmp_path):
    ...
    drift_dirs = [p for p in nook.iterdir() if ".drift-" in p.name]
    assert len(drift_dirs) == 1
    assert (drift_dirs[0] / "data_level0.bin").exists()
...
def test_quarantine_stale_hnsw_leaves_fresh_segment_alone(tmp_path):
    ...
    now = 1_700_000_000.0
    nook, seg = _make_nook_with_segment(tmp_path, hnsw_mtime=now - 10, sqlite_mtime=now)
    moved = quarantine_stale_hnsw(str(nook), stale_seconds=3600.0)
    assert moved == []
    assert seg.exists()


def test_quarantine_stale_hnsw_no_nook(tmp_path):
    """Missing nook path or chroma.sqlite3: return [] without raising."""
...
def test_quarantine_stale_hnsw_skips_already_quarantined(tmp_path):
    ...
    nook.mkdir()
    (nook / "chroma.sqlite3").write_text("")
    os.utime(nook / "chroma.sqlite3", (now, now))
    drift = nook / "abcd-1234.drift-20260101-000000"
    drift.mkdir()
...
def test_make_client_quarantines_only_on_first_call_per_nook(tmp_path, monkeypatch):
    """Quarantine fires on first ``make_client()`` for a nook, then is
    ...
    os.makedirs(nook_path, exist_ok=True)
    (Path(nook_path) / "chroma.sqlite3").write_text("")
    ...
    monkeypatch.setattr("sage_mcp.backends.chroma.quarantine_stale_hnsw", _spy)

    ChromaBackend.make_client(nook_path)
    ChromaBackend.make_client(nook_path)
    ChromaBackend.make_client(nook_path)

    assert calls == [nook_path], (
        "quarantine_stale_hnsw should fire once per nook per process, not on every reconnect"
    )
...
def test_make_client_gates_invalid_metadata_on_first_call(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("sage_mcp.backends.chroma.quarantine_invalid_hnsw_metadata", _invalid)
    monkeypatch.setattr("sage_mcp.backends.chroma.quarantine_stale_hnsw", _stale)
...
def test_make_client_quarantines_each_nook_independently(tmp_path, monkeypatch):
    """Two distinct nooks each get one quarantine attempt — the gate is
    keyed by nook path, not global."""
    from sage_mcp.backends.chroma import ChromaBackend

    nook_a = str(tmp_path / "nook_a")
    nook_b = str(tmp_path / "nook_b")
    for p in (nook_a, nook_b):
    ...
    ChromaBackend.make_client(nook_a)
    ChromaBackend.make_client(nook_b)
    ChromaBackend.make_client(nook_a)  # already gated
    ChromaBackend.make_client(nook_b)  # already gated

    assert calls == [nook_a, nook_b]
...
def test_client_quarantines_only_on_first_call_per_nook(tmp_path, monkeypatch):
    """Repeated ``_client()`` calls for the same nook re-run quarantine
    ...
    assert calls == [nook_path], (
        "quarantine_stale_hnsw should fire once per nook per process from _client(), not on every call"
    )
...
def test_pin_hnsw_threads_retrofits_existing_collection(tmp_path):
    """Older collections (created without num_threads) get the retrofit applied."""
    nook_path = tmp_path / "existing-nook"
    nook_path.mkdir()

    client = chromadb.PersistentClient(path=str(nook_path))
    col = client.create_collection(
        "nook_drawers",
        metadata={"hnsw:space": "cosine"},  # no num_threads — older config
    )
...
def test_get_collection_applies_retrofit_on_existing_nook(tmp_path):
    """ChromaBackend.get_collection(create=False) applies the retrofit."""
    nook_path = tmp_path / "nook"
    nook_path.mkdir()

    # Simulate an earlier-format nook: create collection without num_threads
    bootstrap_client = chromadb.PersistentClient(path=str(nook_path))
    bootstrap_client.create_collection("nook_drawers", metadata={"hnsw:space": "cosine"})
...
def test_get_collection_raises_nook_not_found_when_dir_missing(tmp_path):
    """create=False on a missing dir raises NookNotFoundError, not the
    ...
    with pytest.raises(NookNotFoundError) as excinfo:
        ChromaBackend().get_collection(
            str(missing),
            collection_name="nook_drawers",
            create=False,
...
def test_get_collection_raises_collection_not_initialized_on_empty_nook(tmp_path):
    """When the nook dir + DB exist but the collection has never been
    ...
    chromadb.PersistentClient(path=str(nook_path))
    assert (nook_path / "chroma.sqlite3").is_file()
    ...
    # Backward-compat: subclass of NookNotFoundError (and FileNotFoundError).
    assert isinstance(excinfo.value, NookNotFoundError)
...
def test_quarantine_invalid_hnsw_metadata_renames_missing_dimensionality(tmp_path):
    ...
    moved = quarantine_invalid_hnsw_metadata(str(nook))
...
def test_quarantine_invalid_hnsw_metadata_skips_transient_read_errors(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "sage_mcp.backends.chroma._SafePersistentDataUnpickler.load",
...
def test_chroma_backend_preflights_metadata_before_persistent_client(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("sage_mcp.backends.chroma._fix_blob_seq_ids", _record("blob"))
    monkeypatch.setattr(
        "sage_mcp.backends.chroma.quarantine_invalid_hnsw_metadata", _record("invalid")
    )
    monkeypatch.setattr("sage_mcp.backends.chroma.quarantine_stale_hnsw", _record("stale"))

    class DummyClient:
        pass

    monkeypatch.setattr(
        "sage_mcp.backends.chroma.chromadb.PersistentClient", lambda path: DummyClient()
    )
...
def test_explain_ef_mismatch_recognizes_chromadb_conflict():
    ...
    (user changed SAGE_EMBEDDING_MODEL on an existing nook), the
    ...
    msg = ChromaBackend._explain_ef_mismatch(err, "/tmp/nook.db")
    assert msg is not None
    assert "/tmp/nook.db" in msg
    assert "SAGE_EMBEDDING_MODEL" in msg
    # The remediation must name the REAL command (`sage repair`), not the
    # non-existent `sage repair rebuild-index` the old message printed (E2E F1).
    assert "sage repair" in msg
    assert "rebuild-index" not in msg
...
def test_explain_ef_mismatch_returns_none_for_unrelated_errors():
    ...
    assert ChromaBackend._explain_ef_mismatch(err, "/tmp/nook.db") is None


def test_get_collection_translates_ef_mismatch_to_helpful_error(tmp_path):
    """End-to-end: create a nook with the default EF, then try to read it
    ...
    coll = backend.get_collection(nook_path, "drawers", create=True)
    coll.add(documents=["seed"], ids=["1"])

    # Now swap in an incompatible EF name (simulates the user setting
    # SAGE_EMBEDDING_MODEL=embeddinggemma without rebuild-index).
    ...
    backend.close_nook(nook_path)

    try:
        with pytest.raises(ValueError, match=r"sage repair"):
            backend.get_collection(nook_path, "drawers", create=False)
    finally:
        backend._resolve_embedding_function = original_resolver
        backend.close_nook(nook_path)


def test_nook_get_collection_uses_configured_collection_name(monkeypatch):
    from sage_mcp import nook

    captured = {}

    def fake_get_collection(nook_path, collection_name=None, create=False):
        captured["nook_path"] = nook_path
        captured["collection_name"] = collection_name
        captured["create"] = create
        return object()

    monkeypatch.setattr(nook._DEFAULT_BACKEND, "get_collection", fake_get_collection)
    monkeypatch.setattr("sage_mcp.config.get_configured_collection_name", lambda: "custom_drawers")

    nook.get_collection("/nook", create=False)
...
# ── HealthStatus + ChromaBackend.health()/reset() (ADR-0074) ────────────────


def test_health_status_matrix():
    from sage_mcp.backends.base import HealthStatus

    assert HealthStatus.healthy().ok is True
    assert HealthStatus.healthy().vector_disabled is False
    assert HealthStatus.unhealthy("closed").ok is False
    deg = HealthStatus.degraded("diverged", capacity={"divergence": 9})
    assert deg.ok is True and deg.vector_disabled is True
    assert deg.detail == "diverged" and deg.capacity == {"divergence": 9}


def test_chroma_health_closed_is_unhealthy():
    from sage_mcp.backends.chroma import ChromaBackend

    b = ChromaBackend()
    b.close()
    status = b.health("/any/nook")
    assert status.ok is False
    assert "closed" in status.detail


def test_chroma_health_no_nook_is_healthy():
    from sage_mcp.backends.chroma import ChromaBackend

    status = ChromaBackend().health()
    assert status.ok is True and status.vector_disabled is False


def test_chroma_health_diverged_is_degraded(monkeypatch):
    import sage_mcp.backends.chroma as chroma

    monkeypatch.setattr(
        chroma,
        "hnsw_capacity_status",
        lambda nook, coll: {"diverged": True, "message": "bloat", "divergence": 5},
    )
    status = chroma.ChromaBackend().health("/n", "nook_drawers")
    assert status.ok is True
    assert status.vector_disabled is True
    assert status.detail == "bloat"
    assert status.capacity == {"diverged": True, "message": "bloat", "divergence": 5}


def test_chroma_health_healthy_carries_capacity(monkeypatch):
    import sage_mcp.backends.chroma as chroma

    monkeypatch.setattr(
        chroma, "hnsw_capacity_status", lambda nook, coll: {"diverged": False, "sqlite_count": 10}
    )
    status = chroma.ChromaBackend().health("/n", "nook_drawers")
    assert status.vector_disabled is False
    assert status.capacity == {"diverged": False, "sqlite_count": 10}


def test_chroma_health_probe_exception_fails_open(monkeypatch):
    """A crashing probe must not block search — fail open to the vector path."""
    import sage_mcp.backends.chroma as chroma

    def boom(*a, **k):
        raise RuntimeError("sqlite locked")

    monkeypatch.setattr(chroma, "hnsw_capacity_status", boom)
    status = chroma.ChromaBackend().health("/n", "nook_drawers")
    assert status.ok is True and status.vector_disabled is False


def test_chroma_health_resolves_none_collection(monkeypatch):
    """collection_name=None resolves to the configured name, not literal None."""
    import sage_mcp.backends.chroma as chroma
    import sage_mcp.config as config

    monkeypatch.setattr(config, "get_configured_collection_name", lambda: "cfg_coll")
    seen = {}
    monkeypatch.setattr(
        chroma,
        "hnsw_capacity_status",
        lambda nook, coll: seen.update(coll=coll) or {"diverged": False},
    )
    chroma.ChromaBackend().health("/n", None)
    assert seen["coll"] == "cfg_coll"


def test_chroma_reset_closes_pops_and_rearms_quarantine():
    from sage_mcp.backends.chroma import ChromaBackend

    b = ChromaBackend()
    path = "/tmp/sage-reset-test-nook"
    b._clients[path] = object()  # bare handle; _close_client swallows missing .close()
    b._freshness[path] = (1, 2.0)
    ChromaBackend._quarantined_paths.add(path)

    b.reset(path)

    assert path not in b._clients
    assert path not in b._freshness
    assert path not in ChromaBackend._quarantined_paths  # re-armed for next open


def test_harden_nook_perms_restricts_db_and_dir(tmp_path):
    """ADR-0077: chroma.sqlite3 (verbatim drawer content) is chmod'd 0600, dir 0700.

    Tests the hardening helper directly (no chromadb client needed)."""
    import os
    ...
    nook.mkdir()
    os.chmod(nook, 0o755)
    db = nook / "chroma.sqlite3"
    db.touch()
    db.chmod(0o644)  # simulate chromadb's umask-default creation

    ChromaBackend._harden_nook_perms(str(nook))

    assert (db.stat().st_mode & 0o777) == 0o600, "chroma.sqlite3 not restricted to 0600"
    assert (nook.stat().st_mode & 0o777) == 0o700, "nook dir not restricted to 0700"


def test_harden_nook_perms_noop_when_db_absent(tmp_path):
    """No DB yet → dir still hardened, no crash."""
    from sage_mcp.backends.chroma import ChromaBackend

    nook = tmp_path / "nook2"
    nook.mkdir()
    ChromaBackend._harden_nook_perms(str(nook))  # must not raise
    assert (nook.stat().st_mode & 0o777) == 0o700


def test_harden_nook_perms_covers_bins_and_sidecars(tmp_path):
    """ADR-0077 / E2E round-C: harden HNSW .bin + sqlite sidecars, not just chroma.sqlite3."""
    from sage_mcp.backends.chroma import ChromaBackend

    nook = tmp_path / "nook"
    seg = nook / "seg-abc"
    seg.mkdir(parents=True)
    files = [nook / "chroma.sqlite3", nook / "chroma.sqlite3-wal", seg / "data_level0.bin"]
    for f in files:
        f.touch()
        f.chmod(0o644)
    ChromaBackend._harden_nook_perms(str(nook))
    for f in files:
        assert (f.stat().st_mode & 0o777) == 0o600, f"{f.name} not hardened to 0600"
