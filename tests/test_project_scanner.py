"""Tests for sage.project_scanner."""
...
from sage_mcp.project_scanner import (
    PersonInfo,
    ProjectInfo,
    _dedupe_people,
    _is_bot,
...
def test_is_bot_does_not_flag_github_privacy_email():
    ...
    assert not _is_bot("Bob Examplename", "123456+example@users.noreply.github.com")
...
def test_looks_like_real_name_accepts_human():
    assert _looks_like_real_name("Bob Examplename")
...
def test_looks_like_real_name_rejects_handles():
    ...
    assert not _looks_like_real_name("username")
...
def test_unionfind_merges_shared_email():
    commits = [
        ("Alice J", "shared@example.com", "repo1"),
        ("MSL", "shared@example.com", "repo1"),
        ("Alice J", "other@example.com", "repo1"),
    ]
    people = _dedupe_people(commits)
    # All three commits collapse into one "Alice J" person (MSL is filtered
    # as display name because it lacks a space but its commits still count).
    assert "Alice J" in people
    assert people["Alice J"].total_commits == 3
...
def test_project_scanner_scan_empty_dir(tmp_path):
    projects, people = scan(tmp_path)
    assert projects == []
    assert people == []
...
def test_merge_primary_wins_case_insensitive():
    ...
    # `Sage` (uncertain) is deduped against `sage` (project) case-insensitively
