...
have no signal that their nook is broken.
...
def test_create_collection_sets_cosine(tmp_path):
    backend = ChromaBackend()
    col = backend.create_collection(str(tmp_path), "nook_drawers")
    _assert_cosine(col, "create_collection")


def test_new_get_collection_with_create_sets_cosine(tmp_path):
    """The typed surface — ``get_collection(..., create=True)`` is the
    path the miner + init flow take. Must also set cosine."""
    backend = ChromaBackend()
    col = backend.get_collection(str(tmp_path), "nook_drawers", create=True)
    _assert_cosine(col, "get_collection(create=True)")


def test_nook_module_get_collection_sets_cosine(tmp_path):
    """The public ``sage.nook.get_collection`` is what most callers
    use. Must produce cosine nooks."""
    col = get_collection(str(tmp_path), "nook_drawers", create=True)
    _assert_cosine(col, "nook.get_collection(create=True)")


def test_reopening_cosine_nook_preserves_metric(tmp_path):
    """Opening a previously-created cosine nook (create=False) must
    still expose the cosine metadata — catches any regression where
    reopening drops or overwrites metadata."""
    backend = ChromaBackend()
    backend.create_collection(str(tmp_path), "nook_drawers")
    # Fresh backend simulates a process restart
    backend2 = ChromaBackend()
    col = backend2.get_collection(str(tmp_path), "nook_drawers", create=False)
    _assert_cosine(col, "re-opened nook")


def test_fresh_nook_via_full_stack_gets_cosine(tmp_path):
    """End-to-end: build a nook with the public API the way a new user
    ...
    """
    col = get_collection(str(tmp_path), "nook_drawers", create=True)
    _assert_cosine(col, "full-stack new nook")

    # And the closets collection too
    closets = get_collection(str(tmp_path), "nook_closets", create=True)
