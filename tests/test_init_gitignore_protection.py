"""Regression tests for issue #185 — gitignore protection on `sage init`.

Issue #185 reports that `sage init <dir>` writes `sage.yaml` and
`entities.json` into the project root, where they could be committed by
accident. The fix adds `_ensure_nook_files_gitignored()` which appends
the two filenames to `.gitignore` when `<dir>` is a git repository.
"""

from pathlib import Path

from sage_mcp.cli import _ensure_nook_files_gitignored
...
def test_no_op_when_not_a_git_repo(tmp_path):
    assert _ensure_nook_files_gitignored(tmp_path) is False
    assert not (tmp_path / ".gitignore").exists()


def test_creates_gitignore_with_both_entries(tmp_path):
    _git_init(tmp_path)
    assert _ensure_nook_files_gitignored(tmp_path) is True
    contents = (tmp_path / ".gitignore").read_text()
    assert "sage.yaml" in contents
...
def test_appends_only_missing_entries(tmp_path):
    ...
    assert _ensure_nook_files_gitignored(tmp_path) is True
    contents = (tmp_path / ".gitignore").read_text()
    # sage.yaml must not be duplicated
    assert contents.count("sage.yaml") == 1
