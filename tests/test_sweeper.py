...
For each session in the transcript directory:
  ...
  4. Idempotent: re-running sweeps should find nothing new on a complete nook.

This test file is TDD — written BEFORE sage/sweeper.py exists.
"""
...
class TestSweeperParsing:
    def test_parse_yields_only_user_and_assistant(self, mock_claude_jsonl):
        from sage_mcp.sweeper import parse_claude_jsonl
...
class TestSweeperTandem:
    ...
    def test_sweep_empty_nook_ingests_all_messages(self, mock_claude_jsonl, tmp_path):
        from sage_mcp.sweeper import sweep

        nook_path = str(tmp_path / "nook")
        result = sweep(str(mock_claude_jsonl), nook_path)
        assert result["drawers_added"] == 4, (
            f"Empty nook: all 4 user/assistant messages should ingest. "
    ...
    def test_sweep_is_idempotent(self, mock_claude_jsonl, tmp_path):
        ...
        nook_path = str(tmp_path / "nook")
        first = sweep(str(mock_claude_jsonl), nook_path)
        second = sweep(str(mock_claude_jsonl), nook_path)
    ...
    def test_sweep_resumes_from_cursor(self, tmp_path):
        """If half the messages are already in the nook, sweep picks up
        only the later half."""
        from sage_mcp.sweeper import sweep
        ...
        nook_path = str(tmp_path / "nook")
        first = sweep(str(jsonl_path), nook_path)
        ...
        second = sweep(str(jsonl_path), nook_path)
    ...
    def test_sweep_recovers_untaken_message_at_cursor_timestamp(self, tmp_path):
        ...
        """
        from sage_mcp.nook import get_collection
        from sage_mcp.sweeper import (
        ...
        col = get_collection(nook_path, create=True)
        ...
        result = sweep(str(jsonl_path), nook_path)
...
class TestSweeperDrawerMetadata:
    ...
    def test_drawer_has_session_id_and_timestamp_metadata(self, mock_claude_jsonl, tmp_path):
        ...
        nook_path = str(tmp_path / "nook")
        sweep(str(mock_claude_jsonl), nook_path)

        col = get_collection(nook_path, create=False)
...
class TestSweepDirectory:
    """sweep_directory aggregates results across all .jsonl files in a tree."""

    def _make_jsonl(self, path, session_id, uuid_prefix, count=2):
        """Write ``count`` user messages to a .jsonl file."""
        lines = [
            {
                "type": "user",
    ...
    def test_sweep_directory_aggregates_across_files(self, tmp_path):
        """sweep_directory finds all .jsonl files and sums drawers_added."""
        from sage_mcp.sweeper import sweep_directory

        jsonl_dir = tmp_path / "transcripts"
        jsonl_dir.mkdir()
        self._make_jsonl(jsonl_dir / "session1.jsonl", "s1", "u1", count=2)
        self._make_jsonl(jsonl_dir / "session2.jsonl", "s2", "u2", count=3)

        nook_path = str(tmp_path / "nook")
        result = sweep_directory(str(jsonl_dir), nook_path)

        assert result["files_attempted"] == 2
        assert result["files_succeeded"] == 2
        assert result["drawers_added"] == 5, (
            f"Expected 2+3=5 drawers across both files, got {result['drawers_added']}"
        )
        assert result["failures"] == []

    def test_sweep_directory_recursive(self, tmp_path):
        """sweep_directory recurses into subdirectories."""
        from sage_mcp.sweeper import sweep_directory

        subdir = tmp_path / "transcripts" / "sub"
        subdir.mkdir(parents=True)
        self._make_jsonl(subdir / "deep.jsonl", "sd", "ud", count=1)

        nook_path = str(tmp_path / "nook")
        result = sweep_directory(str(tmp_path / "transcripts"), nook_path)

        assert result["files_attempted"] == 1
        assert result["drawers_added"] == 1

    def test_sweep_directory_empty_dir_returns_zero_counts(self, tmp_path):
        """sweep_directory on a directory with no .jsonl files returns zero counts."""
        from sage_mcp.sweeper import sweep_directory

        empty_dir = tmp_path / "empty"
        empty_dir.mkdir()
        nook_path = str(tmp_path / "nook")
        result = sweep_directory(str(empty_dir), nook_path)

        assert result["files_attempted"] == 0
        assert result["drawers_added"] == 0
        assert result["failures"] == []

    def test_sweep_directory_records_failure_on_unreadable_file(self, tmp_path):
        """sweep_directory records a failure entry when a file cannot be opened."""
        from sage_mcp.sweeper import sweep_directory

        jsonl_dir = tmp_path / "transcripts"
        jsonl_dir.mkdir()
        self._make_jsonl(jsonl_dir / "good.jsonl", "sg", "ug", count=1)
        bad_file = jsonl_dir / "unreadable.jsonl"
        bad_file.write_text("{}\n")
        bad_file.chmod(0o000)  # remove all permissions — open() will raise PermissionError

        nook_path = str(tmp_path / "nook")
        try:
            result = sweep_directory(str(jsonl_dir), nook_path)
        finally:
            bad_file.chmod(0o644)  # restore so tmp_path cleanup can delete it

        assert result["files_attempted"] == 2
        assert result["files_succeeded"] == 1
        assert len(result["failures"]) == 1
        assert str(bad_file) == result["failures"][0]["file"]
