...
class TestCandidateUnion:
    def test_default_vector_strategy_unchanged(self, tmp_path):
        ...
        _seed_drawers(nook)
        without = search_memories(_NARRATIVE_QUERY, nook, n_results=5)
        with_default = search_memories(
            _NARRATIVE_QUERY, nook, n_results=5, candidate_strategy="vector"
        )
    ...
    def test_union_surfaces_bm25_strong_vector_distant_doc(self, tmp_path):
        ...
        nook = str(tmp_path / "nook")
        _seed_drawers(nook)
        result = search_memories(_NARRATIVE_QUERY, nook, n_results=5, candidate_strategy="union")
    ...
    def test_union_preserves_vector_hits(self, tmp_path):
        ...
        nook = str(tmp_path / "nook")
        _seed_drawers(nook)
        vector = search_memories(_NARRATIVE_QUERY, nook, n_results=5, candidate_strategy="vector")
        union = search_memories(_NARRATIVE_QUERY, nook, n_results=5, candidate_strategy="union")
    ...
    def test_union_handles_empty_nook(self, tmp_path):
        """No drawers — union mode should return empty results, not crash."""
        nook = str(tmp_path / "nook")
        get_collection(nook, create=True)  # create empty collection
        result = search_memories("anything", nook, n_results=5, candidate_strategy="union")
    ...
    def test_invalid_candidate_strategy_raises(self, tmp_path):
        ...
        with pytest.raises(ValueError, match="candidate_strategy"):
            search_memories("anything", nook, n_results=5, candidate_strategy="bogus")
    ...
    def test_union_respects_n_results_limit(self, tmp_path):
        ...
        result = search_memories(_NARRATIVE_QUERY, nook, n_results=2, candidate_strategy="union")
    ...
    def test_union_skipped_when_max_distance_set(self, tmp_path):
        ...
        unfiltered = search_memories(
            _NARRATIVE_QUERY, nook, n_results=5, candidate_strategy="union"
        )
    ...
    def test_union_dedup_is_chunk_precise_not_basename(self, tmp_path):
        ...
        """
        nook = str(tmp_path / "nook")
        col = get_collection(nook, create=True)
        col.upsert(
            ...
            documents=[
                ...
                "Engineers occasionally chat about how the older "
