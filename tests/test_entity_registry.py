"""Tests for sage.entity_registry."""

from unittest.mock import patch

import pytest

from sage_mcp.entity_registry import (
...
def test_save_and_load_roundtrip(tmp_path):
    ...
    assert "sage" in loaded.projects
...
def test_save_preserves_previous_on_serialization_failure(tmp_path, monkeypatch):
    ...
    # nook directory and a future diagnostic cannot distinguish stale
...
def test_lookup_known_project(tmp_path):
    registry = EntityRegistry.load(config_dir=tmp_path)
    registry.seed(mode="work", people=[], projects=["sage"])
    result = registry.lookup("sage")
...
def test_research_local_only_by_default(tmp_path):
    ...
    with patch(
        "sage_mcp.entity_registry._wikipedia_lookup",
...
def test_wikipedia_404_returns_unknown(tmp_path):
    ...
    with patch("sage_mcp.entity_registry._wikipedia_lookup", return_value=mock_result):
