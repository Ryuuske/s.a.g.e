...
sage mine must not print "Done." and exit 0 on a nook whose
chroma.sqlite3 left FTS5 in a malformed state. The validation hook in
``nook._validate_nook_fts5_after_mine`` runs PRAGMA quick_check at
...
from sage_mcp import cli, convo_miner, miner
...
def _build_nook_with_drawer(nook_path: Path) -> None:
    """Create a real chromadb nook with one drawer so chroma.sqlite3 exists."""
    nook_path.mkdir(parents=True, exist_ok=True)
    from sage_mcp.backends.chroma import ChromaBackend

    backend = ChromaBackend()
    try:
        col = backend.create_collection(str(nook_path), "nook_drawers")
        col.upsert(
            ids=["d1"],
...
def _mine_args(
    nook: str, src: str, *, mode: str = "project", dry_run: bool = False
) -> SimpleNamespace:
...
# ── 1. Helper returns silently on a clean nook ────────────────────


def test_helper_returns_silently_on_clean_nook(tmp_path):
    nook = tmp_path / "nook"
    _build_nook_with_drawer(nook)

    # Should not raise: quick_check returns ('ok',) so no errors collected.
    assert _validate_nook_fts5_after_mine(str(nook)) is None
...
def test_helper_raises_on_page_mangled_sqlite(tmp_path):
    nook = tmp_path / "nook"
    _build_nook_with_drawer(nook)
    _page_mangle(nook / "chroma.sqlite3")

    with pytest.raises(MineValidationError) as exc_info:
        _validate_nook_fts5_after_mine(str(nook))

    err = exc_info.value
    assert err.nook_path == str(nook)
...
def test_helper_raises_on_fts5_segment_corruption(tmp_path):
    ...
    nook = tmp_path / "nook"
    _build_nook_with_drawer(nook)
    _corrupt_fts5_segment(nook / "chroma.sqlite3")
...
def test_cmd_mine_project_mode_exits_nonzero_with_banner(tmp_path, monkeypatch, capsys):
    ...
    def _raise(*_, **__):
        raise MineValidationError(
            nook, ["malformed inverted index for FTS5 table main.embedding_fulltext_search"]
        )

    monkeypatch.setattr(miner, "mine", _raise)

    with pytest.raises(SystemExit) as exit_info:
        cli.cmd_mine(_mine_args(nook, str(src), mode="project"))
    ...
    assert "sage repair --yes" in combined
...
def test_cmd_mine_convos_mode_exits_nonzero_with_banner(tmp_path, monkeypatch, capsys):
    ...
    def _raise(*_, **__):
        raise MineValidationError(nook, ["malformed inverted index for FTS5 table"])

    monkeypatch.setattr(convo_miner, "mine_convos", _raise)

    with pytest.raises(SystemExit) as exit_info:
        cli.cmd_mine(_mine_args(nook, str(src), mode="convos"))
...
def test_validate_skipped_on_dry_run(tmp_path, monkeypatch):
    ...
    def _spy(nook_path):
        calls.append(nook_path)

    monkeypatch.setattr(miner, "_validate_nook_fts5_after_mine", _spy)
...
def test_full_chain_raises_through_mine_impl(tmp_path, monkeypatch):
    ...
    from sage_mcp import nook as nook_mod
    ...
    called = []
    real_validator = nook_mod._validate_nook_fts5_after_mine

    def _validator_spy(path):
        called.append(path)
        return real_validator(path)

    monkeypatch.setattr(miner_mod, "_validate_nook_fts5_after_mine", _validator_spy)
    ...
    assert called == [str(nook)], f"validator must be the raise-source; spy recorded: {called}"
    assert "fts5" in " ".join(exc_info.value.errors).lower()
    assert exc_info.value.nook_path == str(nook)


def test_mine_impl_does_not_print_partial_summary_on_validation_error(tmp_path, capsys):
    """When _validate_nook_fts5_after_mine raises, miner._mine_impl must
...
def test_mine_validation_error_rejects_empty_errors():
    ...
    with pytest.raises(ValueError, match="non-empty nook_path"):
...
def test_helper_silent_on_missing_nook(tmp_path):
    """Helper returns silently when chroma.sqlite3 doesn't exist (nook
    ...
    """
    missing = tmp_path / "no_nook_here"
    assert _validate_nook_fts5_after_mine(str(missing)) is None
...
def test_convo_miner_dry_run_skips_validator(tmp_path, monkeypatch):
    ...
    """
    from sage_mcp import convo_miner as convo_mod
    ...
    monkeypatch.setattr(convo_mod, "_validate_nook_fts5_after_mine", _spy)
...
def test_close_handles_called_before_quick_check(tmp_path, monkeypatch):
    ...
    from sage_mcp import repair as repair_mod

    real_close = repair_mod._close_chroma_handles
    real_errors = repair_mod.sqlite_integrity_errors

    def _close_spy(nook_path, *args, **kwargs):
        order.append("close")
        return real_close(nook_path, *args, **kwargs)

    def _errors_spy(nook_path, *args, **kwargs):
        order.append("quick_check")
        return real_errors(nook_path, *args, **kwargs)
...
def test_cmd_mine_extract_mode_exits_nonzero_with_banner(tmp_path, monkeypatch, capsys):
    ...
    The third mine entry point landed via #1555 (prior release)
    after #1548 was written, so without this wire-up the extract path
    would exit 0 on a corrupted FTS5 nook.
    """
    from sage_mcp import format_miner as format_mod
    ...
    with pytest.raises(SystemExit) as exit_info:
        cli.cmd_mine(_mine_args(nook, str(src), mode="extract"))
...
def test_mine_formats_dry_run_skips_validator(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(format_mod, "_validate_nook_fts5_after_mine", _spy)
...
def test_mine_formats_full_chain_raises_when_fts5_corrupt(tmp_path, monkeypatch):
    """End-to-end: a nook with corrupted FTS5 + real mine_formats call
    must propagate MineValidationError from `_validate_nook_fts5_after_mine`.
    ...
    monkeypatch.setattr(format_mod, "_validate_nook_fts5_after_mine", _validator_spy)
