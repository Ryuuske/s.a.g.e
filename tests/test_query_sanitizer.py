...
from sage_mcp.query_sanitizer import (
    MAX_QUERY_LENGTH,
    MIN_QUERY_LENGTH,
    SAFE_QUERY_LENGTH,
    sanitize_query,
...
class TestQuestionExtraction:
    ...
    def test_question_in_system_prompt_ignored_when_real_question_exists(self):
        ...
        real_query = "What databases does sage support?"
        query = system_prompt + real_query
        result = sanitize_query(query)
        assert result["was_sanitized"] is True
        assert "sage" in result["clean_query"] or "database" in result["clean_query"].lower()
...
class TestTailSentence:
    ...
    def test_keyword_style_query(self):
        ...
        query = system_prompt + "\nsage ChromaDB integration setup"
        result = sanitize_query(query)
        assert result["was_sanitized"] is True
        assert "sage" in result["clean_query"] or "ChromaDB" in result["clean_query"]
...
class TestStripWrappingQuotes:
    """Cover _strip_wrapping_quotes branches not reached by other tests.

    Findings [25]/[22] from pycore audit cluster.
    The sanitizer is reached via the tail_sentence path on long inputs; we
    construct inputs long enough to trigger sanitization but short enough in
    the final fragment that the quote-stripping branches are the interesting part.
    """

    def _long_prefix(self, n: int = 30) -> str:
        return "Prefix sentence. " * n

    def test_leading_only_quote_is_stripped(self):
        """A fragment with only a leading quote loses it (line 84 branch)."""
        # Construct a long input whose tail_sentence fragment starts with ".
        prefix = self._long_prefix()
        # Tail fragment: '"hello world' (leading " only)
        query = prefix + '\n"hello world'
        result = sanitize_query(query)
        assert not result["clean_query"].startswith('"'), (
            f"leading quote should be stripped, got: {result['clean_query']!r}"
        )
        assert "hello world" in result["clean_query"]

    def test_trailing_only_quote_is_stripped(self):
        """A fragment with only a trailing quote loses it (line 86 branch)."""
        prefix = self._long_prefix()
        query = prefix + '\nhello world"'
        result = sanitize_query(query)
        assert not result["clean_query"].endswith('"'), (
            f"trailing quote should be stripped, got: {result['clean_query']!r}"
        )
        assert "hello world" in result["clean_query"]

    def test_quotes_only_fragment_returns_empty(self):
        """A fragment that is only quote characters collapses to '' (line 82 branch)."""
        # We need a tail that consists only of quote chars after sanitization;
        # feed a long prefix + a fragment that is just '""'.
        prefix = self._long_prefix()
        query = prefix + '\n""'
        result = sanitize_query(query)
        # The fragment '""' strips to '' — the sanitizer may then fall
        # through to a different candidate or tail_truncation. Either way,
        # the returned query must not be '""'.
        assert result["clean_query"] != '""'
...
class TestRealWorldScenarios:
    ...
    def test_sage_wakeup_prepended(self):
        """Simulates sage wake-up output prepended to a query."""
        wakeup = (
            "sage loaded. Wings: technical, emotions, identity. "
            "Rooms: chromadb-setup, error-handling, project-planning. "
            "Total drawers: 234. Knowledge graph: 89 entities, 156 triples. "
            "Nook ready. Protocol: verify before responding. "
        ) * 5  # ~1000 chars
    ...
    def test_memory_md_prepended(self):
        ...
        memory_md = (
            ...
            "- Verbatim storage with semantic search retrieval\n"
        ) * 10  # ~750 chars
    ...
    def test_2000_char_system_prompt_with_question(self):
        ...
        real_query = "What is the status of the sage project?"
        query = system_prompt + real_query
        result = sanitize_query(query)
        assert result["was_sanitized"] is True
