"""Tests for explicit tunnel helpers in sage.nook_graph."""
...
with patch.dict("sys.modules", {"chromadb": MagicMock()}):
    import sage_mcp.nook_graph as nook_graph


def _use_tmp_tunnel_file(monkeypatch, tmp_path):
    """Redirect the tunnel-file resolver at the tmp_path.

    Also neutralizes ``_get_collection`` so the endpoint-existence
    validation falls through to the "can't verify, allow" branch by
    default. Tests that exercise the validation path supply their own
    stub via a subsequent monkeypatch.setattr.
    """
...
class TestTunnelStorage:
    def test_load_tunnels_missing_file_returns_empty_list(self, tmp_path, monkeypatch):
        ...
        assert nook_graph._load_tunnels() == []
    ...
    def test_save_and_load_round_trip(self, tmp_path, monkeypatch):
        ...
        nook_graph._save_tunnels(tunnels)
        assert nook_graph._load_tunnels() == tunnels
...
class TestExplicitTunnels:
    def test_normalize_wing_uses_shared_rule_and_trims_empty(self):
        assert nook_graph._normalize_wing(" Nook-Public ") == "nook_public"
        assert nook_graph._normalize_wing("   ") is None
        assert nook_graph._normalize_wing(None) is None
        ...
        assert nook_graph._normalize_wing(42) is None
        assert nook_graph._normalize_wing(["x"]) is None
    ...
    def test_create_tunnel_rejects_empty_names(self, tmp_path, monkeypatch):
        ...
        with pytest.raises(ValueError):
            nook_graph.create_tunnel("", "auth", "wing_people", "users")

    def test_list_tunnels_filters_by_either_side(self, tmp_path, monkeypatch):
        _use_tmp_tunnel_file(monkeypatch, tmp_path)

        nook_graph.create_tunnel("wing_code", "auth", "wing_people", "users", label="A")
        nook_graph.create_tunnel("wing_ops", "deploy", "wing_people", "users", label="B")

        assert len(nook_graph.list_tunnels()) == 2
        assert len(nook_graph.list_tunnels("wing_people")) == 2
        assert len(nook_graph.list_tunnels("wing_code")) == 1

    def test_delete_tunnel_removes_saved_tunnel(self, tmp_path, monkeypatch):
        _use_tmp_tunnel_file(monkeypatch, tmp_path)

        tunnel = nook_graph.create_tunnel(
            "wing_code", "auth", "wing_people", "users", label="same concept"
        )

        assert nook_graph.delete_tunnel(tunnel["id"]) == {"deleted": tunnel["id"]}
        assert nook_graph.list_tunnels() == []

    def test_follow_tunnels_returns_direction_and_preview(self, tmp_path, monkeypatch):
        _use_tmp_tunnel_file(monkeypatch, tmp_path)

        nook_graph.create_tunnel(
            "wing_code",
            "auth",
            "wing_people",
            "users",
        ...
        outgoing = nook_graph.follow_tunnels("wing_code", "auth", col=col)
        ...
        incoming = nook_graph.follow_tunnels("wing_people", "users", col=col)
    ...
    def test_follow_tunnels_returns_connections_even_if_collection_lookup_fails(
        ...
        connections = nook_graph.follow_tunnels("wing_code", "auth", col=col)
...
class TestTopicTunnels:
    ...
    def test_compute_topic_tunnels_creates_link_for_shared_topic(self, tmp_path, monkeypatch):
        ...
        created = nook_graph.compute_topic_tunnels(topics_by_wing, min_count=1)
        ...
        listed = nook_graph.list_tunnels()
        assert len(listed) == 1
    ...
    def test_compute_topic_tunnels_no_link_below_threshold(self, tmp_path, monkeypatch):
        ...
        created = nook_graph.compute_topic_tunnels(topics_by_wing, min_count=2)
        assert created == []
        assert nook_graph.list_tunnels() == []
    ...
    def test_compute_topic_tunnels_empty_input_is_noop(self, tmp_path, monkeypatch):
        _use_tmp_tunnel_file(monkeypatch, tmp_path)
        assert nook_graph.compute_topic_tunnels({}) == []
        assert nook_graph.compute_topic_tunnels({"wing_a": []}) == []
        assert nook_graph.list_tunnels() == []
    ...
    def test_topic_tunnels_for_wing_only_links_that_wing(self, tmp_path, monkeypatch):
        ...
        created = nook_graph.topic_tunnels_for_wing("wing_a", topics_by_wing)
        endpoint_pairs = {
            tuple(sorted([t["source"]["wing"], t["target"]["wing"]])) for t in created
        }
    ...
    def test_topic_tunnels_for_wing_unknown_wing_is_noop(self, tmp_path, monkeypatch):
        ...
        assert nook_graph.topic_tunnels_for_wing("wing_missing", topics_by_wing) == []
        assert nook_graph.list_tunnels() == []
    ...
    def test_topic_tunnels_for_wing_matches_across_slug_forms(self, tmp_path, monkeypatch):
        ...
        created = nook_graph.topic_tunnels_for_wing("my-wing", topics_by_wing)
        assert len(created) == 1
    ...
    def test_compute_topic_tunnels_dedupe_on_recompute(self, tmp_path, monkeypatch):
        ...
        first = nook_graph.compute_topic_tunnels(topics_by_wing, min_count=1)
        second = nook_graph.compute_topic_tunnels(topics_by_wing, min_count=1)
    ...
    def test_topic_tunnels_carry_kind_field(self, tmp_path, monkeypatch):
        _use_tmp_tunnel_file(monkeypatch, tmp_path)
        nook_graph.create_tunnel("wing_a", "auth", "wing_b", "users", label="x")
        nook_graph.compute_topic_tunnels({"wing_a": ["Redis"], "wing_b": ["Redis"]}, min_count=1)

        tunnels = nook_graph.list_tunnels()
...
class TestHyphenatedWingNormalization:
    ...
    * ``sage mine`` without ``--wing`` derives the slug from the dir
      name through ``normalize_wing_name`` → stored as ``nook_public``.
    * ``sage mine --wing my-wing`` (or any explicit slug) is stored
    ...
    def test_list_tunnels_filters_hyphenated_wing(self, tmp_path, monkeypatch):
        ...
        nook_graph.create_tunnel("nook_public", "auth", "wing_people", "users")

        assert len(nook_graph.list_tunnels("nook-public")) == 1
        assert len(nook_graph.list_tunnels("nook_public")) == 1
    ...
    def test_follow_tunnels_matches_hyphenated_wing(self, tmp_path, monkeypatch):
        ...
        by_hyphen = nook_graph.follow_tunnels("nook-public", "auth")
        by_under = nook_graph.follow_tunnels("nook_public", "auth")
    ...
    def test_create_tunnel_preserves_hyphenated_wing_names(self, tmp_path, monkeypatch):
        """Regression for #1504: wings created via ``sage mine --wing my-wing``
        ...
        t = nook_graph.create_tunnel("my-project", "src", "your-project", "dst", label="cross")
        assert t["source"]["wing"] == "my-project"
        assert t["target"]["wing"] == "your-project"
        assert len(nook_graph.list_tunnels("my-project")) == 1
        assert len(nook_graph.list_tunnels("my_project")) == 1
    ...
    def test_find_tunnels_warns_on_empty_result(self, tmp_path, monkeypatch, caplog):
        ...
        with caplog.at_level("WARNING", logger="nook_graph"):
            result = nook_graph.find_tunnels("nonexistent-wing")
        assert result == []
    ...
    def test_read_path_skips_records_with_null_endpoints(self, tmp_path, monkeypatch):
        ...
        assert {t["id"] for t in nook_graph.list_tunnels("wing_a")} == {"ok"}
        connections = nook_graph.follow_tunnels("wing_a", "r1")
    ...
    def test_pre_1504_underscore_tunnels_remain_findable(self, tmp_path, monkeypatch):
        ...
        nook_graph._save_tunnels(
            [
                {
                    "id": "pre_1504_record",
                    "source": {"wing": "nook_public", "room": "auth"},
                    "target": {"wing": "wing_people", "room": "users"},
        ...
        assert len(nook_graph.follow_tunnels("nook_public", "auth")) == 1
        assert len(nook_graph.follow_tunnels("nook-public", "auth")) == 1


# =============================================================================
# Regression: tunnel file follows nook_path config (#1467)
# =============================================================================
class TestTunnelFileFollowsConfig:
    """Bug A: prior to this fix the tunnel file was hardcoded at
    ``~/.sage/tunnels.json`` regardless of SageConfig.nook_path.
    ...
    to every other process touching the configured nook.
    """

    def test_default_tunnel_file_unchanged(self):
        """Regression: with default config, tunnel_file resolves to
        ``~/.sage/tunnels.json`` so existing single-user installs are
        not silently relocated."""
        from sage_mcp.config import DEFAULT_NOOK_PATH, SageConfig

        cfg = SageConfig()
        # Default nook_path is ~/.sage/nook, so tunnel is sibling.
        expected = os.path.join(os.path.dirname(DEFAULT_NOOK_PATH), "tunnels.json")
        assert cfg.tunnel_file == expected
        assert nook_graph._get_tunnel_file(cfg) == expected

    def test_tunnel_file_follows_nook_path(self, tmp_path):
        """Custom nook_path → tunnel sits beside the nook, not at the
        hardcoded location."""
        from sage_mcp.config import SageConfig

        custom_dir = tmp_path / "custom-nook"
        cfg = SageConfig(config_dir=tmp_path)
        cfg._file_config["nook_path"] = str(custom_dir)
        assert cfg.tunnel_file == str(tmp_path / "tunnels.json")
        assert nook_graph._get_tunnel_file(cfg) == str(tmp_path / "tunnels.json")
...
class TestCreateTunnelEndpointValidation:
    """Bug B: previously ``create_tunnel`` only validated wing/room names
    ...
    def test_create_tunnel_rejects_nonexistent_target_room(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(nook_graph, "_get_collection", lambda config=None: col)

        with pytest.raises(ValueError) as exc_info:
            nook_graph.create_tunnel("wing_code", "auth", "wing_people", "phantom")
    ...
    def test_create_tunnel_rejects_nonexistent_source_room(self, tmp_path, monkeypatch):
        ...
        with pytest.raises(ValueError) as exc_info:
            nook_graph.create_tunnel("wing_code", "phantom", "wing_people", "users")
        msg = str(exc_info.value)
        assert "phantom" in msg
    ...
    def test_create_tunnel_succeeds_when_both_rooms_exist(self, tmp_path, monkeypatch):
        ...
        t = nook_graph.create_tunnel("wing_code", "auth", "wing_people", "users", label="verified")
    ...
    def test_create_tunnel_skips_validation_when_collection_unreachable(
        ...
        """When chroma is unreachable (nook not yet created, transient
        failure, tests without a real backend), validation is skipped
        rather than fail-closed — matches the tolerance pattern used
        throughout nook_graph._get_collection()."""
        _use_tmp_tunnel_file(monkeypatch, tmp_path)
        monkeypatch.setattr(nook_graph, "_get_collection", lambda config=None: None)

        t = nook_graph.create_tunnel("wing_code", "any", "wing_people", "any", label="cold-start")
    ...
    def test_create_tunnel_tolerates_collection_query_exception(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(nook_graph, "_get_collection", lambda config=None: _AngryCollection())

        # Should not raise — fall back to "allow" rather than fail-closed.
        t = nook_graph.create_tunnel("wing_code", "x", "wing_people", "y", label="best-effort")
    ...
    def test_compute_topic_tunnels_skips_endpoint_validation(self, tmp_path, monkeypatch):
        ...
        nook_graph.compute_topic_tunnels(topics_by_wing, min_count=1)
        tunnels = nook_graph.list_tunnels()
...
class TestEntityTunnels:
    ...
    parallel for one release cycle so existing nooks don't lose
    ...
    def test_entity_tunnels_creates_cross_wing_tunnel_for_shared_entity(
        ...
        """Ben appears in a hallway in wing_alice AND in wing_demo →
        exactly one entity tunnel between those two wings, anchored on Ben."""
        _use_tmp_tunnel_file(monkeypatch, tmp_path)
        hallways = [
            {"wing": "wing_alice", "entity_a": "Ben", "entity_b": "Alice"},
            {"wing": "wing_demo", "entity_a": "Ben", "entity_b": "Bob"},
        ]
        created = nook_graph.entity_tunnels_for_wing("wing_alice", hallways)
        assert len(created) == 1
        tunnel = created[0]
    ...
    def test_entity_tunnels_skips_entities_in_only_one_wing(self, tmp_path, monkeypatch):
        """Alice has hallways only in wing_alice → no tunnel for her."""
        _use_tmp_tunnel_file(monkeypatch, tmp_path)
        hallways = [
            {"wing": "wing_alice", "entity_a": "Alice", "entity_b": "Bob"},
            {"wing": "wing_alice", "entity_a": "Alice", "entity_b": "Ever"},
        ]
        created = nook_graph.entity_tunnels_for_wing("wing_alice", hallways)
        assert created == []
    ...
    def test_entity_tunnels_counts_entity_in_either_pair_position(self, tmp_path, monkeypatch):
        ...
        _use_tmp_tunnel_file(monkeypatch, tmp_path)
        hallways = [
            # Ben is entity_b in wing_alice
            {"wing": "wing_alice", "entity_a": "Alice", "entity_b": "Ben"},
            # Ben is entity_a in wing_demo
            {"wing": "wing_demo", "entity_a": "Ben", "entity_b": "Bob"},
        ]
        created = nook_graph.entity_tunnels_for_wing("wing_alice", hallways)
        assert len(created) == 1
    ...
    def test_entity_tunnels_three_wings_pairwise_from_focus_wing(self, tmp_path, monkeypatch):
        """When Ben has hallways in {wing_alice, wing_mp, wing_lt} and we
        compute for wing_alice, we create wing_alice↔wing_mp AND wing_alice↔wing_lt
        ...
        hallways = [
            {"wing": "wing_alice", "entity_a": "Ben", "entity_b": "Alice"},
            {"wing": "wing_demo", "entity_a": "Ben", "entity_b": "Bob"},
            {"wing": "wing_lantern", "entity_a": "Ben", "entity_b": "Bob"},
        ]
        created = nook_graph.entity_tunnels_for_wing("wing_alice", hallways)
        ...
        for t in created:
            other = (
                t["target"]["wing"] if t["source"]["wing"] == "wing_alice" else t["source"]["wing"]
            )
            target_wings.add(other)
        assert target_wings == {"wing_demo", "wing_lantern"}
    ...
    def test_entity_tunnels_idempotent_on_rerun(self, tmp_path, monkeypatch):
        ...
        nook_graph.entity_tunnels_for_wing("wing_alice", hallways)
        nook_graph.entity_tunnels_for_wing("wing_alice", hallways)
        listed = nook_graph.list_tunnels()
    ...
    def test_entity_tunnels_empty_hallways_is_noop(self, tmp_path, monkeypatch):
        ...
        _use_tmp_tunnel_file(monkeypatch, tmp_path)
        assert nook_graph.entity_tunnels_for_wing("wing_alice", []) == []
    ...
    def test_entity_tunnels_unknown_wing_is_noop(self, tmp_path, monkeypatch):
        ...
        _use_tmp_tunnel_file(monkeypatch, tmp_path)
        hallways = [
            {"wing": "wing_other", "entity_a": "Ben", "entity_b": "Alice"},
        ]
        assert nook_graph.entity_tunnels_for_wing("wing_nonexistent", hallways) == []
    ...
    def test_entity_tunnel_room_does_not_collide_with_literal_room(self, tmp_path, monkeypatch):
        ...
        nook_graph.create_tunnel(
            "wing_alice", "Ben", "wing_demo", "Ben", label="literal", kind="explicit"
        )
        hallways = [
            {"wing": "wing_alice", "entity_a": "Ben", "entity_b": "Alice"},
            {"wing": "wing_demo", "entity_a": "Ben", "entity_b": "Bob"},
...
class TestTunnelDynamicsIntegration:
    ...
    def test_recreate_tunnel_preserves_accumulated_dynamics(self, tmp_path, monkeypatch):
        ...
        _use_tmp_tunnel_file(monkeypatch, tmp_path)
        first = nook_graph.create_tunnel("wing_a", "room_x", "wing_b", "room_y", label="initial")

        # Simulate user activity bumping the tunnel's dynamics.
        stored = nook_graph._load_tunnels()
        for t in stored:
            if t["id"] == first["id"]:
                t["strength"] = 2.7
                t["access_count"] = 12
                t["stability"] = 1.5
        nook_graph._save_tunnels(stored)
    ...
    def test_recreate_tunnel_initializes_dynamics_for_minimal_records(self, tmp_path, monkeypatch):
        ...
        _use_tmp_tunnel_file(monkeypatch, tmp_path)
        earlier_tunnel = {
            "id": nook_graph._canonical_tunnel_id("wing_a", "room_x", "wing_b", "room_y"),
        ...
        nook_graph._save_tunnels([earlier_tunnel])

        # Recreate — should add the missing dynamics fields.
        recreated = nook_graph.create_tunnel(
