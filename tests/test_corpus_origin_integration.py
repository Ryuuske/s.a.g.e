...
infrastructure that nobody can prove the corpus-origin feature improves.
...
agents. On plain entity_detector (no corpus_origin context), misclassifies these as PEOPLE.
...
Two tests sit side by side:
  ...
  test_baseline_v333_misclassifies_persona_names_as_people
    Pins the prior behavior. If this starts failing, the PR's motivation
...
Together: documented before/after of the corpus-origin feature.
"""
...
# names appear repeatedly in dialogue patterns that the older entity_detector
...
#
# The point is: every name here CAN be a real human name. The pre-feature implementation had no
...
# ── Baseline test: pin the prior behavior ────────────────────────────────────


def test_baseline_v333_misclassifies_persona_names_as_people(ai_dialogue_corpus: Path):
    """Without corpus_origin context, entity_detector cannot
    ...
    # No agent_personas bucket existed before this feature.
    assert "agent_personas" not in detected, (
        "The pre-feature implementation had no concept of agent_personas — if this key exists, "
...
def test_discover_entities_threads_corpus_origin_through(
    ...
    """
    from sage_mcp.project_scanner import discover_entities
...
def test_discover_entities_no_origin_unchanged_shape(ai_dialogue_corpus: Path):
    ...
    shape stays exactly as it was pre-feature (no agent_personas key).
    ...
    assert "agent_personas" not in detected, (
        ...
        "was not provided — that would be a silent behavior change for callers "
...
def _stub_cfg(nook_dir: Path):
    """Build a SageConfig stub whose nook_path points at tmp space.

    Used by Pass 0 tests so the origin.json write is captured in tmp_path
    instead of hitting the real ~/.sage location.
    """
    cfg = MagicMock()
    cfg.nook_path = str(nook_dir)
    cfg.entity_languages = ["en"]
    return cfg


def test_init_pass_zero_writes_origin_json_to_nook(ai_dialogue_corpus: Path, tmp_path: Path):
    """cmd_init must run corpus_origin detection BEFORE entity detection
    and persist the result to ``<nook>/.sage/origin.json`` in the
    documented schema_version=1 wrapper.
    """
    from sage_mcp.cli import cmd_init
    ...
    # classification that overrides the correct heuristic answer (the prior PR
...
def test_init_pass_zero_passes_corpus_origin_to_discover_entities(
    ...
    # on test_init_pass_zero_writes_origin_json_to_nook.
    args = argparse.Namespace(dir=str(ai_dialogue_corpus), yes=True, no_llm=True)
    ...
    with (
        patch("sage_mcp.cli.SageConfig", return_value=_stub_cfg(nook)),
        patch("sage_mcp.project_scanner.discover_entities", side_effect=fake_discover),
        patch("sage_mcp.cli._maybe_run_mine_after_init"),
        patch("sage_mcp.room_detector_local.detect_rooms_local"),
    ):
        cmd_init(args)
...
def test_init_pass_zero_skipped_when_no_readable_files(tmp_path: Path):
    ...
    without crashing. Alice's earlier finding: don't fail init on missing samples.
    """
    from sage_mcp.cli import cmd_init
...
def test_init_pass_zero_uses_full_file_content_not_front_sampled(tmp_path: Path):
    """Per Alice's pushback: Tier 1 must read full file content, not bias-sample
    ...
    # heuristic answer. See the prior PR review for the full failure mode
    # mode and its fix.
    args = argparse.Namespace(dir=str(project), yes=True, no_llm=True)
    ...
    assert data["result"]["likely_ai_dialogue"] is True, (
        ...
        "full content per Alice's design pushback."
    )
...
def test_llm_refine_includes_corpus_origin_context_in_prompt(
    ...
    from sage_mcp.llm_refine import refine_entities
...
def test_llm_refine_no_origin_keeps_v333_prompt_shape(monkeypatch):
    ...
    from sage_mcp.llm_refine import SYSTEM_PROMPT, refine_entities
    ...
    assert captured["system"] == SYSTEM_PROMPT, (
        ...
        "SYSTEM_PROMPT — no silent prompt drift for callers."
    )


# ── sage mine --redetect-origin flag ───────────────────────────────
...
def test_mine_default_does_not_redetect_origin(ai_dialogue_corpus: Path, tmp_path: Path):
    """Default `sage mine` (no --redetect-origin flag) must NOT run
    corpus_origin detection — the flag is opt-in.
    """
    from sage_mcp.cli import cmd_mine
    ...
    with (
        patch("sage_mcp.cli.SageConfig", return_value=_stub_cfg(nook)),
        patch("sage_mcp.cli._run_pass_zero") as mock_pass_zero,
        patch("sage_mcp.miner.mine"),
    ):
        cmd_mine(args)

    mock_pass_zero.assert_not_called()
    assert not (nook / ".sage" / "origin.json").exists()


def test_mine_with_redetect_origin_flag_writes_origin_json(
    ai_dialogue_corpus: Path, tmp_path: Path
):
    """`sage mine --redetect-origin` re-runs corpus_origin detection
    on the project and persists the result to <nook>/.sage/origin.json.
    """
    from sage_mcp.cli import cmd_mine
    ...
    origin_path = nook / ".sage" / "origin.json"
    assert origin_path.exists(), "--redetect-origin must write <nook>/.sage/origin.json"
    data = json.loads(origin_path.read_text())
...
def test_mine_redetect_uses_full_content_not_sampled(tmp_path: Path):
    """Regression for Alice's pushback: --redetect-origin must use the same
    ...
    data = json.loads((nook / ".sage" / "origin.json").read_text())
    assert data["result"]["likely_ai_dialogue"] is True, (
...
def test_init_default_attempts_llm_provider(ai_dialogue_corpus: Path, tmp_path: Path):
    """``sage init`` (no flags) MUST try to acquire an LLM
    ...
    with (
        patch("sage_mcp.cli.SageConfig", return_value=_stub_cfg(nook)),
        patch("sage_mcp.cli.get_provider", return_value=fake_provider) as mock_get,
        patch("sage_mcp.cli._maybe_run_mine_after_init"),
        patch("sage_mcp.room_detector_local.detect_rooms_local"),
    ):
        cmd_init(args)
    ...
    (
        ...
        (
            "Default `sage init` did not attempt LLM provider acquisition. "
...
def test_init_no_llm_skips_provider_acquisition(ai_dialogue_corpus: Path, tmp_path: Path):
    """``sage init --no-llm`` is the explicit opt-out path. No
    ...
    with (
        patch("sage_mcp.cli.SageConfig", return_value=_stub_cfg(nook)),
        patch("sage_mcp.cli.get_provider") as mock_get,
        patch("sage_mcp.cli._maybe_run_mine_after_init"),
        patch("sage_mcp.room_detector_local.detect_rooms_local"),
    ):
        cmd_init(args)
...
def test_init_graceful_fallback_when_provider_unavailable(
    ...
    with (
        patch("sage_mcp.cli.SageConfig", return_value=_stub_cfg(nook)),
        patch("sage_mcp.cli.get_provider", return_value=fake_provider),
        patch("sage_mcp.cli._maybe_run_mine_after_init"),
        patch("sage_mcp.room_detector_local.detect_rooms_local"),
    ):
...
def test_init_graceful_fallback_on_provider_construction_error(
    ...
    """
    from sage_mcp.cli import cmd_init
    from sage_mcp.llm_client import LLMError
    ...
    with (
        patch("sage_mcp.cli.SageConfig", return_value=_stub_cfg(nook)),
        patch("sage_mcp.cli.get_provider", side_effect=LLMError("no api key")),
        patch("sage_mcp.cli._maybe_run_mine_after_init"),
        patch("sage_mcp.room_detector_local.detect_rooms_local"),
    ):
        cmd_init(args)  # MUST NOT raise
...
def test_init_llm_flag_compatible(ai_dialogue_corpus: Path, tmp_path: Path):
    """Backwards compatibility: `sage init --llm` still works as
...
def test_end_to_end_init_with_llm_separates_personas(ai_dialogue_corpus: Path, tmp_path: Path):
    """End-to-end through `sage init` on the DEFAULT path (LLM enabled).
    ...
    """
    from sage_mcp.cli import cmd_init
    from sage_mcp.corpus_origin import CorpusOriginResult
    ...
    with (
        ...
        patch("sage_mcp.cli.get_provider", return_value=fake_provider),
        patch(
            "sage_mcp.cli.detect_origin_llm",
    ...
    origin_data = json.loads((nook / ".sage" / "origin.json").read_text())
...
def test_no_llm_path_matches_v333_classification(ai_dialogue_corpus: Path, tmp_path: Path):
    ...
    The test PINS that the pre-feature behavior on this path:
    persona names appear in entities.json's people list, exactly as they
    would in the pre-feature implementation. Users who want persona reclassification must
    ...
    origin = json.loads((nook / ".sage" / "origin.json").read_text())
    ...
    # entities.json shows the pre-feature classification: persona names
...
def test_re_init_idempotent(ai_dialogue_corpus: Path, tmp_path: Path):
    """Running `sage init` twice on the same project produces the
    ...
    with (
        ...
        cmd_init(args)
        first = json.loads((nook / ".sage" / "origin.json").read_text())
        cmd_init(args)
        second = json.loads((nook / ".sage" / "origin.json").read_text())
...
def test_persona_user_name_collision_user_kept_in_people(
    ...
    """
    from sage_mcp.entity_detector import detect_entities
    ...
    from sage_mcp.entity_detector import scan_for_detection
...
# ─────────────────────────────────────────────────────────────────────────
# corpus-origin integration tests
#
# These tests pin the intersection points between corpus-origin (this PR) and
# the prior branch's other in-flight work that landed earlier. They exist
# specifically to prove the cherry-pick onto main produced a coherent
...
def test_integration_cmd_init_runs_pass_zero_to_pass_four_in_order(
    ...
    real_run_pass_zero = __import__("sage_mcp.cli", fromlist=["_run_pass_zero"])._run_pass_zero
    ...
    with (
        patch("sage_mcp.cli.SageConfig", return_value=_stub_cfg(nook)),
        patch("sage_mcp.cli._run_pass_zero", side_effect=trace_pass_zero),
        patch("sage_mcp.project_scanner.discover_entities", side_effect=trace_discover),
        patch("sage_mcp.room_detector_local.detect_rooms_local", side_effect=trace_rooms),
        patch("sage_mcp.cli._ensure_nook_files_gitignored", side_effect=trace_gitignore),
        patch("sage_mcp.cli._maybe_run_mine_after_init", side_effect=trace_mine_prompt),
    ):
        cmd_init(args)
    ...
    assert call_log == expected, (
        f"cmd_init pass ordering broke after corpus-origin merge.\n"
        f"  expected: {expected}\n"
        f"  actual:   {call_log}\n"
...
def test_integration_topics_and_agent_personas_coexist(
    ...
    """the prior branch adds a 'topics' bucket (PR #1184 cross-wing tunnels);
    ...
    Catches the most-likely merge regression: dropping the prior branch's topics
    ...
    # the prior branch's topics bucket must still exist (even if empty for this fixture)
    assert "topics" in detected, (
        "corpus-origin reclassification dropped the prior branch's 'topics' bucket. "
...
def test_integration_entities_json_includes_topics_excludes_personas(
    ...
    tools read) must:
      - INCLUDE the topics list (the prior branch's contribution)
      - NOT include persona names in the people list (corpus-origin's contribution)

    This is the contract downstream tools (miner, nook_graph cross-wing
    tunnels) depend on.
    """
    from sage_mcp.cli import cmd_init
    from sage_mcp.corpus_origin import CorpusOriginResult
    ...
    with (
        patch("sage_mcp.cli.SageConfig", return_value=_stub_cfg(nook)),
        patch("sage_mcp.cli.get_provider", return_value=fake_provider),
        patch("sage_mcp.cli.detect_origin_llm", return_value=fake_origin),
        patch("sage_mcp.cli._maybe_run_mine_after_init"),
        patch("sage_mcp.room_detector_local.detect_rooms_local"),
    ):
        cmd_init(args)
    ...
    # the prior branch's contract: topics key is present (even if empty list)
    assert "topics" in entities, (
        "entities.json missing 'topics' key — the prior PR #1184 "
    ...
    assert not leaked, (
        f"corpus-origin broken on the prior branch: persona names {leaked} leaked into "
...
def test_integration_add_to_known_entities_called_with_wing(
    ...
    """The prior branch changed add_to_known_entities to take a ``wing=`` kwarg
    ...
    with (
        ...
        patch("sage_mcp.cli._maybe_run_mine_after_init"),
        patch("sage_mcp.room_detector_local.detect_rooms_local"),
        patch("sage_mcp.miner.add_to_known_entities") as mock_add,
    ):
        cmd_init(args)

    if mock_add.called:
        # Inspect the call kwargs — wing= must be present per the prior signature.
        _, kwargs = mock_add.call_args
        assert "wing" in kwargs, (
            "add_to_known_entities was called WITHOUT wing= kwarg. "
            "the prior PR #1184 added this parameter; the corpus-origin call site "
...
def test_integration_llm_refine_corpus_origin_preamble_does_not_break_topic_label(
    ...
    """The prior branch added TOPIC as a valid llm_refine label (PR #1184).
    ...
    from sage_mcp.llm_refine import VALID_LABELS, refine_entities

    # TOPIC is preserved as a valid label
    assert "TOPIC" in VALID_LABELS, "the TOPIC label was dropped during corpus-origin merge"
    ...
    # Both signals must be in the prompt: the prior branch's TOPIC instructions AND
...
# Reviewer section markers (§2,,,,) for shorthand.
...
# instead.
#
# This test asserts nothing in `sage/` or `tests/` contains those
# markers. If a future commit re-introduces "Phase 1" or "the review §"
# anywhere, this test goes RED and blocks the merge.
#
# Pre-existing exception: the `sage/sources/` and `sage/backends/`
# packages cite the source-adapter spec sections (e.g. "§5.5") as legitimate spec
# references. Those are allowed.
# ─────────────────────────────────────────────────────────────────────────


def test_no_internal_coordination_jargon_in_source_or_tests():
    """Catches Phase 1 / the review / §N leaks before push.
    ...
    leak_re = re.compile(r"(Phase ?[12]|reviewer notes|prior spec)", re.IGNORECASE)
...
#
# Per the PR #1211 review: a small local model (e.g. Ollama
...
def test_merge_tier_fields_heuristic_yes_llm_no_keeps_heuristic_bool():
    ...
    heuristic's likely_ai_dialogue=True. The PR #1211 review caught
    ...
    from sage_mcp.cli import _run_pass_zero
    from sage_mcp.corpus_origin import CorpusOriginResult
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        nook_dir = Path(tmp_dir) / "nook"

        with patch("sage_mcp.cli.detect_origin_llm", return_value=llm_wrong_result):
            wrapped = _run_pass_zero(
                project_dir=str(project_dir),
                nook_dir=str(nook_dir),
    ...
    from sage_mcp.corpus_origin import detect_origin_heuristic
...
def test_merge_tier_fields_heuristic_no_no_personas_leak():
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        with patch("sage_mcp.cli.detect_origin_llm", return_value=llm_agreeing_result):
            wrapped = _run_pass_zero(
                project_dir=str(project_dir),
                nook_dir=str(nook_dir),
                llm_provider=fake_provider,
...
def test_merge_tier_fields_confidence_matches_heuristic_call():
    ...
    from sage_mcp.cli import _run_pass_zero
    from sage_mcp.corpus_origin import CorpusOriginResult, detect_origin_heuristic
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        with patch("sage_mcp.cli.detect_origin_llm", return_value=llm_distinct_result):
            wrapped = _run_pass_zero(
                project_dir=str(project_dir),
                nook_dir=str(nook_dir),
                llm_provider=fake_provider,
...
def test_merge_tier_fields_no_llm_provider_returns_heuristic_only():
    ...
    merge logic fires. This pins the older entity_detector contract.
    """
    from sage_mcp.cli import _run_pass_zero
...
#
# When sage init resolves an LLM provider whose endpoint will send
# user content off the local machine/network, init MUST print a clear
# warning naming the provider, stating that sage doesn't control
...
def test_init_prints_privacy_warning_when_provider_is_external(
    ...
    # The warning should also tell users sage isn't responsible
    ...
    assert (
        ...
        f"Privacy warning must clarify sage doesn't control how the "
