...
from sage_mcp.llm_client import LLMError, LLMResponse
from sage_mcp.llm_refine import (
...
def test_parse_response_restores_canonical_casing():
    ...
    text = '{"classifications": [{"name": "sage", "label": "PROJECT"}]}'
    out = _parse_response(text, ["Sage"])
    assert "Sage" in out
    assert out["Sage"][0] == "PROJECT"
...
def test_apply_classifications_keeps_unvisited_entries():
    ...
    # No decision for Bob — should stay untouched
    new, reclass, dropped = _apply_classifications(detected, {})
    assert new["people"][0]["name"] == "Bob"
    assert reclass == 0
    assert dropped == 0
...
def test_refine_entities_end_to_end_with_fake_provider():
    ...
    assert "Bob" in names_in_people  # untouched
