...
def _seed_strong_closet_for(nook_path, drawer_id, source_file, topics):
...
class TestHybridInvariant:
    def test_no_closets_degrades_to_direct_drawer_search(self, tmp_path):
        ...
        _seed_drawers(nook)
        # No closets created.
        result = search_memories("Kafka rebalance timeout", nook, n_results=3)
    ...
    def test_weak_closets_do_not_hide_direct_drawer_hits(self, tmp_path):
        ...
        result = search_memories("Kafka consumer rebalance timeout", nook, n_results=5)
        ids = [h["source_file"] for h in result["results"]]
    ...
    def test_closet_boost_lifts_matching_drawer(self, tmp_path):
        ...
        result = search_memories("JWT auth tokens expiry", nook, n_results=3)
        ids = [h["source_file"] for h in result["results"]]
...
class TestClosetMetadata:
    def test_closet_preview_exposed_when_boosted(self, tmp_path):
        ...
        result = search_memories("JWT authentication", nook, n_results=2)
        top = result["results"][0]
    ...
    def test_drawer_only_hits_have_no_closet_preview(self, tmp_path):
        ...
        _seed_drawers(nook)
        # No closets
        result = search_memories("TanStack Query", nook, n_results=2)
