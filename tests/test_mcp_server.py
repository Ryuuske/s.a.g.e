...
dispatch layer (integration-level). Uses isolated nook + KG fixtures
...
_MCP_LEAK_PREFIX = "/__nook_mcp_leak_sentinel__"


def test_mcp_main_strips_leaked_pythonpath_from_env():
    """sage_mcp.mcp_server:main must drop PYTHONPATH from the process env
    ...
    code = (
        "import os, sys\n"
        "from sage_mcp.mcp_server import main\n"
...
def _get_collection(nook_path, create=False):
    """Helper to get collection from test nook.
    ...
    if create:
        return (
            client,
            client.get_or_create_collection("nook_drawers", metadata={"hnsw:space": "cosine"}),
        )
    return client, client.get_collection("nook_drawers")
...
class TestColdStartDiagnostics:
    """``SAGE_LOG_FILE`` + ``SAGE_EAGER_WARMUP`` (#1495).
    ...
    * The whole point of the new env vars is process-startup behaviour
    ...
    def _run_main(env_overrides: dict, extra_code: str = "", timeout: int = 30):
        ...
        for k, v in env_overrides.items():
            if v is None:
                ...
                env[k] = v
        code = extra_code + "from sage_mcp.mcp_server import main\nmain()\n"
    ...
    def test_log_file_unset_attaches_only_stream_handler(self, tmp_path):
        ...
        env_overrides = {"SAGE_LOG_FILE": None}
        extra = (
            "import logging, pathlib\n"
            "from sage_mcp import mcp_server  # noqa: F401 — triggers _init_logging()\n"
    ...
    def test_log_file_empty_string_attaches_only_stream_handler(self, tmp_path):
        marker = tmp_path / "handlers.txt"
        env_overrides = {"SAGE_LOG_FILE": "   "}  # whitespace counts as unset after .strip()
        extra = (
            "import logging, pathlib\n"
            "from sage_mcp import mcp_server  # noqa: F401\n"
            f"pathlib.Path({str(marker)!r}).write_text("
            "','.join(type(h).__name__ for h in logging.getLogger().handlers)"
            ")\n"
            "raise SystemExit(0)\n"
    ...
    def test_log_file_set_attaches_file_handler_and_persists_startup_line(self, tmp_path):
        ...
        result = self._run_main({"SAGE_LOG_FILE": str(log_path)})
        assert result.returncode == 0, f"stderr={result.stderr!r}"
        assert log_path.exists(), f"log file missing; stderr={result.stderr!r}"
        body = log_path.read_text(encoding="utf-8")
        assert "sage MCP Server starting" in body, body
    ...
    def test_log_file_invalid_path_falls_back_to_stderr_with_warning(self, tmp_path):
        ...
        result = self._run_main({"SAGE_LOG_FILE": str(bad_path)})
        assert result.returncode == 0, f"stderr={result.stderr!r}"
    ...
    def test_eager_warmup_off_by_default_does_not_open_collection(self, tmp_path):
        ...
        nook = self._make_fake_nook(tmp_path)
        result = self._run_main(
            {"SAGE_EAGER_WARMUP": None, "SAGE_NOOK_PATH": nook},
    ...
    def test_eager_warmup_explicit_falsy_skips_collection_open_without_warning(
        ...
        nook = self._make_fake_nook(tmp_path)
        result = self._run_main(
            {"SAGE_EAGER_WARMUP": value, "SAGE_NOOK_PATH": nook},
            extra_code=self._spy_get_collection_extra(marker),
        )
        assert result.returncode == 0, f"stderr={result.stderr!r}"
    ...
    def test_eager_warmup_truthy_opens_collection_and_invokes_query(self, tmp_path, value):
        ...
        assert "__nook_warmup_probe__" in kwargs_repr, kwargs_repr
        assert "n_results" in kwargs_repr and "1" in kwargs_repr, kwargs_repr
        # Success path logs embedder + HNSW readiness + nook + device for ops.
        assert "embedder + HNSW ready" in result.stderr, result.stderr
        assert f"nook={nook}" in result.stderr, result.stderr

    def test_eager_warmup_fresh_install_skips_without_creating_nook(self, tmp_path):
        """Real integration test (no monkeypatch): an empty nook dir with no
        ``chroma.sqlite3`` must trigger the pre-check skip path BEFORE any
        chromadb call materializes the nook scaffold on disk.
        ...
        # nook scaffold on disk before the user runs ``sage init``.
        assert not db_path.exists(), (
            f"warmup materialized chroma.sqlite3 in a fresh nook dir "
    ...
    def test_eager_warmup_collection_returning_none_surfaces_warning(self, tmp_path):
        ...
        result = self._run_main(
            {"SAGE_EAGER_WARMUP": "1", "SAGE_NOOK_PATH": nook},
            extra_code=extra,
        )
        assert result.returncode == 0, f"stderr={result.stderr!r}"
    ...
    def test_eager_warmup_collection_open_failure_logs_and_does_not_block_server(self, tmp_path):
        ...
        # nook + error class included in the diagnostic
        assert f"nook={nook}" in result.stderr, result.stderr
    ...
    def test_eager_warmup_query_failure_logs_and_persists_to_log_file(self, tmp_path):
        ...
        both stderr AND ``SAGE_LOG_FILE`` — the latter is the whole
    ...
    def test_log_file_path_with_embedded_newline_does_not_crash(self, tmp_path):
        """``SAGE_LOG_FILE`` containing a newline (rare misconfig from
        ...
        result = self._run_main({"SAGE_LOG_FILE": bad_path})
        assert result.returncode == 0, f"stderr={result.stderr!r}"
    ...
    def test_log_file_invalid_path_failure_surfaces_before_first_log_record(self, tmp_path):
        ...
        BEFORE the ``sage MCP Server starting...`` banner — proving that
        ...
        banner_pos = result.stderr.find("sage MCP Server starting")
...
class TestHandleRequest:
    def test_initialize(self):
        from sage_mcp.mcp_server import handle_request

        resp = handle_request({"method": "initialize", "id": 1, "params": {}})
        assert resp["result"]["serverInfo"]["name"] == "sage"
        assert resp["id"] == 1
    ...
    def test_initialize_unknown_version_falls_back_to_latest(self):
        ...
        from sage_mcp.mcp_server import SUPPORTED_PROTOCOL_VERSIONS

        assert resp["result"]["protocolVersion"] == SUPPORTED_PROTOCOL_VERSIONS[0]

    def test_initialize_missing_version_uses_oldest(self):
        from sage_mcp.mcp_server import handle_request, SUPPORTED_PROTOCOL_VERSIONS
    ...
    def test_tools_list(self):
        ...
        assert "nook_status" in names
        assert "nook_search" in names
        assert "nook_add_drawer" in names
        assert "nook_kg_add" in names

    def test_null_arguments_does_not_hang(self, monkeypatch, config, nook_path, seeded_kg):
        """Sending arguments: null should return a result, not hang (#394)."""
        _patch_mcp_server(monkeypatch, config, seeded_kg)
        from sage_mcp.mcp_server import handle_request

        _client, _col = _get_collection(nook_path, create=True)
        del _client
        resp = handle_request(
            {
                "method": "tools/call",
                "id": 10,
                "params": {"name": "nook_status", "arguments": None},
            }
        )
    ...
    def test_tools_call_dispatches(self, monkeypatch, config, nook_path, seeded_kg):
        _patch_mcp_server(monkeypatch, config, seeded_kg)
        from sage_mcp.mcp_server import handle_request
        ...
        resp = handle_request(
            {
                "method": "tools/call",
                "id": 5,
                "params": {"name": "nook_status", "arguments": {}},
            }
        )
...
class TestReadTools:
    def test_status_cold_start_no_collection(self, monkeypatch, config, nook_path, kg):
        """Status on a valid nook with no ChromaDB collection yet (#830).

        After `sage init`, chroma.sqlite3 exists but the nook_drawers
        collection has not been created (no mine or add_drawer yet).  Status
        should return total_drawers: 0, not 'No nook found'.
        """
        import chromadb
    ...
    def test_status_empty_nook(self, monkeypatch, config, nook_path, kg):
        _patch_mcp_server(monkeypatch, config, kg)
        _client, _col = _get_collection(nook_path, create=True)
        del _client
        from sage_mcp.mcp_server import tool_status
    ...
    def test_status_with_data(self, monkeypatch, config, nook_path, seeded_collection, kg):
        _patch_mcp_server(monkeypatch, config, kg)
        from sage_mcp.mcp_server import tool_status
    ...
    def test_status_handles_none_metadata_without_partial(self, monkeypatch, config, nook_path, kg):
        """tool_status must not crash or go partial when the metadata cache
        returns a ``None`` entry — nooks can contain drawers with no
        ...
        with _patch("sage_mcp.mcp_server._get_collection") as mock_get_col:
            fake_col = type("C", (), {"count": lambda self: 2})()
            mock_get_col.return_value = fake_col
            with _patch(
                "sage_mcp.mcp_server._get_cached_metadata",
    ...
    def test_list_wings(self, monkeypatch, config, nook_path, seeded_collection, kg):
        _patch_mcp_server(monkeypatch, config, kg)
    ...
    def test_list_rooms_all(self, monkeypatch, config, nook_path, seeded_collection, kg):
        _patch_mcp_server(monkeypatch, config, kg)
        from sage_mcp.mcp_server import tool_list_rooms
    ...
    def test_list_rooms_filtered(self, monkeypatch, config, nook_path, seeded_collection, kg):
        _patch_mcp_server(monkeypatch, config, kg)
        from sage_mcp.mcp_server import tool_list_rooms
    ...
    def test_get_taxonomy(self, monkeypatch, config, nook_path, seeded_collection, kg):
        _patch_mcp_server(monkeypatch, config, kg)
        from sage_mcp.mcp_server import tool_get_taxonomy
    ...
    def test_no_nook_returns_error(self, monkeypatch, config, kg):
        _patch_mcp_server(monkeypatch, config, kg)
        from sage_mcp.mcp_server import tool_status
...
class TestNoneMetadataSafety:
    ...
    insert time, so we can't reproduce the prior state by writing
    ...
    def test_safe_meta_helper_coerces_none_to_empty_dict(self):
        from sage_mcp.mcp_server import _safe_meta
    ...
    def test_get_drawer_tolerates_none_metadata(self, monkeypatch, config, nook_path, kg):
        _patch_mcp_server(monkeypatch, config, kg)
        from unittest.mock import MagicMock
    ...
    def test_list_drawers_tolerates_none_metadata(self, monkeypatch, config, nook_path, kg):
        _patch_mcp_server(monkeypatch, config, kg)
        from unittest.mock import MagicMock
    ...
    def test_update_drawer_tolerates_none_metadata(self, monkeypatch, config, nook_path, kg):
        _patch_mcp_server(monkeypatch, config, kg)
        from unittest.mock import MagicMock
    ...
    def test_delete_drawer_audit_log_tolerates_none_metadata(
        self, monkeypatch, config, nook_path, kg
    ):
        _patch_mcp_server(monkeypatch, config, kg)
        from unittest.mock import MagicMock
...
class TestSearchTool:
    def test_search_basic(self, monkeypatch, config, nook_path, seeded_collection, kg):
        _patch_mcp_server(monkeypatch, config, kg)
    ...
    def test_search_with_wing_filter(self, monkeypatch, config, nook_path, seeded_collection, kg):
        _patch_mcp_server(monkeypatch, config, kg)
        from sage_mcp.mcp_server import tool_search
    ...
    def test_search_with_room_filter(self, monkeypatch, config, nook_path, seeded_collection, kg):
        _patch_mcp_server(monkeypatch, config, kg)
        from sage_mcp.mcp_server import tool_search
    ...
    def test_search_rejects_invalid_room(self, monkeypatch, config, kg):
        ...
        # Name validation lives in ops.search now and runs before any search,
        # so the bad room is rejected without touching the index.
        result = mcp_server.tool_search(query="JWT", room="../backend")
        assert "error" in result
        assert result.get("error_kind") == "validation"

    def test_search_delegates_to_ops(self, monkeypatch, config, kg):
        """tool_search delegates to ops.search with decoded args. Per ADR-0074
        ops resolves the backend itself (health()/reset() live on the backend),
        so tool_search no longer injects probe/reset functions; the resilience
        policy is covered in test_ops.py."""
        _patch_mcp_server(monkeypatch, config, kg)
        from sage_mcp import mcp_server, ops

        captured = {}

        def fake_ops_search(**kwargs):
            captured.update(kwargs)
            return {"results": []}

        monkeypatch.setattr(ops, "search", fake_ops_search)

        result = mcp_server.tool_search(query="anything")

        assert result == {"results": []}
        assert captured["nook_path"] == mcp_server._config.nook_path
        assert captured["collection_name"] == mcp_server._config.collection_name
        # The retired injected hooks are gone.
        assert "probe_fn" not in captured
        assert "reset_hook" not in captured
    ...
    def test_wal_redacts_sensitive_fields(self, monkeypatch, config, kg, tmp_path):
        ...
        # _WAL_FILE was replaced by _wal_file_cache in C3 (lazy-init); patch the
        # cache directly so _ensure_wal() returns our tmp path without writing to
        # the real ~/.sage/wal.
        monkeypatch.setattr(mcp_server, "_wal_file_cache", wal_file)
...
class TestWriteTools:
    def test_add_drawer(self, monkeypatch, config, nook_path, kg):
        _patch_mcp_server(monkeypatch, config, kg)
        _client, _col = _get_collection(nook_path, create=True)
        del _client
    ...
    def test_add_drawer_duplicate_detection(self, monkeypatch, config, nook_path, kg):
        _patch_mcp_server(monkeypatch, config, kg)
        _client, _col = _get_collection(nook_path, create=True)
        del _client
        from sage_mcp.mcp_server import tool_add_drawer
    ...
    def test_add_drawer_scrubs_secret_tokens(self, monkeypatch, config, nook_path, kg):
        """Drawer content containing a fake secret token is scrubbed at the write boundary.

        `tool_add_drawer` applies `scrub_secrets` before persisting so no
        secret-pattern token (GitHub PAT, Anthropic key, etc.) survives
        in the stored nook document.
        """
        _patch_mcp_server(monkeypatch, config, kg)
        _client, _col = _get_collection(nook_path, create=True)
        del _client
        ...
        raw_content = (
            "The CI pipeline uses token ghp_FakeGitHubTokenForTestOnly1234567890 "
            "to authenticate with GitHub. This should be redacted on storage."
        )
        result = tool_add_drawer(wing="test_wing", room="secrets_test", content=raw_content)
        assert result["success"] is True

        # Retrieve the stored document and confirm the token was scrubbed.
        import chromadb

        client = chromadb.PersistentClient(path=nook_path)
        col = client.get_collection("nook_drawers")
        stored = col.get(ids=[result["drawer_id"]], include=["documents"])
        assert stored["documents"], "Drawer was not found after write"
        stored_doc = stored["documents"][0]

        assert "ghp_FakeGitHubTokenForTestOnly1234567890" not in stored_doc, (
            "Secret token must not survive in persisted nook document"
        )
        assert "[REDACTED]" in stored_doc, (
            "Scrubbed token must be replaced with [REDACTED] in stored document"
        )

    def test_add_drawer_shared_header_no_collision(self, monkeypatch, config, nook_path, kg):
    ...
    def test_delete_drawer(self, monkeypatch, config, nook_path, seeded_collection, kg):
        _patch_mcp_server(monkeypatch, config, kg)
        from sage_mcp.mcp_server import tool_delete_drawer

        result = tool_delete_drawer("drawer_proj_backend_aaa")
        assert result["success"] is True
        assert seeded_collection.count() == 3

    def test_delete_drawer_not_found(self, monkeypatch, config, nook_path, seeded_collection, kg):
        _patch_mcp_server(monkeypatch, config, kg)
        from sage_mcp.mcp_server import tool_delete_drawer
    ...
    def test_check_duplicate(self, monkeypatch, config, nook_path, seeded_collection, kg):
        _patch_mcp_server(monkeypatch, config, kg)
    ...
    def test_check_duplicate_short_circuits_when_vector_disabled(self, monkeypatch):
        from sage_mcp import mcp_server
        import sage_mcp.backends.chroma as chroma

        # The capacity probe now runs through backend.health() → chroma
        # (ADR-0074); patch it at its real home, not the old mcp_server alias.
        monkeypatch.setattr(
            chroma,
    ...
    def test_get_drawer(self, monkeypatch, config, nook_path, seeded_collection, kg):
        _patch_mcp_server(monkeypatch, config, kg)
    ...
    def test_get_drawer_not_found(self, monkeypatch, config, nook_path, seeded_collection, kg):
        _patch_mcp_server(monkeypatch, config, kg)
        from sage_mcp.mcp_server import tool_get_drawer
    ...
    def test_list_drawers(self, monkeypatch, config, nook_path, seeded_collection, kg):
        _patch_mcp_server(monkeypatch, config, kg)
    ...
    def test_list_drawers_with_wing_filter(
        self, monkeypatch, config, nook_path, seeded_collection, kg
    ):
        _patch_mcp_server(monkeypatch, config, kg)
        from sage_mcp.mcp_server import tool_list_drawers
    ...
    def test_list_drawers_pagination(self, monkeypatch, config, nook_path, seeded_collection, kg):
        _patch_mcp_server(monkeypatch, config, kg)
        from sage_mcp.mcp_server import tool_list_drawers
    ...
    def test_update_drawer_content(self, monkeypatch, config, nook_path, seeded_collection, kg):
        _patch_mcp_server(monkeypatch, config, kg)
        from sage_mcp.mcp_server import tool_update_drawer, tool_get_drawer
    ...
    def test_update_drawer_not_found(self, monkeypatch, config, nook_path, seeded_collection, kg):
        _patch_mcp_server(monkeypatch, config, kg)
        from sage_mcp.mcp_server import tool_update_drawer
    ...
    def test_update_drawer_noop(self, monkeypatch, config, nook_path, seeded_collection, kg):
        _patch_mcp_server(monkeypatch, config, kg)
        from sage_mcp.mcp_server import tool_update_drawer
    ...
    def test_update_drawer_scrubs_secret_tokens(
        self, monkeypatch, config, nook_path, seeded_collection, kg
    ):
        """ADR-0042: tool_update_drawer must scrub secrets from new content
        before persisting (write-boundary scrub applies to all write paths)."""
        _patch_mcp_server(monkeypatch, config, kg)
        from sage_mcp.mcp_server import tool_update_drawer, tool_get_drawer

        raw_content = (
            "Updated note: token ghp_FakeGitHubTokenForTestOnly1234567890 "
            "was rotated. This must be redacted."
        )
        result = tool_update_drawer("drawer_proj_backend_aaa", content=raw_content)
        assert result["success"] is True

        fetched = tool_get_drawer("drawer_proj_backend_aaa")
        assert "ghp_FakeGitHubTokenForTestOnly1234567890" not in fetched["content"], (
            "Secret token must not survive in updated nook document (ADR-0042)"
        )
        assert "[REDACTED]" in fetched["content"]

    def test_update_drawer_git_sha_survives(
        self, monkeypatch, config, nook_path, seeded_collection, kg
    ):
        """ADR-0042: git SHA-1 (40 hex chars) must survive verbatim in update_drawer.
        The high-confidence write-boundary scrub does NOT redact hex-only strings."""
        _patch_mcp_server(monkeypatch, config, kg)
        from sage_mcp.mcp_server import tool_update_drawer, tool_get_drawer

        git_sha = "a" * 40  # 40-char hex-only string (git SHA-1 shape)
        content = f"Fixed in commit {git_sha} — resolved the race condition."
        result = tool_update_drawer("drawer_proj_backend_aaa", content=content)
        assert result["success"] is True

        fetched = tool_get_drawer("drawer_proj_backend_aaa")
        assert git_sha in fetched["content"], (
            f"Git SHA-1 {git_sha!r} was incorrectly redacted by update_drawer (ADR-0042 I#5)"
        )
    ...
    def test_tool_create_tunnel_preserves_hyphenated_wings(self, monkeypatch, tmp_path):
        ...
        from sage_mcp import mcp_server, nook_graph

        tunnel_file = tmp_path / "tunnels.json"
        monkeypatch.setattr(nook_graph, "_get_tunnel_file", lambda *a, **kw: str(tunnel_file))
        monkeypatch.setattr(nook_graph, "_get_collection", lambda *a, **kw: None)
    ...
    def test_tool_create_tunnel_rejects_unregistered_source_wing(self, monkeypatch):
        """Fail-loud invariant: an unregistered source_wing must produce a
        WingNotRegisteredError that surfaces as {"error": ...}, never a
        phantom tunnel. Regression for the Cat 9 fail-loud gap closed in
        the multi-agent audit pass."""
        from sage_mcp import mcp_server

        result = mcp_server.tool_create_tunnel(
            source_wing="this-wing-is-definitely-not-registered",
            source_room="room-a",
            target_wing="sage",
            target_room="room-b",
        )
        assert "error" in result
        assert "this-wing-is-definitely-not-registered" in result["error"]

    def test_tool_create_tunnel_rejects_unregistered_target_wing(self, monkeypatch):
        """Same fail-loud invariant for target_wing."""
        from sage_mcp import mcp_server

        result = mcp_server.tool_create_tunnel(
            source_wing="sage",
            source_room="room-a",
            target_wing="this-wing-is-definitely-not-registered",
            target_room="room-b",
        )
        assert "error" in result
        assert "this-wing-is-definitely-not-registered" in result["error"]

    def test_add_drawer_normal_content_single_drawer(self, monkeypatch, config, nook_path, kg):
        ...
        _client2, col = _get_collection(nook_path)
        del _client2
        assert col.count() == 1
        assert col.get()["ids"] == [result["drawer_id"]]

    def test_add_drawer_oversized_content_chunked(self, monkeypatch, config, nook_path, kg):
    ...
    def test_add_drawer_oversized_idempotency_skips_duplicate_chunk_writes(
        ...
        also the older logical drawer_id so a pre-#1539 single-row
    ...
    def test_add_drawer_chunked_logical_id_not_fetchable_directly(
        ...
        _client, _col = _get_collection(nook_path, create=True)
        del _client
        from sage_mcp.mcp_server import tool_add_drawer, tool_delete_drawer, tool_get_drawer
...
class TestKGTools:
    def test_kg_add(self, monkeypatch, config, nook_path, kg):
        _patch_mcp_server(monkeypatch, config, kg)
        from sage_mcp.mcp_server import tool_kg_add
    ...
    def test_kg_query(self, monkeypatch, config, nook_path, seeded_kg):
        _patch_mcp_server(monkeypatch, config, seeded_kg)
        from sage_mcp.mcp_server import tool_kg_query

        result = tool_kg_query(entity="Max")
        assert result["count"] > 0

    def test_kg_invalidate(self, monkeypatch, config, nook_path, seeded_kg):
        _patch_mcp_server(monkeypatch, config, seeded_kg)
        from sage_mcp.mcp_server import tool_kg_invalidate
    ...
    def test_kg_add_forwards_valid_to(self, monkeypatch, config, nook_path, kg):
    ...
    def test_kg_add_forwards_source_provenance(self, monkeypatch, config, nook_path, kg):
    ...
    def test_kg_invalidate_returns_actual_ended_date(
        self, monkeypatch, config, nook_path, seeded_kg
    ):
    ...
    def test_kg_timeline(self, monkeypatch, config, nook_path, seeded_kg):
        _patch_mcp_server(monkeypatch, config, seeded_kg)
        from sage_mcp.mcp_server import tool_kg_timeline

        result = tool_kg_timeline(entity="Alice")
        assert result["count"] > 0

    def test_kg_stats(self, monkeypatch, config, nook_path, seeded_kg):
        _patch_mcp_server(monkeypatch, config, seeded_kg)
        from sage_mcp.mcp_server import tool_kg_stats
    ...
    def test_kg_add_rejects_invalid_valid_from(self, monkeypatch, config, nook_path, kg):
        _patch_mcp_server(monkeypatch, config, kg)
        from sage_mcp.mcp_server import tool_kg_add
    ...
    def test_kg_query_rejects_invalid_as_of(self, monkeypatch, config, nook_path, seeded_kg):
        _patch_mcp_server(monkeypatch, config, seeded_kg)
        from sage_mcp.mcp_server import tool_kg_query
    ...
    def test_kg_invalidate_rejects_invalid_ended(self, monkeypatch, config, nook_path, seeded_kg):
        _patch_mcp_server(monkeypatch, config, seeded_kg)
        from sage_mcp.mcp_server import tool_kg_invalidate
    ...
    def test_kg_query_rejects_partial_iso_dates(self, monkeypatch, config, nook_path, seeded_kg):
        _patch_mcp_server(monkeypatch, config, seeded_kg)
        from sage_mcp.mcp_server import tool_kg_query
    ...
    def test_kg_add_accepts_datetime_valid_from(self, monkeypatch, config, nook_path, kg):
        _patch_mcp_server(monkeypatch, config, kg)
    ...
    def test_kg_add_accepts_datetime_valid_to(self, monkeypatch, config, nook_path, kg):
        _patch_mcp_server(monkeypatch, config, kg)
    ...
    def test_kg_query_accepts_datetime_as_of(self, monkeypatch, config, nook_path, kg):
        _patch_mcp_server(monkeypatch, config, kg)
    ...
    def test_kg_invalidate_accepts_datetime_ended(self, monkeypatch, config, nook_path, kg):
        _patch_mcp_server(monkeypatch, config, kg)
    ...
    def test_kg_add_rejects_non_canonical_datetimes(self, monkeypatch, config, nook_path, kg):
        _patch_mcp_server(monkeypatch, config, kg)
    ...
    def test_kg_invalidate_rejects_non_canonical_ended(self, monkeypatch, config, nook_path, kg):
        _patch_mcp_server(monkeypatch, config, kg)
    ...
    def test_kg_add_rejects_timezone_offset_datetime(self, monkeypatch, config, nook_path, kg):
        _patch_mcp_server(monkeypatch, config, kg)
    ...
    def test_kg_add_scrubs_secrets_from_subject_and_object(
        self, monkeypatch, config, nook_path, kg
    ):
        """ADR-0042: tool_kg_add must scrub secrets from subject and object
        before persisting (write-boundary scrub applies to all write paths)."""
        _patch_mcp_server(monkeypatch, config, kg)
        from sage_mcp.mcp_server import tool_kg_add, tool_kg_query

        raw_object = "ghp_FakeGitHubTokenForTestOnly1234567890 provisioned"
        result = tool_kg_add(
            subject="CI_pipeline",
            predicate="uses_token",
            object=raw_object,
        )
        assert result["success"] is True

        facts = tool_kg_query("CI_pipeline")
        stored_object = facts["facts"][0]["object"] if facts["facts"] else ""
        assert "ghp_FakeGitHubTokenForTestOnly1234567890" not in stored_object, (
            "Secret token must not survive in KG triple object (ADR-0042)"
        )
        assert "[REDACTED]" in stored_object

    def test_kg_add_git_sha_survives(self, monkeypatch, config, nook_path, kg):
        """ADR-0042: git SHA-1 (40 hex chars) must survive in KG triple values.
        The high-confidence write-boundary scrub does NOT redact hex-only strings."""
        _patch_mcp_server(monkeypatch, config, kg)
        from sage_mcp.mcp_server import tool_kg_add, tool_kg_query

        git_sha = "d" * 40  # 40-char hex-only string (git SHA-1 shape)
        result = tool_kg_add(
            subject="release",
            predicate="tagged_at",
            object=git_sha,
        ...
        facts = tool_kg_query("release")
        stored_object = facts["facts"][0]["object"] if facts["facts"] else ""
        assert git_sha in stored_object, (
            f"Git SHA-1 {git_sha!r} was incorrectly redacted by kg_add (ADR-0042 I#5)"
        )
...
class TestDiaryTools:
    def test_diary_write_and_read(self, monkeypatch, config, nook_path, kg):
        _patch_mcp_server(monkeypatch, config, kg)
        _client, _col = _get_collection(nook_path, create=True)
        del _client
        from sage_mcp.mcp_server import tool_diary_write, tool_diary_read
    ...
    def test_diary_read_empty(self, monkeypatch, config, nook_path, kg):
        _patch_mcp_server(monkeypatch, config, kg)
        _client, _col = _get_collection(nook_path, create=True)
        del _client
    ...
    def test_diary_write_rejects_unregistered_explicit_wing(
        self, monkeypatch, config, nook_path, kg
    ):
        """Fail-loud invariant: an explicit wing= argument naming an
        unregistered wing must produce success=False with an error
        message, never a phantom wing. Regression for the Cat 9
        fail-loud gap closed in the multi-agent audit pass. The empty
        wing path (auto-generated `wing_<agent>` diary pattern) is
        exempted by ``is_registered``'s ``_DIARY_WING_PREFIX`` allowance
        and is covered by ``test_diary_write_and_read``."""
        _patch_mcp_server(monkeypatch, config, kg)
        _client, _col = _get_collection(nook_path, create=True)
        del _client
        ...
        result = tool_diary_write(
            ...
            entry="probe",
            topic="probe",
            wing="this-wing-is-definitely-not-registered",
        )
        assert result["success"] is False
        assert "this-wing-is-definitely-not-registered" in result["error"]
    ...
    def test_diary_write_same_second_shared_prefix_no_collision(
        ...
        from sage_mcp.mcp_server import tool_diary_read, tool_diary_write
    ...
    def test_diary_read_empty_wing_spans_all_wings(self, monkeypatch, config, nook_path, kg):
    ...
    def test_diary_read_case_insensitive_agent(self, monkeypatch, config, nook_path, kg):
    ...
    def test_diary_write_normal_entry_single_drawer(self, monkeypatch, config, nook_path, kg):
    ...
    def test_diary_write_oversized_entry_chunked(self, monkeypatch, config, nook_path, kg):
    ...
    def test_diary_write_chunk_index_metadata(self, monkeypatch, config, nook_path, kg):
    ...
    def test_diary_write_scrubs_secret_tokens(self, monkeypatch, config, nook_path, kg):
        """ADR-0042: tool_diary_write must scrub secrets from the entry
        before persisting (write-boundary scrub applies to all write paths)."""
        _patch_mcp_server(monkeypatch, config, kg)
        _client, _col = _get_collection(nook_path, create=True)
        del _client
        ...
        raw_entry = (
            "Today I rotated the API key. Old token: sk-ant-api01-FakeKeyForTest1234567890 "
            "was invalidated. New key provisioned."
        )
        w = tool_diary_write(agent_name="TestAgent", entry=raw_entry, topic="security")
        assert w["success"] is True

        r = tool_diary_read(agent_name="TestAgent")
        stored_content = r["entries"][0]["content"]
        assert "sk-ant-api01-FakeKeyForTest1234567890" not in stored_content, (
            "Secret token must not survive in diary entry (ADR-0042)"
        )
        assert "[REDACTED]" in stored_content

    def test_diary_write_git_sha_survives(self, monkeypatch, config, nook_path, kg):
        """ADR-0042: git SHA-1 (40 hex chars) must survive verbatim in diary_write.
        The high-confidence write-boundary scrub does NOT redact hex-only strings."""
        _patch_mcp_server(monkeypatch, config, kg)
        _client, _col = _get_collection(nook_path, create=True)
        del _client
        ...
        git_sha = "c" * 40  # 40-char hex-only string (git SHA-1 shape)
        entry = f"Merged commit {git_sha} into main — resolves #1234."
        w = tool_diary_write(agent_name="TestAgent", entry=entry, topic="releases")
        assert w["success"] is True

        r = tool_diary_read(agent_name="TestAgent")
        assert git_sha in r["entries"][0]["content"], (
            f"Git SHA-1 {git_sha!r} was incorrectly redacted by diary_write (ADR-0042 I#5)"
        )
...
class TestCacheInvalidation:
    ...
    def test_mtime_change_invalidates_cache(self, monkeypatch, config, nook_path, kg):
        ...
        old_mtime = mcp_server._nook_db_mtime
        monkeypatch.setattr(mcp_server, "_nook_db_mtime", old_mtime - 10.0)

        # _get_collection should detect the mtime drift and reconnect
        col2 = mcp_server._get_collection()
        assert col2 is not None

    def test_inode_change_invalidates_cache(self, monkeypatch, config, nook_path, kg):
        ...
        monkeypatch.setattr(mcp_server, "_nook_db_inode", 99999)
    ...
    def test_missing_db_invalidates_cache(self, monkeypatch, config, nook_path, kg):
        ...
        db_file = os.path.join(nook_path, "chroma.sqlite3")
        ...
        assert mcp_server._nook_db_inode == 0
        assert mcp_server._nook_db_mtime == 0.0

    def test_reconnect_reports_failure_when_no_nook(self, monkeypatch, config, kg):
        ...
        assert result["success"] is False
        assert "No nook found" in result["message"]
        assert result["drawers"] == 0

    def test_reconnect_reports_success(self, monkeypatch, config, nook_path, kg):
    ...
    def test_reconnect_closes_shared_backend(self, monkeypatch, config, kg):
        ...
        from sage_mcp import mcp_server, nook

        close_nook = MagicMock()
        monkeypatch.setattr(nook._DEFAULT_BACKEND, "close_nook", close_nook)
        ...
        result = mcp_server.tool_reconnect()
        assert result["success"] is True
        close_nook.assert_called_once_with(config.nook_path)
    ...
    def test_get_collection_passes_embedding_function(self, monkeypatch, config, nook_path, kg):
        """Regression for #1299.
    ...
    def test_get_collection_retries_once_on_exception(self, monkeypatch, config, nook_path, kg):
...
class TestKGLazyCache:
    ...
    def test_lazy_init_no_import_side_effect(self, tmp_path):
        ...
        kg_file = tmp_path / ".sage" / "knowledge_graph.sqlite3"
        env = dict(os.environ)
        env["HOME"] = str(tmp_path)
        env["USERPROFILE"] = str(tmp_path)
        result = subprocess.run(
            [sys.executable, "-c", "import sage_mcp.mcp_server"],
            env=env,
            capture_output=True,
            text=True,
            timeout=30,
    ...
    def test_get_kg_returns_same_instance(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(mcp_server, "_nook_flag_given", True)
        monkeypatch.setenv("SAGE_NOOK_PATH", str(tmp_path))
    ...
    def test_get_kg_different_paths_different_instances(self, tmp_path, monkeypatch):
        """Different nook paths map to different KG instances."""
        from sage_mcp import mcp_server
        ...
        monkeypatch.setenv("SAGE_NOOK_PATH", str(tmp_a))
        kg_a = mcp_server._get_kg()
        monkeypatch.setenv("SAGE_NOOK_PATH", str(tmp_b))
    ...
    def test_multi_tenant_env_switch(self, tmp_path, monkeypatch):
        ...
        Rotating SAGE_NOOK_PATH between MCP tool calls must route
...
class TestStructuredErrors:
    ...
    def test_internal_tool_error_without_exc_has_no_data_field(self):
        ...
        from sage_mcp.mcp_server import _internal_tool_error

        try:
            raise ValueError("test error")
        except ValueError:
            resp = _internal_tool_error("req-1", "nook_search")
    ...
    def test_internal_tool_error_with_exc_includes_structured_data(self):
        ...
        try:
            ...
            resp = _internal_tool_error("req-2", "nook_add_drawer", exc)
    ...
    def test_internal_tool_error_exception_dispatch_passes_exc(self, monkeypatch):
        ...
        req = {
            ...
            "id": "r1",
            "method": "tools/call",
            "params": {"name": "nook_fake", "arguments": {}},
        }
    ...
    def test_tool_sync_mine_already_running_returns_error_class(self, monkeypatch, tmp_path):
        ...
        import sage_mcp.sync as sync_mod

        monkeypatch.setattr(sync_mod, "sync_nook", _raise_locked, raising=False)
    ...
    def test_mcp_idle_timeout_invalid_env_disables_watchdog(self, monkeypatch):
        """Invalid SAGE_MCP_IDLE_HOURS disables idle auto-exit."""
        from sage_mcp import mcp_server

        monkeypatch.setenv("SAGE_MCP_IDLE_HOURS", "not-a-float")
    ...
    def test_call_kg_retries_after_concurrent_close(self, monkeypatch):
        ...
        path = "/fake/nook/knowledge_graph.sqlite3"
    ...
    def test_canonicalize_kg_path_collapses_symlink_alias(self, tmp_path):
        """A symlink layer over the nook directory must collapse to one
    ...
    def test_canonicalize_kg_path_routes_through_normcase(self, monkeypatch):
        ...
        (``C:\\nook`` vs ``c:\\nook``). On POSIX runners normcase is a
...
class TestParamShapeDiagnostics:
    ...
    def test_missing_required_returns_32602_with_param_name(self):
        ...
        assert "nook_diary_write" in resp["error"]["message"]
    ...
    def test_handler_internal_typeerror_stays_generic_32000(self, monkeypatch):
        ...
        monkeypatch.setitem(mcp_server.TOOLS["nook_status"], "handler", boom)
        ...
        resp = handle_request(
            {
                "method": "tools/call",
                "id": 4,
                "params": {"name": "nook_diary_write", "arguments": {}},
            }
        )
        assert resp["error"]["code"] == -32602
    ...
    def test_handler_internal_signature_shape_stays_generic(self, monkeypatch):
        ...
        monkeypatch.setitem(mcp_server.TOOLS["nook_status"], "handler", calling_handler)
...
class TestUnknownParamName:
    ...
    **kwargs pass-through handlers must keep accepting unknown kwargs.
    ...
    def test_unknown_param_returns_32602_naming_the_wrong_kwarg(self):
        ...
        assert "nook_add_drawer" in message
    ...
    def test_wait_for_previous_not_flagged_as_unknown(self, monkeypatch):
        ...
        monkeypatch.setitem(mcp_server.TOOLS["nook_diary_write"], "handler", stub)
    ...
    def test_kwargs_passthrough_handler_keeps_accepting_unknown(self, monkeypatch):
        ...
        monkeypatch.setitem(mcp_server.TOOLS["nook_status"], "handler", passthrough)

        resp = mcp_server.handle_request(
            {
                "method": "tools/call",
                "id": 10,
                "params": {"name": "nook_status", "arguments": {"bogus": 1}},
            }
        )
        assert "error" not in resp
        assert "result" in resp
...
# ── C3: WAL lazy init ─────────────────────────────────────────────────────
# Verifies that importing sage_mcp.mcp_server does NOT create ~/.sage/wal under
# a redirected HOME.  The WAL dir+file must only appear after the first
# _wal_log() call (ADR-0095; C3 isolation hardening).


def test_wal_not_created_on_import(tmp_path):
    """Importing sage_mcp.mcp_server with HOME=tmp must leave ~/.sage/wal absent."""
    env = os.environ.copy()
    env["HOME"] = str(tmp_path)
    env["USERPROFILE"] = str(tmp_path)
    code = "import sage_mcp.mcp_server"
    result = subprocess.run(
        [sys.executable, "-c", code],
        env=env,
        capture_output=True,
    ...
    wal_dir = tmp_path / ".sage" / "wal"
    assert not wal_dir.exists(), (
        f"WAL dir created at import time (should be lazy): {wal_dir}; stderr={result.stderr!r}"
    )


def test_wal_created_on_first_write(tmp_path, monkeypatch):
    """_wal_log() must create ~/.sage/wal/write_log.jsonl on first call."""
    monkeypatch.setenv("HOME", str(tmp_path))
    monkeypatch.setenv("USERPROFILE", str(tmp_path))

    from sage_mcp import mcp_server

    # Reset the cached WAL path so _ensure_wal() resolves fresh.
    monkeypatch.setattr(mcp_server, "_wal_file_cache", None)

    mcp_server._wal_log("test_op", {"key": "val"})

    wal_file = tmp_path / ".sage" / "wal" / "write_log.jsonl"
    assert wal_file.exists(), "WAL file should exist after first _wal_log() call"
    entry = json.loads(wal_file.read_text().strip())
