...
hid behind ``except Exception: pass``:
  ...
  * ``KnowledgeGraph(nook_path=...)`` is not a valid kwarg — code
    must pass ``db_path``.
...
Also pins the three feature contracts:
  * similar_name  — "Lisa" vs "Liza" in a registry with both.
...
from sage_mcp.fact_checker import (
...
class TestEntityConfusion:
    def test_flags_near_name_when_only_one_mentioned(self):
        registry = {"people": ["Lisa", "Liza"]}
        issues = _check_entity_confusion("I spoke with  Liza today.", registry)
        # "Liza" mentioned, "Lisa" not — registry has both at edit-distance 1,
        # flag the possible confusion.
        assert len(issues) == 1
        assert issues[0]["type"] == "similar_name"
        assert set(issues[0]["names"]) == {"Liza", "Lisa"}
        assert issues[0]["distance"] == 1

    def test_no_false_positive_when_both_names_mentioned(self):
        """Regression: a text discussing both  Liza and Lisa is fine —
        the user clearly knows they're different. Don't nag."""
        registry = {"people": ["Lisa", "Liza"]}
        issues = _check_entity_confusion("Liza and Lisa met for lunch.", registry)
        assert issues == []
    ...
    def test_registry_dict_shape_is_supported(self):
        ...
        registry = {"people": {"Lisa": {"role": "owner"}, "Liza": {}}}
        issues = _check_entity_confusion("I messaged  Liza yesterday", registry)
        assert any("Lisa" in (i["names"] or []) for i in issues)
...
class TestEditDistance:
    ...
    def test_performance_bounded_by_mentioned_names(self):
        ...
        # 0 here) — if it's still doing O(n²) over 500 names, this is
        # tens of seconds. Budget set to 1.0s to absorb CI / WSL load
        # spikes while still catching the regression.
        assert elapsed < 1.0, f"entity confusion took {elapsed:.3f}s on empty mentions"
...
# ── KG integration (uses a real tmp SQLite nook) ───────────────────


@pytest.fixture
def nook_with_kg(tmp_path):
    """Nook directory with a real KG pre-seeded with a few triples.

    The KG file lives at ``<nook>/knowledge_graph.sqlite3`` — same
    convention used by the MCP server. Fact-checker must find it via
    that path, not via a bogus ``nook_path`` kwarg.
    """
    nook = tmp_path / "nook"
    nook.mkdir()
    db = str(nook / "knowledge_graph.sqlite3")
    kg = KnowledgeGraph(db_path=db)
    yield nook, kg


class TestKGContradictions:
    def test_kg_init_uses_db_path_not_nook_path_kwarg(self):
        """Regression: the original code passed ``nook_path=`` to a
    ...
    def test_no_false_positive_when_kg_has_no_facts_about_subject(self, nook_with_kg):
        nook, _ = nook_with_kg
        # KG is empty → no mismatch should fire.
        assert check_text("Bob is Alice's brother", str(nook)) == []

    def test_stale_fact_detected(self, nook_with_kg):
        nook, kg = nook_with_kg
    ...
    def test_current_fact_same_triple_is_not_flagged(self, nook_with_kg):
        nook, kg = nook_with_kg
        kg.add_triple("Bob", "brother", "Alice", valid_from="2010-01-01")
        issues = check_text("Bob is Alice's brother", str(nook))
        assert issues == []

    def test_missing_nook_does_not_crash(self, tmp_path):
        """Brand-new nook (no KG file yet) — check_text must return []
...
class TestCheckTextContract:
    def test_empty_text_returns_empty_list(self, tmp_path):
        assert check_text("", str(tmp_path / "nook")) == []
    ...
    def test_registry_confusion_path_isolated_from_kg(self, tmp_path, monkeypatch):
        ...
        registry.write_text(json.dumps({"people": ["Lisa", "Liza"]}))
        ...
        issues = check_text("Chatted with Liza.", str(tmp_path / "nonexistent_nook"))
...
class TestCLI:
    def test_exits_nonzero_when_issues_found(self, tmp_path, monkeypatch, capsys):
        ...
        registry = tmp_path / "known_entities.json"
        registry.write_text(json.dumps({"people": ["Lisa", "Liza"]}))
        from sage_mcp import fact_checker, miner

        monkeypatch.setattr(miner, "_ENTITY_REGISTRY_PATH", str(registry))
        miner._ENTITY_REGISTRY_CACHE.update({"mtime": None, "names": frozenset(), "raw": {}})

        # Simulate argv: "Liza said hi"
        monkeypatch.setattr(
            "sys.argv",
            ["fact_checker", "Liza said hi", "--nook", str(tmp_path / "nook")],
        )
        with pytest.raises(SystemExit) as excinfo:
            # Re-exec the __main__ block via runpy.
            import runpy

            runpy.run_module("sage_mcp.fact_checker", run_name="__main__")
    ...
    def test_reconfigures_stdio_to_utf8_on_windows(self):
        ...
        from sage_mcp.fact_checker import _reconfigure_stdio_utf8_on_windows
