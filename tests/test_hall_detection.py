...
class TestDetectHall:
    ...
    def test_function_exists(self):
        from sage_mcp.miner import detect_hall
...
class TestDrawerHasHallMetadata:
    ...
    def test_add_drawer_includes_hall(self, nook_path):
        ...
        add_drawer(
            ...
            source_file=os.path.join(nook_path, "test.py"),
            chunk_index=0,
            agent="test",
        )
...
class TestConvoMinerWritesHalls:
    ...
    def test_convo_miner_drawers_have_hall(self, tmp_dir):
        ...
        nook_dir = os.path.join(tmp_dir, "nook")
        ...
        mine_convos(convo_dir, nook_dir, wing="test", agent="test")

        col = get_collection(nook_dir, create=False)
...
class TestDetectHallCaching:
    ...
    def test_detect_hall_does_not_reread_config(self):
        """After first call, config should be cached — no new SageConfig()."""
        import sage_mcp.miner as miner_mod
...
class TestMineProjectWritesHalls:
    ...
    def test_mined_drawers_have_hall(self, tmp_dir):
        ...
        with open(os.path.join(project_dir, "sage.yaml"), "w") as f:
            yaml.dump(config, f)
        ...
        mine(project_dir, nook_dir, wing_override="test", agent="test")
