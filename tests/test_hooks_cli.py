...
import sage_mcp.hooks_cli as hooks_cli_mod
from sage_mcp.hooks_cli import (
    SAVE_INTERVAL,
...
def _isolated_existing_nook_root(monkeypatch, tmp_path):
    """Give every test an isolated, *existing* SAGE_ROOT/STATE_DIR.

    Regression for #1510: nine save / log / precompact tests assumed
    ``~/.sage`` existed and only passed in the full suite because an
    earlier test file (``test_cli.py``) created it as a side effect, so
    the ``_nook_root_exists()`` kill-switch was satisfied. Run in
    isolation they short-circuited and failed.

    Defaulting every test to a per-test nook root that exists makes
    ...
    ``_redirect_nook_root`` (or set their own SAGE_ROOT) *after* this
    ...
    monkeypatch.setattr(hooks_cli_mod, "SAGE_ROOT", root)
...
def test_sage_python_returns_string():
    result = _sage_python()
    assert isinstance(result, str)
    assert "python" in result


def test_sage_python_finds_venv():
    """Should resolve to a valid Python interpreter path."""
    result = _sage_python()
    assert result and "python" in os.path.basename(result).lower()


def test_sage_python_handles_shallow_path_without_crashing(monkeypatch):
    """Regression: _sage_python must not raise IndexError when the
    ...
    sage/`` install. In editable installs at a shallow path (Docker
    ...
    with patch("sage_mcp.hooks_cli.Path", return_value=fake_path):
...
def _capture_hook_output(hook_fn, data, harness="claude-code", state_dir=None):
    ...
    patches = [
        patch(
            "sage_mcp.hooks_cli._output",
            side_effect=lambda d: buf.write(json.dumps(d)),
        )
    ]
    if state_dir:
        patches.append(patch("sage_mcp.hooks_cli.STATE_DIR", state_dir))
    # Mock SageConfig so tests don't depend on user's ~/.sage/config.json
    ...
    patches.append(patch("sage_mcp.config.SageConfig", return_value=mock_config))
...
def test_stop_hook_passthrough_when_active(tmp_path):
    with patch("sage_mcp.hooks_cli.STATE_DIR", tmp_path):
...
def test_stop_hook_saves_silently_at_interval(tmp_path):
    ...
    with patch("sage_mcp.hooks_cli._save_diary_direct", return_value=save_result) as mock_save:
        result = _capture_hook_output(
            hook_stop,
    ...
    assert result["systemMessage"].startswith("\u2726 15 memories woven into the nook")
...
def test_stop_hook_tracks_save_point(tmp_path):
    ...
    with patch("sage_mcp.hooks_cli._save_diary_direct", return_value=save_result):
    ...
    with patch("sage_mcp.hooks_cli._save_diary_direct") as mock_save:
        result = _capture_hook_output(hook_stop, data, state_dir=tmp_path)
    assert result == {}
...
def test_wing_from_transcript_path_extracts_project():
    path = "/home/jp/.claude/projects/-home-jp-Projects-memorynook/session.jsonl"
    assert _wing_from_transcript_path(path) == "wing_memorynook"
...
def test_wing_from_transcript_path_non_projects_layout():
    ...
    # system).
    path = "/home/user/.claude/projects/-home-user-dev-foo-bar/session.jsonl"
    assert _wing_from_transcript_path(path) == "wing_foo_bar"
...
def test_wing_from_transcript_path_uses_cwd_from_jsonl(tmp_path):
    ...
    noisier) wing."""
    # Encoded folder says ``-home-user-dev-foo-bar`` (would yield
    # ``wing_foo_bar`` via fallback), but cwd is the truth.
    project_dir = tmp_path / "-home-user-dev-foo-bar"
    project_dir.mkdir()
    transcript = project_dir / "session.jsonl"
    transcript.write_text(
        '{"type":"queue-operation","operation":"enqueue","timestamp":"2026-05-09T00:00:00Z"}\n'
        '{"type":"user","cwd":"/home/user/dev/foo/bar","content":"hi"}\n',
        encoding="utf-8",
    )
...
def test_output_writes_to_real_stdout_fd_when_mcp_server_loaded():
    ...
    fake_module = types.ModuleType("sage_mcp.mcp_server")

    read_fd, write_fd = os.pipe()
    try:
        fake_module._REAL_STDOUT_FD = write_fd
        with patch.dict("sys.modules", {"sage_mcp.mcp_server": fake_module}):
            from sage_mcp.hooks_cli import _output
...
def test_log_oserror_is_silenced(tmp_path):
    ...
    with patch("sage_mcp.hooks_cli.STATE_DIR", Path("/nonexistent/deeply/nested/dir")):
        # Should not raise
...
def test_maybe_auto_ingest_no_env(tmp_path):
    """Without SAGE_DIR or transcript_path, does nothing."""
...
def test_maybe_auto_ingest_with_env(tmp_path):
    """With SAGE_DIR set, spawns mine in projects mode against that dir."""
    sage_dir = tmp_path / "project"
    sage_dir.mkdir()
    with patch.dict("os.environ", {"SAGE_DIR": str(sage_dir)}):
        with patch("sage_mcp.hooks_cli.STATE_DIR", tmp_path):
            with patch("sage_mcp.hooks_cli._MINE_PID_DIR", tmp_path / "mine_pids"):
                with patch("sage_mcp.hooks_cli.subprocess.Popen") as mock_popen:
                    _maybe_auto_ingest()
                    mock_popen.assert_called_once()
                    cmd = mock_popen.call_args[0][0]
                    assert "mine" in cmd
                    assert str(sage_dir.resolve()) in cmd
                    assert cmd[cmd.index("--mode") + 1] == "projects"


def test_maybe_auto_ingest_uses_sage_python(tmp_path):
    """Spawned mine command uses _sage_python(), not bare sys.executable.
    ...
    under (typically the venv that owns sage).
    """
    sage_dir = tmp_path / "project"
    sage_dir.mkdir()
    with patch.dict("os.environ", {"SAGE_DIR": str(sage_dir)}):
        with patch("sage_mcp.hooks_cli.STATE_DIR", tmp_path):
            with patch("sage_mcp.hooks_cli._MINE_PID_DIR", tmp_path / "mine_pids"):
                with patch("sage_mcp.hooks_cli._sage_python", return_value="/fake/venv/python"):
                    with patch("sage_mcp.hooks_cli.subprocess.Popen") as mock_popen:
                        _maybe_auto_ingest()
                        cmd = mock_popen.call_args[0][0]
...
def test_mine_sync_with_env_uses_projects_mode(tmp_path):
    """Precompact sync path uses projects mode when SAGE_DIR is set."""
    sage_dir = tmp_path / "project"
    sage_dir.mkdir()
    with patch.dict("os.environ", {"SAGE_DIR": str(sage_dir)}):
        with patch("sage_mcp.hooks_cli.STATE_DIR", tmp_path):
            with patch("sage_mcp.hooks_cli.subprocess.run") as mock_run:
                _mine_sync()
...
def test_mine_sync_uses_sage_python(tmp_path):
    """Sync mine command uses _sage_python(), not bare sys.executable."""
    sage_dir = tmp_path / "project"
    sage_dir.mkdir()
    with patch.dict("os.environ", {"SAGE_DIR": str(sage_dir)}):
        with patch("sage_mcp.hooks_cli.STATE_DIR", tmp_path):
...
def test_claim_mine_slot_writes_live_placeholder_pid(tmp_path):
    ...
    cmd = ["sage", "mine", "/tmp/proj", "--mode", "projects"]
    pid_dir = tmp_path / "mine_pids"

    with patch("sage_mcp.hooks_cli._MINE_PID_DIR", pid_dir):
...
def test_claim_mine_slot_reclaimed_slot_writes_live_placeholder_pid(tmp_path):
    ...
    with (
        patch("sage_mcp.hooks_cli._MINE_PID_DIR", pid_dir),
        patch("sage_mcp.hooks_cli._pid_alive", return_value=False),
    ):
...
def test_maybe_auto_ingest_ignores_transcript_arg_path(tmp_path):
    ...
    in hook handlers). _maybe_auto_ingest only handles SAGE_DIR — even
...
def test_maybe_auto_ingest_oserror(tmp_path):
    ...
    with patch.dict("os.environ", {"SAGE_DIR": str(sage_dir)}):
        with patch("sage_mcp.hooks_cli.STATE_DIR", tmp_path):
            with patch("sage_mcp.hooks_cli._MINE_PID_DIR", tmp_path / "mine_pids"):
                with patch("sage_mcp.hooks_cli.subprocess.Popen", side_effect=OSError("fail")):
                    _maybe_auto_ingest()  # should not raise
    ...
    with patch.dict("os.environ", {"SAGE_DIR": str(sage_dir)}):
        with patch("sage_mcp.hooks_cli.STATE_DIR", tmp_path):
            with patch("sage_mcp.hooks_cli._MINE_PID_DIR", pid_dir):
                ...
                from sage_mcp.hooks_cli import _pid_file_for_cmd
                ...
                with patch("sage_mcp.hooks_cli._sage_python", return_value=sys.executable):
                    with patch("sage_mcp.hooks_cli.subprocess.Popen") as mock_popen:
                        _maybe_auto_ingest()
                        mock_popen.assert_not_called()
...
def test_detached_popen_kwargs_posix(monkeypatch):
    ...
    from sage_mcp.hooks_cli import _detached_popen_kwargs

    monkeypatch.setattr("sage_mcp.hooks_cli.os.name", "posix")
...
def test_detached_popen_kwargs_windows(monkeypatch):
    ...
    monkeypatch.setattr("sage_mcp.hooks_cli.os.name", "nt")
    ...
    monkeypatch.setattr("sage_mcp.hooks_cli.subprocess.DETACHED_PROCESS", 0x00000008, raising=False)
    monkeypatch.setattr(
        "sage_mcp.hooks_cli.subprocess.CREATE_NEW_PROCESS_GROUP", 0x00000200, raising=False
    )
    kwargs = _detached_popen_kwargs()
    assert kwargs.get("stdin") is subprocess.DEVNULL
    assert kwargs.get("close_fds") is True
    ...
    with patch("sage_mcp.hooks_cli.STATE_DIR", tmp_path):
        with patch("sage_mcp.hooks_cli._MINE_PID_DIR", tmp_path / "mine_pids"):
            with patch("sage_mcp.hooks_cli.subprocess.Popen") as mock_popen:
                ...
                from sage_mcp.hooks_cli import _spawn_mine

                _spawn_mine(["sage", "mine", "/tmp/x"])
...
def test_spawn_mine_skips_when_target_running(tmp_path):
    ...
    with patch("sage_mcp.hooks_cli.STATE_DIR", tmp_path):
        with patch("sage_mcp.hooks_cli._MINE_PID_DIR", pid_dir):
            from sage_mcp.hooks_cli import _pid_file_for_cmd, _spawn_mine
    ...
    with patch("sage_mcp.hooks_cli.STATE_DIR", tmp_path):
        with patch("sage_mcp.hooks_cli._MINE_PID_DIR", pid_dir):
            with patch("sage_mcp.hooks_cli.subprocess.Popen") as mock_popen:
                ...
                _spawn_mine(["sage", "mine", "/tmp/a", "--mode", "projects"])
                mock_popen.return_value.pid = 2222
                _spawn_mine(["sage", "mine", "/tmp/b", "--mode", "projects"])
...
def test_spawn_mine_releases_slot_on_oserror(tmp_path):
    ...
    with patch("sage_mcp.hooks_cli.STATE_DIR", tmp_path):
        with patch("sage_mcp.hooks_cli._MINE_PID_DIR", pid_dir):
            ...
            with patch("sage_mcp.hooks_cli.subprocess.Popen", side_effect=OSError("spawn fail")):
...
def test_spawn_mine_passes_pid_file_env_var(tmp_path):
    """The child inherits SAGE_MINE_PID_FILE so its cleanup hook can find the slot."""
    pid_dir = tmp_path / "mine_pids"
    with patch("sage_mcp.hooks_cli.STATE_DIR", tmp_path):
        with patch("sage_mcp.hooks_cli._MINE_PID_DIR", pid_dir):
            with patch("sage_mcp.hooks_cli.subprocess.Popen") as mock_popen:
                ...
                cmd = ["sage", "mine", "/tmp/x", "--mode", "projects"]
                _spawn_mine(cmd)
                child_env = mock_popen.call_args.kwargs.get("env", {})
                expected = str(_pid_file_for_cmd(cmd))
                assert child_env.get("SAGE_MINE_PID_FILE") == expected
...
def test_ingest_transcript_uses_detached_kwargs(tmp_path):
    ...
    with patch("sage_mcp.hooks_cli.STATE_DIR", tmp_path):
        with patch("sage_mcp.hooks_cli._MINE_PID_DIR", tmp_path / "mine_pids"):
            with patch("sage_mcp.hooks_cli.subprocess.Popen") as mock_popen:
                from sage_mcp.hooks_cli import _ingest_transcript
...
def test_ingest_transcript_skips_when_target_running(tmp_path):
    ...
    with patch("sage_mcp.hooks_cli.STATE_DIR", tmp_path):
        with patch("sage_mcp.hooks_cli._MINE_PID_DIR", pid_dir):
            with patch("sage_mcp.hooks_cli._sage_python", return_value=sys.executable):
                from sage_mcp.hooks_cli import _ingest_transcript, _pid_file_for_cmd
...
def test_mine_already_running_bare_pid_old_mtime_is_stale(tmp_path):
    ...
    with (
        patch("sage_mcp.hooks_cli._MINE_PID_DIR", pid_dir),
        patch.dict("os.environ", {"SAGE_MINE_TIMEOUT_HOURS": "1"}),
    ):
        assert _mine_already_running(cmd) is False
...
def test_mine_slot_timeout_invalid_env_disables_timeout():
    """Invalid SAGE_MINE_TIMEOUT_HOURS disables stale-by-age checks."""
    from sage_mcp.hooks_cli import _mine_slot_timeout_secs

    with patch.dict("os.environ", {"SAGE_MINE_TIMEOUT_HOURS": "nope"}):
...
def test_mine_already_running_live_pid_within_timeout(tmp_path):
    ...
    with (
        patch("sage_mcp.hooks_cli._MINE_PID_DIR", pid_dir),
        patch.dict("os.environ", {"SAGE_MINE_TIMEOUT_HOURS": "2"}),
    ):
        assert _mine_already_running(cmd) is True


def test_mine_already_running_timeout_zero_disables_check(tmp_path):
    """SAGE_MINE_TIMEOUT_HOURS=0 disables the age-based stale check."""
    import time as _time
    ...
    with (
        patch("sage_mcp.hooks_cli._MINE_PID_DIR", pid_dir),
        patch.dict("os.environ", {"SAGE_MINE_TIMEOUT_HOURS": "0"}),
    ):
...
def test_mine_already_running_distinct_cmds_independent(tmp_path):
    ...
    pid_dir = tmp_path / "mine_pids"
    cmd_a = ["sage", "mine", "/tmp/a", "--mode", "projects"]
    cmd_b = ["sage", "mine", "/tmp/b", "--mode", "projects"]
...
def test_get_mine_targets_sage_dir_only(tmp_path):
    """SAGE_DIR alone yields a single projects target, expanded/resolved."""
    sage_dir = tmp_path / "project"
    sage_dir.mkdir()
    with patch.dict("os.environ", {"SAGE_DIR": str(sage_dir)}):
        targets = _get_mine_targets()
    assert len(targets) == 1
    assert Path(targets[0][0]).resolve() == sage_dir.resolve()
    assert targets[0][1] == "projects"


def test_get_mine_targets_sage_dir_tilde(tmp_path):
    """SAGE_DIR with a tilde prefix is expanded correctly."""
    sage_dir = tmp_path / "project"
    sage_dir.mkdir()
    home = Path.home()
    try:
        rel = sage_dir.relative_to(home)
    except ValueError:
        pytest.skip("tmp_path is not under home, cannot build ~-relative path")
    tilde_path = "~/" + str(rel)
    with patch.dict("os.environ", {"SAGE_DIR": tilde_path}):
        targets = _get_mine_targets()
    assert len(targets) == 1
    assert Path(targets[0][0]).resolve() == sage_dir.resolve()
    assert targets[0][1] == "projects"
...
def test_get_mine_targets_only_returns_sage_dir(tmp_path):
    """When SAGE_DIR is set, exactly one projects target — never a convos target."""
    sage_dir = tmp_path / "project"
    sage_dir.mkdir()
    with patch.dict("os.environ", {"SAGE_DIR": str(sage_dir)}):
        targets = _get_mine_targets()
...
def test_get_mine_targets_empty():
    """Returns empty list when SAGE_DIR is unset or invalid."""
    with patch.dict("os.environ", {}, clear=True):
...
# --- hook_precompact with SAGE_DIR ---


def test_precompact_with_sage_dir(tmp_path):
    """Precompact runs subprocess.run (sync) when SAGE_DIR is set."""
    sage_dir = tmp_path / "project"
    sage_dir.mkdir()
    with patch.dict("os.environ", {"SAGE_DIR": str(sage_dir)}):
        with patch("sage_mcp.hooks_cli.subprocess.run") as mock_run:
...
def test_precompact_with_sage_dir_oserror(tmp_path):
    """Precompact handles OSError from subprocess gracefully."""
    sage_dir = tmp_path / "project"
    sage_dir.mkdir()
    with patch.dict("os.environ", {"SAGE_DIR": str(sage_dir)}):
        with patch("sage_mcp.hooks_cli.subprocess.run", side_effect=OSError("fail")):
            result = _capture_hook_output(
                hook_precompact,
                {"session_id": "test"},
                state_dir=tmp_path,
...
def test_precompact_with_timeout(tmp_path):
    ...
    with patch.dict("os.environ", {"SAGE_DIR": str(sage_dir)}):
        with patch(
            "sage_mcp.hooks_cli.subprocess.run",
...
def test_precompact_mines_transcript_dir(tmp_path, monkeypatch):
    ...
    With no SAGE_DIR, _mine_sync is a no-op; the transcript ingest is
    ...
    transcript.write_text("x" * 200)
    monkeypatch.delenv("SAGE_DIR", raising=False)
    with patch("sage_mcp.hooks_cli.subprocess.Popen") as mock_popen:
        with patch("sage_mcp.hooks_cli.subprocess.run") as mock_run:
            result = _capture_hook_output(
                hook_precompact,
...
def test_run_hook_dispatches_session_start(tmp_path):
    ...
    with patch("sys.stdin", io.StringIO(stdin_data)):
        with patch("sage_mcp.hooks_cli.STATE_DIR", tmp_path):
            with patch("sage_mcp.hooks_cli._output") as mock_output:
...
def test_stop_hook_disabled_by_config(tmp_path):
    ...
    with patch("sage_mcp.hooks_cli.SageConfig") as mock_cfg_cls:
...
def test_precompact_hook_enabled_by_default(tmp_path):
    ...
    with patch("sage_mcp.hooks_cli.SageConfig") as mock_cfg_cls:
        mock_cfg_cls.return_value.hooks_auto_save = True
        with patch("sage_mcp.hooks_cli._mine_sync") as mock_mine:
            result = _capture_hook_output(
                hook_precompact,
                {"session_id": "test"},
                state_dir=tmp_path,
...
def test_count_logs_warning_on_rejected_path(tmp_path):
    ...
    with patch("sage_mcp.hooks_cli.STATE_DIR", tmp_path):
        with patch("sage_mcp.hooks_cli._log") as mock_log:
...
def test_stop_hook_rejects_injected_stop_hook_active(tmp_path):
    ...
    it was invoked regardless of silent vs verbose save mode.
    """
    transcript = tmp_path / "t.jsonl"
    _write_transcript(
        transcript,
    ...
    with patch(
        "sage_mcp.hooks_cli._save_diary_direct", return_value={"count": 1, "themes": []}
    ) as mock_save:
        _capture_hook_output(
            hook_stop,
            {
...
# --- Absent nook root: hooks must not recreate ~/.sage ---
#
# When the user removes ~/.sage (e.g. `rm -rf`), that is the strongest
...
def _redirect_nook_root(monkeypatch, tmp_path):
    """Point SAGE_ROOT and STATE_DIR at a tmp location that does NOT exist."""
    fake_root = tmp_path / "absent-sage"
    monkeypatch.setattr(hooks_cli_mod, "SAGE_ROOT", fake_root)
    monkeypatch.setattr(hooks_cli_mod, "STATE_DIR", fake_root / "hook_state")
    monkeypatch.setattr(hooks_cli_mod, "_state_dir_initialized", False)
    return fake_root


def test_hook_stop_does_not_create_nook_dir_when_absent(tmp_path, monkeypatch):
    fake_root = _redirect_nook_root(monkeypatch, tmp_path)
    transcript = tmp_path / "t.jsonl"
    transcript.write_text("")
    buf = io.StringIO()
...
def test_hook_precompact_does_not_create_nook_dir_when_absent(tmp_path, monkeypatch):
    fake_root = _redirect_nook_root(monkeypatch, tmp_path)
    transcript = tmp_path / "t.jsonl"
    transcript.write_text("")
    buf = io.StringIO()
...
def test_hook_session_start_does_not_create_nook_dir_when_absent(tmp_path, monkeypatch):
    fake_root = _redirect_nook_root(monkeypatch, tmp_path)
    buf = io.StringIO()
    with contextlib.redirect_stdout(buf):
...
def test_log_does_not_create_nook_dir_when_absent(tmp_path, monkeypatch):
    fake_root = _redirect_nook_root(monkeypatch, tmp_path)
    _log("test message")
    assert not fake_root.exists()
...
def test_existing_dir_proceeds_normally(tmp_path, monkeypatch):
    """Regression: when SAGE_ROOT exists, hooks must proceed (no short-circuit)."""
    fake_root = tmp_path / "present-sage"
    fake_root.mkdir()
    monkeypatch.setattr(hooks_cli_mod, "SAGE_ROOT", fake_root)
    monkeypatch.setattr(hooks_cli_mod, "STATE_DIR", fake_root / "hook_state")
    monkeypatch.setattr(hooks_cli_mod, "_state_dir_initialized", False)
    _log("test message")
    # _log should have created the state dir under the existing nook root
    assert (fake_root / "hook_state").exists()
    assert (fake_root / "hook_state" / "hook.log").is_file()


def test_regular_file_at_nook_root_treated_as_absent(tmp_path, monkeypatch):
    """A regular file at ~/.sage must be treated the same as absent.
    ...
    on ``NotADirectoryError``. ``_nook_root_exists()`` must use
    ...
    # _nook_root_exists() is the source of truth — it must return False.
    assert hooks_cli_mod._nook_root_exists() is False
...
# ── SAGE_HOOK_PROFILE dial tests (P3.2) ─────────────────────────────────────


class TestHookProfile:
    """Tests for the SAGE_HOOK_PROFILE=minimal|standard|off dial (P3.2)."""

    def test_default_is_standard(self, monkeypatch):
        """Unset env var resolves to 'standard'."""
        monkeypatch.delenv("SAGE_HOOK_PROFILE", raising=False)
        assert _hook_profile() == "standard"

    def test_unknown_value_resolves_to_standard(self, monkeypatch):
        """Unrecognised value resolves to 'standard' (safe default)."""
        monkeypatch.setenv("SAGE_HOOK_PROFILE", "turbo")
        assert _hook_profile() == "standard"

    def test_off_value(self, monkeypatch):
        monkeypatch.setenv("SAGE_HOOK_PROFILE", "off")
        assert _hook_profile() == "off"

    def test_minimal_value(self, monkeypatch):
        monkeypatch.setenv("SAGE_HOOK_PROFILE", "minimal")
        assert _hook_profile() == "minimal"

    def test_standard_value(self, monkeypatch):
        monkeypatch.setenv("SAGE_HOOK_PROFILE", "standard")
        assert _hook_profile() == "standard"

    def test_case_insensitive(self, monkeypatch):
        """Profile values are lowercased before comparison."""
        monkeypatch.setenv("SAGE_HOOK_PROFILE", "OFF")
        assert _hook_profile() == "off"
        monkeypatch.setenv("SAGE_HOOK_PROFILE", "Minimal")
        assert _hook_profile() == "minimal"

    def _run_hook(self, hook_fn, data=None):
        """Run a hook function and return the parsed JSON output."""
        if data is None:
            data = {"session_id": "test123", "transcript_path": "", "stop_hook_active": False}
        buf = io.StringIO()
        with contextlib.redirect_stdout(buf):
            hook_fn(data, "claude-code")
        return json.loads(buf.getvalue() or "{}")

    def test_off_hook_stop_noop(self, monkeypatch):
        """SAGE_HOOK_PROFILE=off causes hook_stop to emit {} immediately."""
        monkeypatch.setenv("SAGE_HOOK_PROFILE", "off")
        result = self._run_hook(hook_stop)
        assert result == {}

    def test_off_hook_session_start_noop(self, monkeypatch):
        """SAGE_HOOK_PROFILE=off causes hook_session_start to emit {} immediately."""
        monkeypatch.setenv("SAGE_HOOK_PROFILE", "off")
        result = self._run_hook(hook_session_start)
        assert result == {}

    def test_off_hook_precompact_noop(self, monkeypatch):
        """SAGE_HOOK_PROFILE=off causes hook_precompact to emit {} immediately."""
        monkeypatch.setenv("SAGE_HOOK_PROFILE", "off")
        result = self._run_hook(hook_precompact)
        assert result == {}

    def test_minimal_hook_stop_noop(self, monkeypatch):
        """SAGE_HOOK_PROFILE=minimal suppresses diary auto-save in hook_stop."""
        monkeypatch.setenv("SAGE_HOOK_PROFILE", "minimal")
        result = self._run_hook(hook_stop)
        assert result == {}

    def test_minimal_hook_precompact_noop(self, monkeypatch):
        """SAGE_HOOK_PROFILE=minimal suppresses transcript ingest in hook_precompact."""
        monkeypatch.setenv("SAGE_HOOK_PROFILE", "minimal")
        result = self._run_hook(hook_precompact)
        assert result == {}

    def test_minimal_hook_session_start_passes_through(self, monkeypatch):
        """SAGE_HOOK_PROFILE=minimal allows session tracking init in hook_session_start."""
        monkeypatch.setenv("SAGE_HOOK_PROFILE", "minimal")
        result = self._run_hook(hook_session_start)
        # session_start always returns {} whether minimal or standard
        assert result == {}

    def test_off_does_not_call_sage_config(self, monkeypatch):
        """SAGE_HOOK_PROFILE=off must short-circuit before reading SageConfig."""
        monkeypatch.setenv("SAGE_HOOK_PROFILE", "off")
        called = []

        class _FakeConfig:
            def __init__(self):
                called.append("SageConfig()")

            hooks_auto_save = True
            hook_silent_save = True
            hook_desktop_toast = False

        monkeypatch.setattr(hooks_cli_mod, "SageConfig", _FakeConfig)
        self._run_hook(hook_stop)
        assert called == [], "SageConfig should not be instantiated when profile=off"

    def test_minimal_does_not_call_sage_config(self, monkeypatch):
        """SAGE_HOOK_PROFILE=minimal must short-circuit before reading SageConfig."""
        monkeypatch.setenv("SAGE_HOOK_PROFILE", "minimal")
        called = []
        ...
        monkeypatch.setattr(hooks_cli_mod, "SageConfig", _FakeConfig)
        self._run_hook(hook_stop)
        assert called == [], "SageConfig should not be instantiated when profile=minimal"


# ── _redaction_enabled config opt-in branch ──────────────────────────────────


class TestRedactionEnabledConfigOptIn:
    """Cover the config-property branch of _redaction_enabled in hooks_cli.py.

    Acceptance criteria (P3 fold, code-reviewer F2):
      - With env UNSET and SageConfig.hooks_redact_secrets=True → redaction on.
      - With env UNSET and SageConfig.hooks_redact_secrets=False → redaction off.
    """

    def test_config_opt_in_enables_redaction(self, monkeypatch):
        """env UNSET + config=True → _redaction_enabled returns True."""
        ...
        monkeypatch.setattr(hooks_cli_mod, "SageConfig", _FakeConfig)
        assert hooks_cli_mod._redaction_enabled() is True

    def test_config_opt_out_disables_redaction(self, monkeypatch):
        """env UNSET + config=False → _redaction_enabled returns False."""
        monkeypatch.delenv("SAGE_REDACT_SECRETS", raising=False)
        ...
        monkeypatch.setattr(hooks_cli_mod, "SageConfig", _FakeConfig)
        assert hooks_cli_mod._redaction_enabled() is False

    def test_env_takes_precedence_over_config(self, monkeypatch):
        """env=1 overrides config=False → redaction on."""
    ...
    def test_env_false_overrides_config_true(self, monkeypatch):
        """env=0 overrides config=True → redaction off."""
    ...
    def test_env_uppercase_TRUE_enables_redaction(self, monkeypatch):
        """SAGE_REDACT_SECRETS=TRUE (all-caps) must enable redaction.

        Before the fix, .strip() without .lower() meant TRUE/YES/True were
        NOT matched → redaction silently stayed OFF.
    ...
    def test_env_uppercase_YES_enables_redaction(self, monkeypatch):
    ...
    def test_env_titlecase_True_enables_redaction(self, monkeypatch):
    ...
    def test_env_zero_stays_off(self, monkeypatch):
        """SAGE_REDACT_SECRETS=0 remains OFF regardless of case fix."""
        monkeypatch.setenv("SAGE_REDACT_SECRETS", "0")
...
# ── diary redaction path ──────────────────────────────────────────────────────


class TestDiaryRedactionPath:
    """Cover the _maybe_scrub wiring in _save_diary_direct (P3 fold, adversarial F3)."""

    def test_diary_entry_scrubbed_when_redaction_on(self, tmp_path, monkeypatch):
        """When SAGE_REDACT_SECRETS=1, credential-shaped content in the diary entry
        is scrubbed before being passed to tool_diary_write."""
        monkeypatch.setenv("SAGE_REDACT_SECRETS", "1")

        # Build a transcript with a credential-shaped message.
        transcript = tmp_path / "session.jsonl"
            ...
            _json.dumps(
                {
                    "message": {
                        "role": "user",
                        "content": "key=sk-ant-api03-FAKE_CREDENTIAL_FOR_DIARY_TEST",
                    }
                }
            )
            + "\n",
        ...
        def _fake_diary_write(agent_name, entry, topic, wing):
            captured_entry.append(entry)
            return {"success": True, "entry_id": "test-id"}

        with patch("sage_mcp.hooks_cli.SageConfig") as mock_cfg_cls:
            mock_cfg_cls.return_value.hooks_redact_secrets = True

            with patch("sage_mcp.mcp_server.tool_diary_write", _fake_diary_write):
                from sage_mcp.hooks_cli import _save_diary_direct

                _save_diary_direct(str(transcript), session_id="test-session")

        if captured_entry:
            # The credential must not appear verbatim in the stored entry.
            assert "sk-ant-api03" not in captured_entry[0], (
                f"Credential passed through unredacted: {captured_entry[0]}"
            )
        # If captured_entry is empty the transcript had no messages to save;
        # the scrubbing path was not exercised but the call did not error.

    def test_diary_entry_unchanged_when_redaction_off(self, tmp_path, monkeypatch):
        """When redaction is off (default), normal content passes through unmodified."""
        monkeypatch.delenv("SAGE_REDACT_SECRETS", raising=False)
        ...
        transcript.write_text(
            _json.dumps(
                {"message": {"role": "user", "content": "User discussed refactoring plans"}}
            )
            + "\n",
            encoding="utf-8",
        )
        ...
        with patch("sage_mcp.hooks_cli.SageConfig", _FakeConfig):
            with patch("sage_mcp.mcp_server.tool_diary_write", _fake_diary_write):
                from sage_mcp.hooks_cli import _save_diary_direct
        ...
        if captured_entry:
            assert "refactoring" in captured_entry[0], (
                f"Normal content unexpectedly modified: {captured_entry[0]}"
            )
