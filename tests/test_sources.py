"""Tests for the the source-adapter spec source-adapter scaffolding."""
...
from sage_mcp.sources.transforms import (
...
def test_conforming_adapter_instantiates_and_yields_typed_records():
    ...
    results = list(adapter.ingest(source=SourceRef(uri="foo"), nook=None))
...
def test_resolve_adapter_priority_order():
    ...
    # Default is filesystem (preserves existing ``sage mine <path>`` behavior).
...
def test_nook_context_upsert_drawer_stamps_adapter_metadata():
...
def test_nook_context_drawer_id_is_sha256_prefix_not_sha1():
    """Guards against the pre-review sha1[:16]=64-bit id scheme.

    64-bit ids sit close to the birthday bound for nook-sized corpora.
    The helper uses sha256[:24]=96 bits so collision risk stays negligible.
    """
    import hashlib

    from sage_mcp.sources.context import _build_drawer_id
...
def test_nook_context_skip_current_item_sets_flag():
    ctx = NookContext(
...
def test_nook_context_emit_dispatches_to_hooks_and_swallows_errors():
    calls = []
    err_calls = []
...
def test_nook_context_uses_route_hint_when_present():
...
# ---------------------------------------------------------------------------
# KnowledgeGraph new provenance params
# ---------------------------------------------------------------------------
...
def test_knowledge_graph_fresh_schema_includes_new_columns(tmp_path):
    """Brand-new nooks should get source_drawer_id / adapter_name directly
    from CREATE TABLE, not via a post-hoc ALTER. _migrate_schema exists only
    for earlier-format nooks."""
    from sage_mcp.knowledge_graph import KnowledgeGraph
...
def test_knowledge_graph_migration_adds_missing_columns_to_old_schema(tmp_path):
    """An old-schema triples table (pre-the source-adapter spec) should auto-migrate on open."""
...
def test_knowledge_graph_add_triple_backwards_compatible_without_new_kwargs(tmp_path):
    """Existing callers that omit the the source-adapter spec kwargs keep working unchanged."""
    from sage_mcp.knowledge_graph import KnowledgeGraph
...
def test_entry_point_group_exists_and_returns_zero_or_more_adapters():
    ...
    # but the ``sage.sources`` entry-point group is declared so third-
