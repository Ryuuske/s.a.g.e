"""Tests for sage.nook_graph — graph traversal layer.
...
# Patch chromadb at import time so nook_graph can be imported
...
class TestBuildGraph:
    ...
    def test_none_metadata_does_not_crash(self):
        """ChromaDB can return None for drawers with missing metadata
        (partial writes). build_graph must skip None entries silently
        rather than crash the whole graph build with AttributeError."""
        col = _make_fake_collection(
            [
                {"room": "auth", "wing": "wing_code", "hall": "security", "date": "2026-01-01"},
                None,  # partial-write drawer with no metadata
    ...
    def test_cache_returns_same_result(self):
        ...
        correct for the MCP server's single-nook use case. Callers that
