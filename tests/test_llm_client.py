...
from sage_mcp.llm_client import (
    AnthropicProvider,
    LLMError,
    OllamaProvider,
...
def test_http_post_json_success():
    ...
    with patch("sage_mcp.llm_client.urlopen", return_value=mock_resp):
...
def test_http_post_json_http_error_wraps_as_llm_error():
    ...
    with patch("sage_mcp.llm_client.urlopen", side_effect=err):
...
def test_http_post_json_url_error_wraps_as_llm_error():
    ...
    with patch("sage_mcp.llm_client.urlopen", side_effect=URLError("conn refused")):
...
def test_ollama_check_available_finds_model():
    ...
    mock.__enter__.return_value = mock
    mock.__exit__.return_value = False
    with patch("sage_mcp.llm_client.urlopen", return_value=mock):
...
def test_ollama_check_available_unreachable():
    ...
    with patch("sage_mcp.llm_client.urlopen", side_effect=URLError("refused")):
        p = OllamaProvider(model="gemma4:e4b")
        ok, msg = p.check_available()
    assert not ok
...
def test_ollama_classify_sends_json_format():
    ...
    with patch("sage_mcp.llm_client.urlopen", side_effect=fake_urlopen):
        p = OllamaProvider(model="gemma4:e4b")
...
def test_ollama_classify_empty_content_raises():
    with patch("sage_mcp.llm_client.urlopen", return_value=_mock_ollama_chat_response("")):
...
# off the user's machine/network. Used by sage init to print a
