"""Tests for format_miner (sage 0.1.0, integrated in PR #1555).
...
from sage_mcp.format_miner import (  # noqa: E402
...
def _make_symlink_or_skip(link: Path, target: Path) -> None:
    ...
    has nothing to do with the code under test. Per code review:
...
def test_fringe_too_large_respects_caller_max(tmp_path: Path):
    ...
    with patch("sage_mcp.format_miner._extract_via_markitdown", return_value="dummy text"):
...
def test_fringe_missing_markitdown(tmp_path: Path):
    ...
    f.write_bytes(b"%PDF-1.4\nstub")
    with patch(
        "sage_mcp.format_miner._extract_via_markitdown",
...
def test_fringe_icloud_skip_extraction(tmp_path: Path):
    ...
    f = tmp_path / "doc.pdf.icloud"
    f.write_bytes(b"placeholder")
    with patch("sage_mcp.format_miner._extract_via_markitdown") as mocked:
        text, status = extract_text(f)
    assert text is None
...
def test_extract_text_accepts_pathlib_path(tmp_path: Path):
    ...
    f = tmp_path / "doc.pdf"
    f.write_bytes(b"%PDF-1.4\nstub")
    with patch("sage_mcp.format_miner._extract_via_markitdown", return_value="content"):
        text, status = extract_text(f)
    assert status == ExtractionStatus.OK
...
def test_fringe_markitdown_returns_none(tmp_path: Path):
    ...
    f.write_bytes(b"%PDF-1.4\nweird")
    with patch("sage_mcp.format_miner._extract_via_markitdown", return_value=None):
        text, status = extract_text(f)
    assert text is None
    assert status == ExtractionStatus.SKIP_EXTRACTION_ERROR
...
def test_happy_path_pdf(tmp_path: Path):
    ...
    f.write_bytes(b"%PDF-1.4\nstub")
    with patch("sage_mcp.format_miner._extract_via_markitdown", return_value="# Research\n\nbody"):
        text, status = extract_text(f)
    assert status == ExtractionStatus.OK
...
def test_happy_path_docx(tmp_path: Path):
    ...
    with patch("sage_mcp.format_miner._extract_via_markitdown", return_value="# Notes\n\nbody"):
        text, status = extract_text(f)
    assert status == ExtractionStatus.OK
...
def test_happy_path_rtf(tmp_path: Path):
    ...
    with patch("sage_mcp.format_miner._extract_via_striprtf", return_value="memo body"):
        text, status = extract_text(f)
    assert status == ExtractionStatus.OK
...
def test_rtf_routes_to_striprtf_not_markitdown(tmp_path: Path):
    ...
    f.write_bytes(b"{\\rtf1\\ansi memo}")
    with (
        patch("sage_mcp.format_miner._extract_via_markitdown") as mock_md,
        patch("sage_mcp.format_miner._extract_via_striprtf", return_value="memo body") as mock_rtf,
    ):
        text, status = extract_text(f)
    assert status == ExtractionStatus.OK
    assert text == "memo body"
...
def test_non_rtf_does_not_touch_striprtf(tmp_path: Path):
    ...
    f.write_bytes(b"%PDF-1.4\nstub")
    with (
        patch("sage_mcp.format_miner._extract_via_markitdown", return_value="pdf text") as mock_md,
        patch("sage_mcp.format_miner._extract_via_striprtf") as mock_rtf,
    ):
        text, status = extract_text(f)
    assert status == ExtractionStatus.OK
...
def test_fringe_missing_striprtf(tmp_path: Path):
    ...
    f.write_bytes(b"{\\rtf1\\ansi memo}")
    with patch(
        "sage_mcp.format_miner._extract_via_striprtf",
...
def test_fringe_striprtf_returns_none(tmp_path: Path):
    ...
    with patch("sage_mcp.format_miner._extract_via_striprtf", return_value=None):
        text, status = extract_text(f)
    assert text is None
    assert status == ExtractionStatus.SKIP_EXTRACTION_ERROR
...
def test_fringe_striprtf_empty_output(tmp_path: Path):
    ...
    with patch("sage_mcp.format_miner._extract_via_striprtf", return_value=""):
        text, status = extract_text(f)
    assert text is None
    assert status == ExtractionStatus.SKIP_EXTRACTION_ERROR
...
def test_extract_via_striprtf_live(tmp_path: Path):
    ...
    from sage_mcp.format_miner import _extract_via_striprtf
...
def test_extract_via_markitdown_live_pdf(tmp_path: Path):
    ...
    from sage_mcp.format_miner import _extract_via_markitdown
...
# chunks the extracted text, and files drawers into the nook. Mirrors the
# shape of mine_convos / mine. The collection + lock primitives are mocked
# so these tests don't write to a real nook.
# ─────────────────────────────────────────────────────────────────────────────
...
def _mine_formats_mocks(tmp_path: Path):
    ...
    with (
        patch("sage_mcp.format_miner.get_collection", return_value=collection) as p_coll,
        patch("sage_mcp.format_miner.mine_lock", side_effect=_fake_lock) as p_lock,
        patch("sage_mcp.format_miner.file_already_mined", return_value=False) as p_mined,
    ):
        yield {
...
def test_mine_formats_walks_directory(_mine_formats_mocks):
    ...
    from unittest.mock import patch
    from sage_mcp.format_miner import mine_formats

    tmp = _mine_formats_mocks["tmp_path"]
    (tmp / "doc.pdf").write_bytes(b"pdf")
    with (
        patch("sage_mcp.format_miner.scan_formats", return_value=[]) as p_scan,
        patch("sage_mcp.format_miner._extract_via_markitdown"),
    ):
        mine_formats(wing="sage", format_dir=str(tmp), nook_path=str(tmp / "nook"))
...
def test_mine_formats_skips_already_mined_files(_mine_formats_mocks):
    ...
    with (
        patch("sage_mcp.format_miner.scan_formats", return_value=[f]),
        patch("sage_mcp.format_miner._extract_via_markitdown") as p_md,
    ):
        mine_formats(wing="sage", format_dir=str(tmp), nook_path=str(tmp / "nook"))
...
def test_mine_formats_files_drawers_for_ok_extractions(_mine_formats_mocks):
    ...
    with (
        patch("sage_mcp.format_miner.scan_formats", return_value=[f]),
        patch("sage_mcp.format_miner._extract_via_markitdown", return_value=long_text),
    ):
        mine_formats(wing="sage", format_dir=str(tmp), nook_path=str(tmp / "nook"))
...
def test_mine_formats_dry_run_does_not_open_collection(_mine_formats_mocks):
    ...
    with (
        patch("sage_mcp.format_miner.scan_formats", return_value=[f]),
        patch("sage_mcp.format_miner._extract_via_markitdown", return_value="some text"),
    ):
        mine_formats(wing="sage", format_dir=str(tmp), nook_path=str(tmp / "nook"), dry_run=True)
...
def test_mine_formats_respects_limit(_mine_formats_mocks):
    ...
    for i in range(5):
        ...
        files.append(p)
    with (
        patch("sage_mcp.format_miner.scan_formats", return_value=files),
        patch(
            "sage_mcp.format_miner._extract_via_markitdown", return_value="long text " * 50
        ) as p_md,
    ):
        mine_formats(wing="sage", format_dir=str(tmp), nook_path=str(tmp / "nook"), limit=2)
...
def test_mine_formats_wing_defaults_from_directory_name(_mine_formats_mocks):
    ...
    with (
        patch("sage_mcp.format_miner.scan_formats", return_value=[f]),
        patch("sage_mcp.format_miner._extract_via_markitdown", return_value="long text " * 50),
    ):
        # No explicit wing — auto-derivation from directory basename runs.
        # "my_research_corpus" is registered in tests/fixtures/wing_config.json
        # so the Phase 4 gate accepts the derived name.
        mine_formats(format_dir=str(target_dir), nook_path=str(tmp / "nook"))
...
def test_mine_formats_ingest_mode_metadata_is_extract(_mine_formats_mocks):
    ...
    distinguishable from project / convo drawers in the nook."""
    from unittest.mock import patch
    from sage_mcp.format_miner import mine_formats
...
#   - nook.SKIP_DIRS reuse (covered indirectly by scan_formats tests)
...
def test_mine_formats_uses_check_mtime_true(_mine_formats_mocks):
    ...
    with (
        patch("sage_mcp.format_miner.scan_formats", return_value=[f]),
        patch("sage_mcp.format_miner._extract_via_markitdown", return_value="long " * 50),
    ):
        mine_formats(wing="sage", format_dir=str(tmp), nook_path=str(tmp / "nook"))
...
def test_mine_formats_continues_after_per_file_error(_mine_formats_mocks):
    ...
    with (
        patch("sage_mcp.format_miner.scan_formats", return_value=[bad, good]),
        patch("sage_mcp.format_miner._extract_via_markitdown", return_value="text"),
        # Patch the module-level binding in format_miner (hoisted in the
        # PR #1555 polish work — Gemini #3). The old patch target
        # ``sage.miner.chunk_text`` no longer works because
        # format_miner now binds chunk_text at its own module scope.
        patch("sage_mcp.format_miner.chunk_text", side_effect=chunk_text_first_fails),
    ):
        # Must not raise
        mine_formats(wing="sage", format_dir=str(tmp), nook_path=str(tmp / "nook"))
...
def test_mine_formats_calls_load_config_for_rooms(_mine_formats_mocks):
    """mine_formats must load sage.yaml (via load_config) to get the
    ...
    with (
        patch("sage_mcp.format_miner.scan_formats", return_value=[f]),
        patch("sage_mcp.format_miner._extract_via_markitdown", return_value="long " * 50),
        patch("sage_mcp.format_miner.load_config", return_value=fake_config) as p_cfg,
    ):
        mine_formats(wing="sage", format_dir=str(tmp), nook_path=str(tmp / "nook"))
...
def test_mine_formats_calls_detect_room_per_file(_mine_formats_mocks):
    ...
    with (
        patch("sage_mcp.format_miner.scan_formats", return_value=[f]),
        patch("sage_mcp.format_miner._extract_via_markitdown", return_value="long " * 50),
        patch("sage_mcp.format_miner.load_config", return_value=fake_config),
        patch("sage_mcp.format_miner.detect_room", return_value="family") as p_room,
    ):
        mine_formats(wing="sage", format_dir=str(tmp), nook_path=str(tmp / "nook"))
...
def test_mine_formats_uses_detected_room_in_drawer_metadata(_mine_formats_mocks):
    ...
    with (
        ...
        patch("sage_mcp.format_miner._extract_via_markitdown", return_value="long " * 50),
        patch("sage_mcp.format_miner.load_config", return_value=fake_config),
        patch("sage_mcp.format_miner.detect_room", return_value="family"),
    ):
        mine_formats(wing="sage", format_dir=str(tmp), nook_path=str(tmp / "nook"))
    upsert_calls = _mine_formats_mocks["collection"].upsert.call_args_list
    found_room = None
...
def test_mine_formats_calls_compute_topic_tunnels_after_loop(_mine_formats_mocks):
    ...
    with (
        ...
        patch("sage_mcp.format_miner._extract_via_markitdown", return_value="long " * 50),
        patch("sage_mcp.format_miner.load_config", return_value=fake_config),
        patch("sage_mcp.format_miner.detect_room", return_value="documents"),
        patch("sage_mcp.format_miner._compute_topic_tunnels_for_wing", return_value=0) as p_tun,
    ):
        mine_formats(format_dir=str(tmp), nook_path=str(tmp / "nook"), wing="wing_alice")
    p_tun.assert_called_once_with("wing_alice")
...
def test_mine_formats_tunnel_failure_does_not_crash_mine(_mine_formats_mocks):
    ...
    with (
        ...
        patch("sage_mcp.format_miner.detect_room", return_value="documents"),
        patch(
            "sage_mcp.format_miner._compute_topic_tunnels_for_wing",
...
# ─────────────────────────────────────────────────────────────────────────────
# Code review — missing-format-deps surfacing + pyproject extras
# ─────────────────────────────────────────────────────────────────────────────
...
def test_extract_text_missing_format_dep_returns_distinct_status(tmp_path: Path, monkeypatch):
    ...
    Per code review: real PDFs hit this in production today
    ...
    """
    from sage_mcp import format_miner
...
def test_pyproject_extract_extra_pulls_markitdown_format_subdeps():
    """The ``sage[extract]`` extra must include MarkItDown's per-format
    sub-extras (``pdf``, ``docx``, ``pptx``, ``xlsx``) — without them, real
    PDF/DOCX/etc files hit ``MissingDependencyException`` at runtime even
    after installing the ``[extract]`` extra from the repo root. Per code review.
...
def test_mine_formats_passes_extract_mode_format_to_file_already_mined(monkeypatch, tmp_path: Path):
    ...
    with (
        ...
        mine_formats(format_dir=str(tmp), nook_path=str(tmp / "nook"), wing="wing_test")
...
def test_mine_formats_catches_unexpected_exception_and_prints_summary(
    ...
    with (
        patch("sage_mcp.format_miner.scan_formats", side_effect=angry_enumerate),
        patch("sage_mcp.format_miner.load_config", return_value=fake_config),
    ):
...
def test_mine_formats_threads_chunk_size_from_user_config(monkeypatch, tmp_path: Path):
    ...
    ``min_chunk_size``) from SageConfig through to ``chunk_text``.
    ...
    # Inject custom config values via a fake SageConfig.
    class _FakeSageConfig:
        chunk_size = 1234
        chunk_overlap = 56
        min_chunk_size = 78
        ...
        @property
        def nook_path(self):
            return str(tmp_path / "nook")

    monkeypatch.setattr(format_miner, "SageConfig", _FakeSageConfig)

    with (
        patch("sage_mcp.format_miner.scan_formats", return_value=[f]),
        patch("sage_mcp.format_miner._extract_via_markitdown", return_value="long " * 200),
        patch("sage_mcp.format_miner.load_config", return_value=fake_config),
        patch("sage_mcp.format_miner.detect_room", return_value="documents"),
        patch("sage_mcp.format_miner.file_already_mined", return_value=False),
    ):
        mine_formats(format_dir=str(tmp), nook_path=str(tmp / "nook"), wing="wing_test")
    ...
    assert call.get("chunk_size") == 1234, (
        f"chunk_text called without user's chunk_size from SageConfig "
