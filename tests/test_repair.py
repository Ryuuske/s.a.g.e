"""Tests for sage_mcp.repair — scan, prune, and rebuild HNSW index."""
...
from sage_mcp import repair


# ── _get_nook_path ──────────────────────────────────────────────────


@patch("sage_mcp.repair.SageConfig", create=True)
def test_get_nook_path_from_config(mock_config_cls):
    mock_config_cls.return_value.nook_path = "/configured/nook"
    with patch.dict("sys.modules", {}):
        # Force reimport to pick up the mock
        result = repair._get_nook_path()
    assert isinstance(result, str)


def test_get_nook_path_fallback():
    with patch("sage_mcp.repair._get_nook_path") as mock_get:
        mock_get.return_value = os.path.join(os.path.expanduser("~"), ".sage", "nook")
        result = mock_get()
        assert ".sage" in result


def test_get_collection_name_from_config():
    from sage_mcp.config import get_configured_collection_name

    get_configured_collection_name.cache_clear()
    with patch("sage_mcp.config.SageConfig") as mock_config_cls:
...
@patch("sage_mcp.repair.ChromaBackend")
def test_scan_nook_no_ids(mock_backend_cls, tmp_path):
    mock_col = MagicMock()
    mock_col.count.return_value = 0
    mock_col.get.return_value = {"ids": []}
    _install_mock_backend(mock_backend_cls, mock_col)

    good, bad = repair.scan_nook(nook_path=str(tmp_path))
    assert good == set()
    assert bad == set()


@patch("sage_mcp.repair.ChromaBackend")
def test_scan_nook_all_good(mock_backend_cls, tmp_path):
    mock_col = MagicMock()
...
@patch("sage_mcp.repair.ChromaBackend")
def test_scan_nook_with_bad_ids(mock_backend_cls, tmp_path):
    mock_col = MagicMock()
    mock_col.count.return_value = 2
...
@patch("sage_mcp.repair.ChromaBackend")
def test_scan_nook_with_wing_filter(mock_backend_cls, tmp_path):
    mock_col = MagicMock()
    ...
    repair.scan_nook(nook_path=str(tmp_path), only_wing="test_wing")
...
def test_prune_corrupt_no_file(mock_backend_cls, tmp_path):
    ...
    repair.prune_corrupt(nook_path=str(tmp_path))
...
def test_prune_corrupt_dry_run(mock_backend_cls, tmp_path):
    ...
    repair.prune_corrupt(nook_path=str(tmp_path), confirm=False)
...
def test_prune_corrupt_confirmed(mock_backend_cls, tmp_path):
    ...
    repair.prune_corrupt(nook_path=str(tmp_path), confirm=True)
...
@patch("sage_mcp.repair.ChromaBackend")
def test_rebuild_index_no_nook(mock_backend_cls, tmp_path):
    nonexistent = str(tmp_path / "nope")
    repair.rebuild_index(nook_path=nonexistent)
    mock_backend_cls.assert_not_called()


@patch("sage_mcp.repair.shutil")
@patch("sage_mcp.repair.ChromaBackend")
def test_rebuild_index_empty_nook(mock_backend_cls, mock_shutil, tmp_path):
    mock_col = MagicMock()
    mock_col.count.return_value = 0
    mock_backend = _install_mock_backend(mock_backend_cls, mock_col)

    repair.rebuild_index(nook_path=str(tmp_path))
...
def test_rebuild_index_success(mock_backend_cls, mock_shutil, tmp_path):
    ...
    assert mock_backend.create_collection.call_args_list == [
        call(str(tmp_path), "nook_drawers__repair_tmp"),
        call(str(tmp_path), "nook_drawers"),
    ]
...
def test_rebuild_index_ignores_missing_temp_collection_at_start(
    ...
    mock_backend.delete_collection.side_effect = [
        ValueError("Collection [nook_drawers__repair_tmp] does not exist"),
        None,
        None,
    ]
...
def test_delete_collection_if_exists_reraises_unexpected_value_error():
    ...
    with pytest.raises(ValueError, match="invalid collection name"):
        repair._delete_collection_if_exists(mock_backend, "/nook", "bad/name")
...
def test_check_extraction_safety_passes_when_counts_match(tmp_path):
    ...
    with patch("sage_mcp.repair.sqlite_drawer_count", return_value=500):
        repair.check_extraction_safety(str(tmp_path), 500)


def test_check_extraction_safety_uses_configured_collection(tmp_path):
    with patch("sage_mcp.repair.sqlite_drawer_count", return_value=500) as count:
...
def test_check_extraction_safety_default_uses_configured_collection(tmp_path):
    with (
        patch("sage_mcp.repair._drawers_collection_name", return_value="custom_drawers"),
        patch("sage_mcp.repair.sqlite_drawer_count", return_value=500) as count,
    ):
        repair.check_extraction_safety(str(tmp_path), 500)
    count.assert_called_once_with(str(tmp_path), "custom_drawers")
...
def test_check_extraction_safety_passes_when_sqlite_unreadable_and_under_cap(tmp_path):
    ...
    with patch("sage_mcp.repair.sqlite_drawer_count", return_value=None):
...
def test_check_extraction_safety_aborts_when_sqlite_higher(tmp_path):
    ...
    with patch("sage_mcp.repair.sqlite_drawer_count", return_value=67_580):
        try:
...
def test_check_extraction_safety_override_skips_check(tmp_path):
    ...
    with patch("sage_mcp.repair.sqlite_drawer_count", return_value=99_999):
...
def test_sqlite_drawer_count_returns_none_on_missing_file(tmp_path):
    """Nook dir exists but no chroma.sqlite3 → None, not crash."""
...
def test_rebuild_index_default_uses_configured_collection(mock_backend_cls, mock_shutil, tmp_path):
    ...
    with (
        patch("sage_mcp.repair._drawers_collection_name", return_value="custom_drawers"),
        patch("sage_mcp.repair.sqlite_drawer_count", return_value=2) as count,
    ):
        repair.rebuild_index(nook_path=str(tmp_path))
...
def test_status_returns_uninitialized_when_db_missing(tmp_path, capsys):
    """repair.status on a nook dir without chroma.sqlite3 returns a
    structured status (no chromadb client opened, per the design that
    repair-status must work even on corrupted nooks — #1498)."""
    # tmp_path exists, no chroma.sqlite3
    result = repair.status(nook_path=str(tmp_path))
...
def test_status_returns_empty_when_db_present_no_drawers(tmp_path, capsys):
    """repair.status on a nook with chroma.sqlite3 but zero drawer rows
    ...
    with patch("sage_mcp.repair.sqlite_drawer_count", return_value=0):
        result = repair.status(nook_path=str(tmp_path))
...
def test_status_empty_nook_never_opens_chromadb_client(tmp_path):
    """Design invariant from #1498: repair.status on an initialized-but-empty
    nook must NOT open a chromadb client. Opening would materialize HNSW
    segment state files on disk, breaking the promise that repair-status is
    safe to run on corrupted nooks.
    ...
    # appear here.
    assert before == after, f"repair.status mutated nook on disk: before={before} after={after}"
...
def test_status_falls_through_to_capacity_when_sqlite_count_unreadable(tmp_path):
    ...
    (tmp_path / "chroma.sqlite3").touch()
    with (
        patch("sage_mcp.repair.sqlite_drawer_count", return_value=None),
        patch("sage_mcp.repair.hnsw_capacity_status") as capacity_status,
    ):
...
def test_status_default_uses_configured_drawer_collection(tmp_path):
    ...
    # and sqlite_drawer_count returns a positive number (nook not empty).
    (tmp_path / "chroma.sqlite3").touch()
    with (
        patch("sage_mcp.repair._drawers_collection_name", return_value="custom_drawers"),
        patch("sage_mcp.repair.sqlite_drawer_count", return_value=1),
        patch("sage_mcp.repair.hnsw_capacity_status") as capacity_status,
    ):
        capacity_status.side_effect = [
            {
        ...
        repair.status(nook_path=str(tmp_path))

    assert capacity_status.call_args_list[0].args == (str(tmp_path), "custom_drawers")
    assert capacity_status.call_args_list[1].args == (str(tmp_path), "nook_closets")
...
def test_rebuild_index_proceeds_with_override(mock_backend_cls, mock_shutil, tmp_path):
    ...
    with patch("sage_mcp.repair.sqlite_drawer_count", return_value=67_580):
        repair.rebuild_index(nook_path=str(tmp_path), confirm_truncation_ok=True)
...
def test_rebuild_index_live_failure_restores_backup(mock_backend_cls, mock_shutil, tmp_path):
    ...
    active_backend.close_nook.assert_called_once_with(str(tmp_path))
    helper_backend.close_nook.assert_not_called()
...
def test_rebuild_collection_via_temp_keeps_original_error_when_cleanup_fails(
    ...
    assert excinfo.value.live_replaced is True
    assert mock_backend.delete_collection.call_args_list == [
        call("/nook", "nook_drawers__repair_tmp"),
        call("/nook", "nook_drawers"),
        call("/nook", "nook_drawers__repair_tmp"),
    ]
...
def _seed_poisoned_max_seq_id(
    ...
    *,
    ...
    """Build a minimal nook with poisoned max_seq_id rows.
...
def test_max_seq_id_detects_poison_rows(tmp_path):
    ...
    seg = _seed_poisoned_max_seq_id(nook)
    db_path = os.path.join(nook, "chroma.sqlite3")
...
def test_max_seq_id_heuristic_uses_collection_max(tmp_path):
    ...
    result = repair.repair_max_seq_id(nook, dry_run=True)
...
def test_max_seq_id_from_sidecar_exact_restore(tmp_path):
    ...
    result = repair.repair_max_seq_id(nook, from_sidecar=sidecar_path, assume_yes=True)
...
def test_max_seq_id_dry_run_no_mutation(tmp_path):
    ...
    assert before == after
    # Nothing dropped into the nook dir either (no backup on dry-run).
    assert not any(fn.startswith("chroma.sqlite3.max-seq-id-backup-") for fn in os.listdir(nook))
...
def test_max_seq_id_segment_filter(tmp_path):
    ...
    result = repair.repair_max_seq_id(nook, segment=seg["drawers_meta"], assume_yes=True)
...
def test_max_seq_id_heuristic_decodes_blob_embeddings_seq_id(tmp_path):
    """`embeddings.seq_id` rows can be BLOB-typed on nooks where chromadb
...
def test_max_seq_id_no_poison_is_noop(tmp_path):
    ...
    result = repair.repair_max_seq_id(nook, assume_yes=True)
    assert result["segment_repaired"] == []
...
def test_max_seq_id_rollback_on_verification_failure(tmp_path, monkeypatch):
    ...
    nook = str(tmp_path / "nook")
    _seed_poisoned_max_seq_id(nook)
    ...
    with pytest.raises(repair.MaxSeqIdVerificationError):
        repair.repair_max_seq_id(nook, assume_yes=True)

    # A backup file is still present — caller can roll back from it.
    leftover = [fn for fn in os.listdir(nook) if "max-seq-id-backup-" in fn]
    assert leftover
...
def test_sqlite_integrity_errors_returns_empty_for_healthy_db(tmp_path):
    ...
    assert repair.sqlite_integrity_errors(str(nook)) == []
...
def test_sqlite_integrity_errors_reports_unreadable_sqlite_file(tmp_path):
    ...
    errors = repair.sqlite_integrity_errors(str(nook))
...
def test_rebuild_index_aborts_on_sqlite_integrity_errors_before_delete_collection(
    ...
    with patch(
        "sage_mcp.repair.sqlite_integrity_errors",
        return_value=[
...
def test_rebuild_index_runs_sqlite_preflight_before_chromadb_open(tmp_path, capsys):
    ...
    # Build a real chromadb nook with one drawer so chroma.sqlite3 exists
    ...
    try:
        col = backend.create_collection(str(nook), "nook_drawers")
        col.upsert(
            ids=["d1"],
            documents=["doc"],
    ...
    sqlite_path = nook / "chroma.sqlite3"
    ...
    repair.rebuild_index(nook_path=str(nook))
...
def test_max_seq_id_preflight_preserves_embeddings_queue(tmp_path):
    ...
    with sqlite3.connect(db_path) as conn:
        conn.executemany(
            ...
            [
                (seq_id, "persistent://default/default/nook_drawers", f"queued-{seq_id}")
    ...
    # The old the original rebuild path can discard queued writes. The preflight
...
def test_rebuild_index_repairs_poisoned_max_seq_id_before_collection_rebuild(tmp_path, capsys):
    """A poisoned bookmark should short-circuit before the older rebuild path."""

    nook = str(tmp_path / "nook")
    _seed_poisoned_max_seq_id(nook)

    with patch("sage_mcp.repair.ChromaBackend") as mock_backend:
        repair.rebuild_index(nook)
    ...
    # rebuild_index must not continue into the older Chroma collection read /
...
#
# These tests build real chromadb nooks in tmp_path rather than mocking
...
def _seed_nook(nook_path, collection_name, rows):
    """Build a real chromadb nook at ``nook_path`` and add ``rows``.
    ...
    backend = ChromaBackend()
    try:
        col = backend.create_collection(str(nook_path), collection_name)
        col.upsert(
...
def test_extract_via_sqlite_returns_all_rows_with_metadata(tmp_path):
    """Round-trip: a chromadb nook with N upserted rows returns those
    ...
    _seed_nook(tmp_path, "nook_drawers", rows)

    extracted = list(repair.extract_via_sqlite(str(tmp_path), "nook_drawers"))
    ...
    try:
        scopes = {
            ...
            for (scope,) in conn.execute(
                ...
                ("nook_drawers", *(emb_id for emb_id, _, _ in extracted)),
            )
        }
    finally:
        conn.close()
...
def test_extract_via_sqlite_unknown_collection_yields_nothing(tmp_path):
    """Asking for a collection that isn't in the nook must return an
    ...
    """
    _seed_nook(tmp_path, "nook_drawers", [("d1", "doc", {"wing": "w"})])
    _seed_nook(tmp_path, "nook_closets", [("c1", "abbrev", {"wing": "w"})])
    assert list(repair.extract_via_sqlite(str(tmp_path), "not_a_real_collection")) == []


def test_extract_via_sqlite_missing_nook_yields_nothing(tmp_path):
    """No chroma.sqlite3 → empty iterator, no exception. Callers depend
    on this when probing speculatively."""
    empty = tmp_path / "no_nook_here"
    empty.mkdir()
    assert list(repair.extract_via_sqlite(str(empty), "nook_drawers")) == []


def test_rebuild_from_sqlite_roundtrips_via_real_chromadb(tmp_path):
    """End-to-end: seed source nook, rebuild into a fresh dest, then
    ...
    _seed_nook(source, "nook_drawers", rows)
    _seed_nook(
        source,
        "nook_closets",
    ...
    assert counts == {"nook_drawers": 40, "nook_closets": 1}

    backend = ChromaBackend()
    drawers = backend.get_collection(str(dest), "nook_drawers")
    ...
    # Closets: the topic/summary index layer. Re-embedded with the same EF so a
    # known closet ID and its document body must come back intact.
    closets = backend.get_collection(str(dest), "nook_closets")
...
def test_rebuild_from_sqlite_refuses_existing_dest(tmp_path):
    ...
    source = tmp_path / "source"
    dest = tmp_path / "dest"
    _seed_nook(source, "nook_drawers", [("d1", "doc", {"wing": "w"})])
    dest.mkdir()
...
def test_rebuild_from_sqlite_in_place_archives_when_opted_in(tmp_path):
    ...
    nook, AND the archive itself must still contain the original rows.
    ...
    _seed_nook(nook, "nook_drawers", rows)

    counts = repair.rebuild_from_sqlite(str(nook), str(nook), archive_existing_dest=True)
    assert counts["nook_drawers"] == 15

    archives = [p for p in tmp_path.iterdir() if p.name.startswith("nook.pre-rebuild-")]
    ...
    archived_rows = list(repair.extract_via_sqlite(str(archives[0]), "nook_drawers"))
    assert len(archived_rows) == 15

    from sage_mcp.backends.chroma import ChromaBackend

    rebuilt = ChromaBackend().get_collection(str(nook), "nook_drawers")
...
def test_rebuild_from_sqlite_in_place_refuses_without_archive_flag(tmp_path):
    ...
    nook = tmp_path / "nook"
    _seed_nook(nook, "nook_drawers", [("d1", "doc", {"wing": "w"})])
    sqlite_before = (nook / "chroma.sqlite3").stat().st_size

    counts = repair.rebuild_from_sqlite(str(nook), str(nook))
    assert counts == {}
    # Same file, untouched.
    assert (nook / "chroma.sqlite3").stat().st_size == sqlite_before
...
def test_rebuild_from_sqlite_source_missing_chroma_db(tmp_path):
    ...
    source = tmp_path / "source"
    source.mkdir()
    (source / "stray_file").write_text("not a nook")
    dest = tmp_path / "dest"
...
def test_rebuild_from_sqlite_in_place_validates_source_before_archiving(tmp_path):
    ...
    nook = tmp_path / "nook"
    nook.mkdir()
    (nook / "marker.txt").write_text("not a real nook")

    counts = repair.rebuild_from_sqlite(str(nook), str(nook), archive_existing_dest=True)
    assert counts == {}
    # No archive created — original dir still in place with its marker.
    assert nook.exists()
    assert (nook / "marker.txt").read_text() == "not a real nook"
    archives = [p for p in tmp_path.iterdir() if "pre-rebuild" in p.name]
    assert archives == []
...
def test_rebuild_from_sqlite_raises_on_upsert_failure(tmp_path, monkeypatch):
    ...
    # sage's backend returns) keeps the failure path realistic.
    ...
    with pytest.raises(repair.RebuildPartialError) as excinfo:
        repair.rebuild_from_sqlite(str(nook), str(nook), archive_existing_dest=True)

    err = excinfo.value
    assert err.failed_collection == "nook_drawers"
    assert err.partial_counts.get("nook_drawers") == 0
    assert err.archive_path is not None
    assert os.path.isfile(os.path.join(err.archive_path, "chroma.sqlite3"))
    assert err.dest_nook == os.path.abspath(str(nook))
...
def test_rebuild_from_sqlite_honors_configured_drawer_collection_name(tmp_path, monkeypatch):
    ...
    ``SageConfig().collection_name``) must have THAT collection
    rebuilt — not the hardcoded ``nook_drawers``.

    Catches: a regression where the recovery path silently rebuilds the
    default-name collection on a custom-named nook, leaving the user's
    ...
    fixed (``nook_closets``) by design — the closet index references
    ...
    _seed_nook(source, custom_drawers, drawer_rows)
    _seed_nook(source, "nook_closets", closet_rows)

    counts = repair.rebuild_from_sqlite(str(source), str(dest))

    # Rebuilt under the custom name, not under the default "nook_drawers".
    assert counts == {custom_drawers: 3, "nook_closets": 1}
    ...
    try:
        rebuilt_default = backend.get_collection(str(dest), "nook_drawers")
        ...
        assert rebuilt_default.count() == 0, (
            ...
            "custom-name nook — recovery wrote to the wrong collection."
        )
    except Exception:
