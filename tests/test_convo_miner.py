...
from sage_mcp.convo_miner import (
    _is_ai_tool_path,
    _resolve_wing,
    mine_convos,
)
from sage_mcp.nook import MineAlreadyRunning, file_already_mined
...
def test_convo_mining():
    ...
    mine_convos(tmpdir, nook_path, wing="test_convos")
...
def test_mine_convos_does_not_reprocess_short_files(capsys):
    ...
    try:
        ...
        mine_convos(tmpdir, nook_path, wing="test")
...
def test_mine_convos_allows_general_after_exchange(capsys):
    ...
    try:
        ...
        mine_convos(tmpdir, nook_path, wing="test", extract_mode="exchange")
        capsys.readouterr()
        mine_convos(tmpdir, nook_path, wing="test", extract_mode="general")
...
def test_mine_convos_rebuilds_stale_drawers_after_schema_bump(capsys):
    ...
    users just run `sage mine` again and old noise-filled drawers get
    replaced with clean ones."""
    from sage_mcp.nook import NORMALIZE_VERSION
...
def _hold_nook_lock_in_child(nook_path, ready_flag, release_flag):
    """Acquire mine_nook_lock in a child process and hold until signalled.

    Cannot use threads because mine_nook_lock is intentionally re-entrant
    ...
    from sage_mcp.nook import mine_nook_lock as _mpl
...
def test_mine_convos_refuses_concurrent_run_against_same_nook(tmp_path, monkeypatch):
    """A second `mine_convos` against a nook currently being mined must
    ...
    (see test_nook_locks.py) for the convos code path.
    """
    ...
    holder = ctx.Process(
        target=_hold_nook_lock_in_child,
        args=(nook_path, ready_flag, release_flag),
    )
    holder.start()
    try:
        ...
        for _ in range(500):
            if os.path.exists(ready_flag):
                break
            time.sleep(0.01)
        assert os.path.exists(ready_flag), "child never acquired nook lock"

        with pytest.raises(MineAlreadyRunning):
            mine_convos(str(convo_dir), nook_path, wing="test")
    finally:
...
def test_mine_convos_dry_run_bypasses_nook_lock(tmp_path, monkeypatch):
    """Dry-run never writes to the nook, so it must coexist with a live
    mine instead of being blocked by the per-nook flock.
    """
    import multiprocessing
    import time
    ...
    try:
        ...
        mine_convos(str(convo_dir), nook_path, wing="test", dry_run=True)
    finally:
        open(release_flag, "w").close()
        holder.join(timeout=10)
        if holder.is_alive():
...
#
# When a user runs `sage mine --mode convos` against a directory
