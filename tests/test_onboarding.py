"""Tests for sage.onboarding."""

import os
from unittest.mock import patch

from sage_mcp.onboarding import (
    DEFAULT_WINGS,
    _ask,
...
def test_quick_setup_creates_registry(tmp_path):
    ...
    assert "sage" in registry.projects
...
def test_auto_detect_filters_known(tmp_path):
    ...
    with (
        patch("sage_mcp.onboarding.scan_for_detection", return_value=["file.txt"]),
        patch("sage_mcp.onboarding.detect_entities", return_value=fake_detected),
    ):
...
def test_auto_detect_handles_exception(tmp_path):
    with patch("sage_mcp.onboarding.scan_for_detection", side_effect=Exception("boom")):
        result = _auto_detect(str(tmp_path), [])
    assert result == []
...
def test_run_onboarding_basic_flow(tmp_path):
    ...
    with (
        patch("sage_mcp.onboarding._ask_mode", return_value="work"),
        patch("sage_mcp.onboarding._ask_embedding_model", return_value="embeddinggemma"),
        patch(
            "sage_mcp.onboarding._ask_people",
            return_value=([{"name": "Bob", "relationship": "boss", "context": "work"}], {}),
        ),
        patch("sage_mcp.onboarding._ask_projects", return_value=["Acme"]),
        patch("sage_mcp.onboarding._ask_wings", return_value=["projects", "team"]),
        patch("sage_mcp.onboarding._yn", return_value=False),
        patch("sage_mcp.onboarding._warn_ambiguous", return_value=[]),
    ):
...
def test_run_onboarding_with_ambiguous_names(tmp_path):
    ...
    with (
        patch("sage_mcp.onboarding._ask_mode", return_value="personal"),
        patch("sage_mcp.onboarding._ask_embedding_model", return_value="embeddinggemma"),
        patch(
            "sage_mcp.onboarding._ask_people",
            return_value=([{"name": "Grace", "relationship": "friend", "context": "personal"}], {}),
        ),
        patch("sage_mcp.onboarding._ask_projects", return_value=[]),
        patch("sage_mcp.onboarding._ask_wings", return_value=["family"]),
        patch("sage_mcp.onboarding._yn", return_value=False),
    ):
        registry = run_onboarding(directory=".", config_dir=tmp_path, auto_detect=False)
...
def test_run_onboarding_persists_multilingual_choice(tmp_path):
    ...
    with (
        ...
        patch("sage_mcp.onboarding._ask_projects", return_value=[]),
        patch("sage_mcp.onboarding._ask_wings", return_value=["projects"]),
        patch("sage_mcp.onboarding._yn", return_value=False),
        patch("sage_mcp.onboarding._warn_ambiguous", return_value=[]),
    ):
    ...
    assert SageConfig(config_dir=tmp_path).embedding_model == "embeddinggemma"
...
def test_run_onboarding_persists_minilm_choice(tmp_path):
    ...
    with (
        patch("sage_mcp.onboarding._ask_mode", return_value="work"),
        patch("sage_mcp.onboarding._ask_embedding_model", return_value="minilm"),
        patch("sage_mcp.onboarding._ask_people", return_value=([], {})),
        patch("sage_mcp.onboarding._ask_projects", return_value=[]),
        patch("sage_mcp.onboarding._ask_wings", return_value=["projects"]),
        patch("sage_mcp.onboarding._yn", return_value=False),
        patch("sage_mcp.onboarding._warn_ambiguous", return_value=[]),
...
# ── SageConfig.set_embedding_model ────────────────────────────────


def test_set_embedding_model_persists_and_reloads(tmp_path):
    """Setter writes to config.json and a fresh SageConfig reads it back."""
    from sage_mcp.config import SageConfig

    SageConfig(config_dir=tmp_path).set_embedding_model("embeddinggemma")
    assert SageConfig(config_dir=tmp_path).embedding_model == "embeddinggemma"

    SageConfig(config_dir=tmp_path).set_embedding_model("MiniLM")
    assert SageConfig(config_dir=tmp_path).embedding_model == "minilm"
