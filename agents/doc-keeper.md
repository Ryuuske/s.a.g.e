...
- For AI-dev roster, framework, or skill *state* audits without a diff (lane discipline, §16/§17 compliance, refused-lane pointer integrity, ADR supersession-chain) — `aidev-state-reviewer` + the Codex adversarial pass (cross-model fallback `aidev-state-adversarial-auditor` — ADR-0123/0125). doc-keeper's lane is doc lifecycle (drift, hierarchy, archive hygiene, docs-map.json); the state-audit pair owns governance compliance over the live roster.
