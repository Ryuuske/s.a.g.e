...
3. **Validate auditors against the catalog.** For each of `auditor_primary`, `auditor_secondary`, `auditor_tertiary`:
   ...
   - **Codex-pass carve-out:** a `/codex:*` slash-command secondary (the Codex adversarial-review lane, ADR-0123) is NOT a catalog agent. Recognize a `/codex:adversarial-review` entry as a valid Codex pass and SKIP the `~/.claude/agent-catalog.json` check for it; do NOT set `drift_warning` for a `/codex:*` entry. Codex-pass readiness is validated by `codex-budget` / `codex:setup`, not by the agent catalog.
...
auditor_secondary: /codex:adversarial-review (fallback aidev-adversarial-auditor — ADR-0123)
