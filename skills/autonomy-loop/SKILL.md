...
6. **Codex.** Run the `codex-budget` skill, then `/codex:adversarial-review` on the phase diff. Fold any needs-attention findings; re-run codex to confirm (pass-N until APPROVE or the arbiter resolves a genuine split).
