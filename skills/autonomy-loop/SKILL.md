...
6. **§16 adversarial secondary.** The per-phase Codex `/codex:adversarial-review` dispatched in step 4 (budget-gated there) is the §16 adversarial-auditor secondary, not a second distinct pass. Fold any needs-attention findings; re-run the selected adversarial model only to confirm folded findings (the Claude `aidev-adversarial-auditor` when Codex implemented this phase, per ADR-0125) (pass-N until APPROVE or the arbiter resolves a genuine split).
