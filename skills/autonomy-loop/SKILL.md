...
**T1 — Whole-branch review.** Dispatch the aidev auditor pair (`aidev-code-reviewer` + `aidev-adversarial-auditor`) on the full `git diff main...HEAD` output — a cross-phase integration check at the contract level, not a per-phase re-audit. Apply contract-tracing and mirror/symmetry check angles (see `~/.claude/docs/specs/universal-agent-constraints.md` Universal Agent Constraints REVIEWER_DISCIPLINE — Contract-tracing across paths and Mirror/symmetry check). Findings ≥80 are blocking; fold and re-audit (≤6 fold-rounds cap applies). Emit a T1 STATUS card when T1 clears.
