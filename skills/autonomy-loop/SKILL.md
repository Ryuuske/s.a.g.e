...
6. **Codex budget / §16 secondary.** Run the `codex-budget` skill before the step-4 Codex `/codex:adversarial-review`; that per-phase Codex pass is the §16 adversarial-auditor secondary, not a second distinct pass. Fold any needs-attention findings; re-run Codex only to confirm folded findings (pass-N until APPROVE or the arbiter resolves a genuine split).
...
**T1 — Whole-branch review.** Dispatch the aidev auditor pair (`aidev-code-reviewer` + the Codex adversarial pass (`/codex:adversarial-review`; fallback `aidev-adversarial-auditor` when Codex is unavailable — ADR-0123)) on the full `git diff main...HEAD` output — a cross-phase integration check at the contract level, not a per-phase re-audit. Apply contract-tracing and mirror/symmetry check angles (see `~/.claude/docs/specs/universal-agent-constraints.md` Universal Agent Constraints REVIEWER_DISCIPLINE — Contract-tracing across paths and Mirror/symmetry check). Findings ≥80 are blocking; fold and re-audit (≤6 fold-rounds cap applies). Emit a T1 STATUS card when T1 clears.
