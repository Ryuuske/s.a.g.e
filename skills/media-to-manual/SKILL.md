...
Job packages live at `~/dev/media-jobs/<slug>/`. They are local-only and must never be committed or pushed.
...
All job packages are at `~/dev/media-jobs/<slug>/`, outside the framework repo. The framework repo never contains media files, audio, frames, or job-package artifacts. Nothing under `~/dev/media-jobs/` is ever pushed (this path is outside the GitHub publication gate at `~/dev/github/Ryuuske/`).
...
- Any work that does not involve a `~/dev/media-jobs/<slug>/` package or the scripts/media/ pipeline.
