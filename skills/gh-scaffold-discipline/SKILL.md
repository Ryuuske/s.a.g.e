...
- **ADR-0031** (`.development/decisions/0031-gh-repo-scaffolder-identifying-info-exemption.md`) — case-a identifying-info ban exemption for `gh-repo-scaffolder`. Cited by number for identity-intrinsic justification only. This ADR is explicitly NOT the authority for tool grants — tool grants flow from this skill's tool-guidance sections above.
- **ADR-0027** (`.development/decisions/0027-third-party-doc-reference-pause-to-user-pattern.md`) — verbatim PAUSE shape for WebFetch failure and third-party documentation reference lookups. The `[scheduled-annotation: agent pending future session per agent-roster.md step 13]` wrapper is mandatory verbatim at every PAUSE emission site.
- **ADR-0025** (`.development/decisions/0025-adr-0031-corrected-supersession-honors-section-8-append-only.md`) — cited only as a refused-lane discriminator: CHANGELOG-paired version-bump discipline is out of scope for this skill and for `gh-repo-scaffolder`. Release tagging and version bumps route to `gh-release-manager`.
