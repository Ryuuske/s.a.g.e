...
2. **Third opinion.** If both positions are defensible — a real trade-off — consult a third relevant agent for an independent take — invoke `/codex:adversarial-review` ONLY if Codex was not already an auditor on this change; when Codex was the §16 adversarial secondary, the third opinion MUST be a different, non-Codex agent to stay independent (ADR-0125). Document the synthesis.
...
Every committed change passes through **two auditors running in parallel** (per §5). **Pair selection is delegated to the `audit-pairing-lookup` skill, which reads the single source of truth at `docs/specs/audit-pairing-matrix.md`:** AI-dev diffs (changes in `agents/`, `skills/`, `hooks/scripts/`, `statusline/`, `tests/`, `claude-md/`, `.development/decisions/`, `docs/specs/`, `.development/agents/`, or installer scripts) → `aidev-code-reviewer` + a **Codex adversarial pass** (`/codex:adversarial-review`); AI-dev state audits with no diff → `aidev-state-reviewer` + a Codex adversarial pass (optional `doc-keeper` third lane on doc-lifecycle); general code → `dev-code-reviewer` + the UI (`dev-ux-designer`) / security (`sec-auditor`) / test (`dev-test-engineer`) peer the matrix names. **The adversarial lane is cross-model to the implementer (ADR-0123, amended by ADR-0125) — a different model family gives the contrarian read.** Default = Codex `/codex:adversarial-review` (Claude is the usual implementer). When Codex implemented the change under audit — or the implementer is unknown/mixed — the adversarial pass uses the Claude `aidev-adversarial-auditor` (diff rows) / `aidev-state-adversarial-auditor` (state & propagation-batch rows); fail-safe to the Claude auditor on unknown/mixed, never assume Codex. The orchestrator wraps Codex output into a conformant `@@VERDICT` (fail-closed — a Codex error, timeout, or unparseable output is treated as Codex-unavailable → Claude fallback, never APPROVE; budget-refusal also falls back, never skips the lane). Mixed change+state audits decompose-and-sequence per ADR-0015. The invariants below stay resident:
...
- **State-audit three-way voting:** when `doc-keeper` is the third lane, all three must clear for APPROVE; any single blocking finding (≥80) triggers §6, with a non-Codex third-opinion step in lieu of a fourth roster lane (use `/codex:adversarial-review` only when Codex was not already an auditor on this change — ADR-0125). See ADR-0014 (state-audit fork) and ADR-0016 (three-way voting).
