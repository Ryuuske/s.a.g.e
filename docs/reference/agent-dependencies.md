...
| `media-manual-author` | pandoc | `system` | apt-get install pandoc (or brew install pandoc); wkhtmltopdf for PDF rendering | renders the composed markdown to pdf or docx; md output needs no render tool but pdf/docx steps call pandoc directly |
...
| `media-transcriber` | ~/.venvs/media | `venv` | scripts/media/setup.sh | whisper, openai-whisper, and all transcription/packaging deps live in this venv; doctor.py reports missing venv if absent |
