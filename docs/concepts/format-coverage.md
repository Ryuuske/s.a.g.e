<!--
scope-owned: mining format support + skip codes
audience: users
source: hand
review-trigger: new format / miner change
-->

# Format coverage — `sage mine --mode extract`

**Shipped in S.A.G.E. 0.1.0.**
**Module:** `sage format_miner.py`
**CLI entry:** `sage cli.py` — `cmd_mine` dispatcher routes `--mode extract` here.
...
```
sage mine <dir>                  → miner.py        (project files: code, docs, notes)
sage mine <dir> --mode convos    → convo_miner.py  (chat exports)
sage mine <dir> --mode extract   → format_miner.py (binary office documents)   ← NEW
```
...
Same architectural principle as virtual line numbering: **transform at read time, leave storage untouched.**
...
1. **Matches the existing miner-per-content-type pattern.** S.A.G.E. already has `miner.py` and `convo_miner.py`. A third miner is a third instance of the same shape — immediately recognizable to anyone reading the codebase.
...
## Fringe cases handled — 14 (original 12 + striprtf parity + SKIP_UNREADABLE)

Each case maps to a specific `ExtractionStatus` enum value so callers (and reviewers) can audit exactly which path fired. All fourteen are covered by dedicated tests in `tests/test_format_miner.py`.
...
Fourteen documented values (13 named in the table above plus `SKIP_UNREADABLE`). Each is paired with a string value like `"skip:no_markitdown"` for log readability; tests assert on `.name` for stability.
...
One-line addition to `sage cli.py` in `cmd_mine`:
...
Live extraction was verified end-to-end on 2026-05-19 (Python 3.13, MarkItDown 0.1.5 + striprtf 0.0.27) against a mixed real-world corpus: every supported format extracted or skipped with the documented skip-code, no source file modified.

## Backwards compatibility

- **No changes to existing miners.** `miner.py` and `convo_miner.py` are not touched.
- **No new required dependencies.** MarkItDown and striprtf are optional extras (`pip install markitdown striprtf`, or the `[extract]` extra from the repo root: `uv pip install -e ".[extract]"`). MarkItDown requires Python ≥ 3.10 and striprtf 3.6+ — both satisfied at S.A.G.E.'s 3.12 floor:
  ```toml
...
- **No migration step.** Users who already have a nook mined from the other modes can run `sage mine --mode extract ~/docs/` and the new drawers land in the same nook, same wing, same room conventions.
- **Optional `--max-file-size` flag** can be added to `sage mine` so users with legitimate large files (e.g. scanned books) can raise the cap from the 500 MB default.
...
- **Migration importers** (Notion / Obsidian export formats). Each is a separate adapter; not bundled here.

These are real future features; they don't belong in a release whose scope is "additive format coverage via MarkItDown."
