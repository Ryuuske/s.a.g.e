<!--
scope-owned: closet concept
audience: users
source: hand
review-trigger: feature change
-->
...
```
CLOSET: "built auth system|Alice;Bob|→drawer_api_auth_a1b2c3"
...
Closets are created during `sage mine`. For each file mined:
...
### Do closets survive nook rebuilds?

Closets are stored in the `nook_closets` ChromaDB collection alongside `nook_drawers`. If you delete and rebuild the nook, closets are recreated during the next `sage mine`.

## How search uses closets

```
Query → search nook_closets (fast, small documents)
         ↓
    top closet hits → parse `→drawer_id_a,drawer_id_b` pointers
         ↓
    fetch exactly those drawers from nook_drawers (verbatim content)
         ↓
...
If no closets exist (nook created before this feature) — or all closet hits get filtered out by `max_distance` — search falls back to direct drawer search. Closets are created on next mine.
...
Closet functions live in `sage nook.py`:
...
The closet-first search path lives in `sage searcher.py`:
