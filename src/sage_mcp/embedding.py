...
Two embedding models are available, selected via ``SAGE_EMBEDDING_MODEL``
or ``embedding_model`` in ``~/.sage/config.json``:

* ``minilm`` (default) — ``all-MiniLM-L6-v2``, 384-dim, English-only training.
  ChromaDB's default; what every existing nook was built with.
* ``embeddinggemma`` — ``onnx-community/embeddinggemma-300m-ONNX`` (q8), 384-dim
  ...
  on an existing nook requires re-embedding via ``sage repair`` (mode full,
  the default) — the new model is a different vector space.

Supported devices (env ``SAGE_EMBEDDING_DEVICE`` or ``embedding_device``
in ``~/.sage/config.json``):

* ``auto`` — prefer CUDA ▸ CoreML ▸ DirectML, fall back to CPU
* ``cpu`` — force CPU (the historical default)
* ``cuda`` — NVIDIA GPU via ``onnxruntime-gpu`` (``uv pip install -e ".[gpu]"`` from the repo root)
...
def _resolve_providers(device: str) -> tuple[list, str]:
    ...
    if preferred not in available:
        if device not in _WARNED:
            extra = _DEVICE_EXTRA.get(device, "the matching sage extra for your device")
            logger.warning(
...
def _build_ef_class():
    ...
    nooks created with ``DefaultEmbeddingFunction`` *and* nooks we
    create ourselves, with the same GPU-capable ``preferred_providers``.
    """
    from chromadb.utils.embedding_functions import ONNXMiniLM_L6_V2

    class _SageONNX(ONNXMiniLM_L6_V2):
        @staticmethod
        def name() -> str:
            return "default"
...
class EmbeddinggemmaONNX:
    ...
    Switching an existing nook from minilm → embeddinggemma still requires
    re-embedding (different vector space) — collections persist the EF name
    and ChromaDB rejects mismatched reads. Run ``sage repair`` (mode full).
    """
    ...
    def _lazy_load(self) -> None:
        ...
        try:
            ...
            raise ImportError(
                ...
                "numpy — these ship with sage core, so this error usually "
                "means one was uninstalled or pinned to an incompatible version. "
                "Reinstall from the repo root with: pip install --force-reinstall -e ."
            ) from e
...
def get_embedding_function(device: Optional[str] = None, model: Optional[str] = None):
    ...
    ``device=None`` reads :attr:`SageConfig.embedding_device`;
    ``model=None`` reads :attr:`SageConfig.embedding_model`.
...
def describe_device(device: Optional[str] = None) -> str:
    ...
    if device is None:
        ...
        device = SageConfig().embedding_device
