"""
knowledge_graph.py — Temporal Entity-Relationship Graph for sage
=====================================================================
...
DEFAULT_KG_PATH = os.path.expanduser("~/.sage/knowledge_graph.sqlite3")


def read_counts_readonly(db_path: str) -> tuple[int, int]:
    """Return ``(entities, triples)`` from a KG sqlite WITHOUT constructing a
    ``KnowledgeGraph`` (which would mkdir + chmod + run ``_init_db`` / migrations /
    set WAL — i.e. MUTATE even an existing file).
    ...
    try:
        ...
        try:
            ...
            triples = con.execute("SELECT count(*) FROM triples").fetchone()[0]
            return (int(entities), int(triples))
        finally:
            con.close()
    except Exception:
        return (0, 0)
...
def _temporal_filter_sql(as_of: str) -> tuple[str, list[str]]:
    ...
    This keeps earlier date-only facts working when callers query with
...
class KnowledgeGraph:
    def __init__(self, db_path: str = None):
        ...
        self._init_db()
        # Restrict the KG db to owner-only (ADR-0077): it holds plaintext entity
        # /triple facts. The parent dir is 0700 above; chmod the file too,
        # umask-independent, mirroring config.py's 0600 discipline. (The old
        # _harden_nook_perms only covered chroma.sqlite3; the runtime KG db at
        # DEFAULT_KG_PATH was left 0644 — E2E round-C.) WAL sidecars inherit the
        # 0700 dir guard.
        try:
    ...
    def _init_db(self):
        ...
        self._ensure_active_triple_unique_index(conn)
        conn.commit()

    def _ensure_active_triple_unique_index(self, conn):
        """Create the partial UNIQUE index on currently-valid triples.

        The in-process add_triple() guards via threading.Lock + check-then-insert,
        but a second concurrent process bypasses that lock. The index makes the
        database the source of truth: at most one active (subject, predicate,
        object) triple where valid_to IS NULL.

        Existing nooks may already contain duplicates from the pre-index race
        window. Collapse them before the CREATE so the index can be built.
        """
        already = conn.execute(
            "SELECT 1 FROM sqlite_master WHERE type='index' AND name='idx_triples_active_unique'"
        ).fetchone()
        if already:
            return
    ...
    def _migrate_schema(self, conn):
        ...
        Fresh nooks get ``source_drawer_id`` / ``adapter_name``
        directly from the canonical ``CREATE TABLE`` above, so this path is a
        no-op on new installs. It exists for nooks that were created before
    ...
    def add_triple(
        ...
        ``source_drawer_id`` and ``adapter_name`` are provenance
        ...
        # raw string comparison so earlier date-only values and canonical UTC
        ...
        with self._lock:
            ...
            with conn:
                ...
                try:
                    ...
                    # WHERE valid_to IS NULL caught a concurrent insert from
                    # another process. Return the row that won the race so the
