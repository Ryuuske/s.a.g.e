...
Used by sage init before mining begins.
...
Multi-language support:
    ...
    the ``entity`` section of ``sage/i18n/<lang>.json``. Every public
...
Usage:
    from sage_mcp.entity_detector import detect_entities, confirm_entities
...
#
# The data file lives at ``sage/data/coca_content_words.json``. Loaded
...
def _get_coca_filter() -> frozenset[str]:
    ...
    Loads ``sage/data/coca_content_words.json`` on first call and
...
#
# Data file: ``sage/data/known_systems.json``. Loaded once on first
...
def _get_known_systems() -> tuple[tuple[str, "re.Pattern[str]"], ...]:
    ...
    Loads ``sage/data/known_systems.json`` on first call, compiles a
...
def _apply_known_systems_prepass(text: str) -> tuple[str, dict[str, int]]:
    ...
    ``nook.build_closet_lines`` at closet construction, and
...
def detect_entities(
    ...
    Args:
        ...
        corpus_origin: Optional corpus-origin context (the dict produced
            by ``sage.corpus_origin`` and persisted to
            ``<nook>/.sage/origin.json`` by ``sage init``).
