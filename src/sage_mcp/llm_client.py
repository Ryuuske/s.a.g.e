...
- ``ollama`` (default): local models via http://localhost:11434. Works fully
  offline. Honors sage's "zero-API required" principle.
...
class LLMProvider:
    ...
    def __init__(
        ...
        # Privacy-by-architecture: reject non-http(s) schemes so a
        # misconfigured --llm-endpoint cannot exfiltrate local files via
        # file:// (urllib honors FileHandler by default). Mirrors the same
        # guard in closet_llm.py:105-112.
        if self.endpoint:
            scheme = urlparse(self.endpoint).scheme.lower()
            if scheme not in ("http", "https"):
                raise LLMError(f"endpoint must use http:// or https:// (got scheme {scheme!r})")
    ...
    def is_external_service(self) -> bool:
        ...
        Used by ``sage init`` to decide whether to print a privacy
