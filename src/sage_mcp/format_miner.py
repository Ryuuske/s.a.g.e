"""format_miner.py — implemented in sage 0.1.0.
...
Architecture matches the existing miner-per-content-type pattern:
    ...
    sage mine <dir>                  → miner.py
    sage mine <dir> --mode convos    → convo_miner.py
    sage mine <dir> --mode extract   → format_miner.py (this file)
...
If the user runs ``sage mine --mode extract`` without it installed,
...
# sage.format_miner.<name>. Lazy imports inside functions would not
# expose these as attributes of this module, breaking the test seams.
from .config import SageConfig, normalize_wing_name
from .miner import (
...
logger = logging.getLogger("sage_format_miner")
...
def decode_robust(raw: bytes) -> str:
    ...
    the older smart-quote bytes 0x91-0x9F that surface in older Office docs).
...
def extract_text(
    ...
    try:
        ...
        # Fringe Case 14: MarkItDown raises
...
def scan_formats(directory: Union[Path, str]) -> list[Path]:
    ...
    Skips:
      - Hidden / build directories listed in ``nook.SKIP_DIRS``
      ...
      - Files matched by ``.sageignore`` in any ancestor directory
        (WI-8 defense-in-depth: secret-file patterns such as ``*.pem`` and
        ``.env`` are excluded before any content is read)
    ...
    # CLI inputs like ``sage mine --mode extract ~/docs`` work without
    ...
    for dirpath, dirnames, filenames in os.walk(root):
        ...
        # nook.SKIP_DIRS constant so this stays in sync with the other
        # miners' skip set.
        dirnames[:] = [d for d in dirnames if d not in SKIP_DIRS]

        # Load the .sageignore for this directory (mirrors miner.py's
        # load_sageignore_matcher pattern). Collect matchers from root
        # down to current_dir so ancestor rules apply to all descendants.
        nookignore_matchers = []
        try:
        ...
        for depth in range(len(rel_parts) + 1):
            ancestor = root.joinpath(*rel_parts[:depth]) if depth > 0 else root
            m = load_sageignore_matcher(ancestor, matcher_cache)
            if m is not None:
                nookignore_matchers.append(m)
        ...
        for name in filenames:
            ...
            if p.suffix.lower() not in SUPPORTED_FORMATS:
                continue
            # WI-8 defense-in-depth: skip files matched by .sageignore
            # (e.g. *.pem, .env). Mirrors scan_project's nookignore check.
            if nookignore_matchers and is_gitignored(p, nookignore_matchers, is_dir=False):
                continue
            found.append(p)
...
def _register_skip_sentinel_if_appropriate(
    ...
    if status in _TRANSIENT_MISSING_DEP_STATUSES:
        return
    _register_file(collection, source_file, wing, agent, agents=agents)
...
def _file_chunks_locked(
    ...
    with mine_lock(source_file):
        ...
        # nook.file_already_mined reads current mtime from disk and compares
        ...
        for batch_start in range(0, len(chunks), DRAWER_UPSERT_BATCH_SIZE):
            ...
            for chunk in chunks[batch_start : batch_start + DRAWER_UPSERT_BATCH_SIZE]:
                ...
                from .extensions.agent_keyed_drawers import serialize_agents as _serialize_agents
                ...
                meta: dict = {
                    ...
                    "agents": _serialize_agents(agents or []),
                }
...
def mine_formats(
    ...
    """Mine a directory of binary office-format files into the nook.
    ...
    format_dir :
        ...
        (see ``nook.SKIP_DIRS``). Symlinks are skipped (consistency with
        ``miner.py`` and ``convo_miner.py``). Only files matching
        ``SUPPORTED_FORMATS`` are processed.
    nook_path :
        Path to the ChromaDB nook (the destination of the drawers).
    wing :
    ...
    Notes
    -----
    - Loads ``SageConfig`` once at the start. The current chunker
    ...
    """
    # SageConfig + chunk_text are imported at module level (above) so
    # tests can patch ``sage.format_miner.SageConfig`` and
    # ``sage.format_miner.chunk_text`` directly. Hoisted as part of the
    # PR #1555 polish PR (Gemini #3).

    # Load nook-wide config. Chunk parameters (chunk_size, chunk_overlap,
    ...
    # Phase 4 wing-registration gate. Reject unregistered wings so a
    # typo'd or auto-inferred wrong wing doesn't silently fill the nook.
    from .extensions.wing_registry import require_registered_wing

    require_registered_wing(wing)

    # Load the project's sage.yaml (rooms list + wing override) so
    ...
    try:
        ...
        print(f"\n{'=' * 55}")
        print("  sage Mine — Format extraction")
        print(f"{'=' * 55}")
        print(f"  Wing:    {wing}")
        ...
        collection = get_collection(nook_path) if not dry_run else None
                ...
                if status != ExtractionStatus.OK or not text:
                    if not dry_run:
                        _register_skip_sentinel_if_appropriate(
                            collection, source_file, wing, agent, status, agents=agents
                        )
                    print(f"  - [{i:4}/{len(files)}] {filepath.name[:50]:50} {status.name}")
                    continue

                # Thread the user's SageConfig chunk parameters through
                # so format-mode mining honors their tuning. Per PR #1555
                # review (Gemini #3).
                chunks = chunk_text(
                    text,
                    source_file,
                    chunk_size=nook_config.chunk_size,
                    chunk_overlap=nook_config.chunk_overlap,
                    min_chunk_size=nook_config.min_chunk_size,
                )
                if not chunks:
                    if not dry_run:
                        _register_file(collection, source_file, wing, agent, agents=agents)
        ...
        if not dry_run:
            ...
            # finds malformed FTS5 rows so a corrupted nook cannot silently
