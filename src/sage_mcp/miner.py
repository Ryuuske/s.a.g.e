#!/usr/bin/env python3
"""
miner.py — Files everything into the nook.

Reads sage.yaml from the project directory to know the wing + rooms.
...
from .nook import (
    ...
    _validate_nook_fts5_after_mine,
    build_closet_lines,
    bulk_check_mined,
    file_already_mined,
...
# ``sage.miner.compute_hallways_for_wing``. The integration call
...
# Re-export the shared defaults from ``config`` so callers that
# import ``CHUNK_SIZE`` / ``CHUNK_OVERLAP`` / ``MIN_CHUNK_SIZE`` from
# ``sage.miner`` keep working unchanged. Single source of truth
# lives in ``config.DEFAULT_CHUNK_*``.
from .config import (  # noqa: E402  (kept here for the back-compat alias)
...
# this via `SAGE_MAX_CHUNKS_PER_FILE` or
# `sage mine --max-chunks-per-file N` if you hit ONNX bad_alloc on
...
def _resolve_max_chunks_per_file(override: Optional[int] = None) -> int:
    ...
    Precedence: ``override`` (CLI flag) > ``SAGE_MAX_CHUNKS_PER_FILE``
    ...
    if override is not None:
        ...
        return int(override)
    raw = os.environ.get("SAGE_MAX_CHUNKS_PER_FILE")
    if raw is None:
        return MAX_CHUNKS_PER_FILE
    try:
        val = int(raw)
        ...
        print(
            f"  ! WARNING: SAGE_MAX_CHUNKS_PER_FILE={raw!r} is not an integer; "
            f"using default {MAX_CHUNKS_PER_FILE}",
            file=sys.stderr,
        )
        return MAX_CHUNKS_PER_FILE
    if val < 0:
        print(
            f"  ! WARNING: SAGE_MAX_CHUNKS_PER_FILE={val} is negative; "
            f"using default {MAX_CHUNKS_PER_FILE}",
            file=sys.stderr,
        )
        return MAX_CHUNKS_PER_FILE
...
# GitignoreMatcher lives in the lightweight sage.ignore module (no
# chromadb / vector deps) so that ignore-consult paths that don't need the
# full miner import chain (e.g. skill_registry.py) work without chromadb.
# Re-exported here so all existing callers (load_gitignore_matcher, tests,
# convo_miner, etc.) keep working with no import-path changes.
from .ignore import GitignoreMatcher  # noqa: E402, F401  (re-export, mid-file placement)
...
def load_sageignore_matcher(dir_path: Path, cache: dict):
    """Load and cache one directory's ``.sageignore`` matcher.

    Same gitignore-style format; the .sageignore filename keeps
    the mine-ignore lane separate from gitignore so users can exclude
    things from mining without touching their git config (e.g.,
    ``.claude/`` and ``.obsidian/`` are runtime configs that should be
    kept under version control but never mined as knowledge).
    """
    key = (dir_path, ".sageignore")
    if key not in cache:
        cache[key] = GitignoreMatcher.from_dir(dir_path, filename=".sageignore")
    return cache[key]
...
def load_config(project_dir: str) -> dict:
    """Load sage.yaml from project directory."""
    import yaml

    resolved_project_dir = Path(project_dir).expanduser().resolve()
    config_path = resolved_project_dir / "sage.yaml"
...
def chunk_text(
    ...
    # Defensive invariant guard. ``SageConfig.chunk_*`` already
...
# =============================================================================
# NOOK — ChromaDB operations
# =============================================================================


_ENTITY_REGISTRY_PATH = os.path.join(os.path.expanduser("~"), ".sage", "known_entities.json")
...
def _refresh_known_entities_cache() -> None:
    """Reload ``~/.sage/known_entities.json`` into the module cache if
...
def add_to_known_entities(entities_by_category: dict, wing: str = None) -> str:
    """Union ``entities_by_category`` into ``~/.sage/known_entities.json``.

    Accepts ``{category: [names]}`` shape as produced by ``sage init``
    ...
    miner-supported shape with explicit entity codes), new names are
    added as keys with ``None`` values so existing code mappings
    aren't overwritten.
    ...
    ``nook_graph.compute_topic_tunnels`` at mine time. Topics for a
    ...
    _tmp_path = registry_path.with_suffix(registry_path.suffix + ".tmp")
    try:
        _tmp_path.write_text(_json.dumps(existing, indent=2, ensure_ascii=False), encoding="utf-8")
        try:
            _tmp_path.chmod(0o600)
        except (OSError, NotImplementedError):
            pass
        os.replace(_tmp_path, registry_path)
    except OSError:
        try:
            if _tmp_path.exists():
                _tmp_path.unlink()
...
def detect_hall(content: str) -> str:
    ...
    if _HALL_KEYWORDS_CACHE is None:
        ...
        _HALL_KEYWORDS_CACHE = SageConfig().hall_keywords
...
def _extract_entities_for_metadata(content: str) -> str:
    ...
    *before* joining so a name is never cut in half.
    ...
    from .nook import _ENTITY_STOPLIST
    ...
    for name in known:
        ...
        # behavior so a known entity like "Alice" tags drawers that mention
        # "alice" / "ALICE" / "Alice". Without re.IGNORECASE, lowercase
        # mentions in chat transcripts and voice-typed content get silently
        # untagged.
...
# would hallucinate dates from any digit-bearing text (e.g. "tmp_random_5"
# → 2026-05-05, "Version 7.2.1" → 2006-03-03). The fuzzy=True flag is
...
def _build_drawer_metadata(
    ...
    (callers, pre-Tier-6a drawers), the keys are absent from the
    ...
    metadata = {
        ...
        # sage extension: tag every drawer this miner writes with
        # the agents that requested the mine (orchestrator passes them via
        # the `--agents` CLI flag). Searchable later via nook_search /
        # sage recall.
        "agents": serialize_agents(agents or []),
    }
...
def process_file(
    ...
    agents: Optional[list] = None,
    prefetched_mtimes: Optional[dict] = None,
) -> tuple:
    ...
    ``prefetched_mtimes`` is the optional one-shot version-gated
    source_file -> source_mtime map from ``bulk_check_mined`` (K1). When
    provided, the pre-lock "already filed?" check is an O(1) local lookup
    instead of a per-file ChromaDB query — the path public users hit on first
    ``sage bootstrap``. When ``None`` (direct callers / tests), it falls back
    to the per-file ``file_already_mined`` query. The authoritative post-lock
    re-check below always uses the live query, so concurrency safety is
    unchanged.
    """
    effective_min = min_chunk_size if min_chunk_size is not None else MIN_CHUNK_SIZE

    # Skip if already filed (pre-lock fast path).
    source_file = str(filepath)
    if not dry_run:
        if prefetched_mtimes is not None:
            cached_mtime = prefetched_mtimes.get(source_file)
            if cached_mtime is not None:
                try:
                    if abs(cached_mtime - os.path.getmtime(source_file)) < 0.001:
                        return 0, "general", None
                except OSError:
                    pass  # mtime unreadable → fall through and let the read below handle it
        elif file_already_mined(collection, source_file, check_mtime=True):
            return 0, "general", None
    ...
    # WI-8: secret-scan gate (ADR-0042 write-boundary).
    # Apply the high-confidence tier (not aggressive) before any chunk/embed/store.
    # Legitimate git SHAs survive; only real credential patterns are redacted.
    ...
    if effective_cap > 0 and len(chunks) > effective_cap:
        ...
        # line). This keeps ``sage mine ... > out.log 2> err.log``
        ...
        print(
            ...
            f"SAGE_MAX_CHUNKS_PER_FILE (set 0 to disable), or add to "
    ...
    with mine_lock(source_file):
        ...
        drawers_added = 0
        # Accumulate drawer IDs and metadata across batches so the closet
        # emitter below can consume them (Tier 6a date+line locators and
        # pre-computed IDs — avoids re-hashing every chunk_index a second time).
        all_ids: list = []
        all_metas: list = []
        ...
        if closets_col and drawers_added > 0:
            ...
            # back to the the 3-segment form automatically when not.
...
def scan_project(
    ...
    for root, dirs, filenames in os.walk(project_path):
        ...
        # .sageignore is independent of respect_gitignore — runtime
        # configs like .claude/ and .obsidian/ need to be excluded from
        # mining even when the user wants gitignored files included.
        nookignore_matchers = [
            matcher
            for matcher in nookignore_matchers
            if root_path == matcher.base_dir or matcher.base_dir in root_path.parents
        ]
        current_nook = load_sageignore_matcher(root_path, matcher_cache)
        if current_nook is not None:
            nookignore_matchers.append(current_nook)
        ...
        if nookignore_matchers:
            dirs[:] = [
                d
                for d in dirs
                if is_force_included(root_path / d, project_path, include_paths)
                or not is_gitignored(root_path / d, nookignore_matchers, is_dir=True)
            ]
        ...
        for filename in filenames:
            ...
            if respect_gitignore and active_matchers and not force_include:
                if is_gitignored(filepath, active_matchers, is_dir=False):
                    continue
            if nookignore_matchers and not force_include:
                if is_gitignored(filepath, nookignore_matchers, is_dir=False):
                    continue
...
def mine(
    ...
    """Mine a project directory into the nook.
    ...
    ``SAGE_MAX_CHUNKS_PER_FILE`` or ``MAX_CHUNKS_PER_FILE``; ``0``
...
def _mine_impl(
    ...
    nook_config = SageConfig()

    cfg_chunk_size = nook_config.chunk_size
    cfg_chunk_overlap = nook_config.chunk_overlap
    cfg_min_chunk_size = nook_config.min_chunk_size

    wing = wing_override or config["wing"]
    # Phase 4 wing-registration gate. Surface typo'd wings immediately
    # rather than silently filling a phantom wing on disk.
    from .extensions.wing_registry import require_registered_wing
    ...
    if not dry_run:
        collection = get_collection(nook_path)
        closets_col = get_closets_collection(nook_path)
        # K1 (ADR-0070): prefetch the version-gated source->mtime map in ONE
        # paginated scan so each file's pre-lock skip is an O(1) local lookup,
        # not a ChromaDB query per file. Public users hit this on first
        # `sage bootstrap` over a fresh repo; per-file queries made that scan
        # O(files × store-size). Empty on a fresh nook → every file mines.
        prefetched_mtimes = bulk_check_mined(collection)
    else:
        collection = None
        closets_col = None
        prefetched_mtimes = None
    ...
    try:
        for i, filepath in enumerate(files, 1):
            try:
                drawers, room, skip_reason = process_file(
                    ...
                    agents=agents,
                    prefetched_mtimes=prefetched_mtimes,
                )
        ...
        if not dry_run:
            ...
            _validate_nook_fts5_after_mine(nook_path)
        ...
        if files_skipped_chunk_cap > 0:
            ...
            print(
                ...
                f"(raise via --max-chunks-per-file or SAGE_MAX_CHUNKS_PER_FILE; "
                f"set 0 to disable)"
            )
        ...
        print('\n  Next: sage search "what you\'re looking for"')
        print(f"{'=' * 55}\n")
    except KeyboardInterrupt:
        ...
        print(
            f"\n  Re-run `sage mine {shlex.quote(project_dir)}` to resume — "
        ...
        print(
            f"\n  Re-run `sage mine {shlex.quote(project_dir)}` after addressing "
...
def _cleanup_mine_pid_file() -> None:
    ...
    Hook-spawned mines receive ``SAGE_MINE_PID_FILE`` in their env
    pointing at the slot the hook claimed for them
    (``~/.sage/hook_state/mine_pids/mine_<sha>.pid``). When the
    ...
    """
    pid_file_env = os.environ.get("SAGE_MINE_PID_FILE", "")
    if not pid_file_env:
        return
    try:
...
def _compute_topic_tunnels_for_wing(wing: str) -> int:
    ...
    """
    from .config import SageConfig
    from .nook_graph import topic_tunnels_for_wing
...
def _compute_entity_tunnels_for_wing(wing: str) -> int:
    ...
    Reads hallway records (``sage.hallways.list_hallways``) and
    ...
    from .nook_graph import entity_tunnels_for_wing
...
def status(nook_path: str):
    ...
    print(f"\n{'=' * 55}")
    print(f"  sage Status — {total} drawers")
    print(f"{'=' * 55}\n")
