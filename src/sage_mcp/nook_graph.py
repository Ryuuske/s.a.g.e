"""
nook_graph.py — Graph traversal layer for sage
======================================================

Builds a navigable graph from the nook structure:
...
from .nook import get_collection as _get_nook_collection
from .nook import mine_lock

logger = logging.getLogger("nook_graph")
...
def _normalize_wing(wing: str | None) -> str | None:
    ...
    (e.g. ``nook_public``).  Callers that pass the raw directory name
    (``wing-public``) would silently miss.  This helper aligns the lookup
...
def build_graph(col=None, config=None):
    """
    Build the nook graph from ChromaDB metadata.
    ...
    intentional for the MCP server's single-nook use case. Callers
    ...
    # single-nook use case. Callers switching collections must invalidate first.
    with _graph_cache_lock:
    ...
    while offset < total:
        ...
        for meta in batch["metadatas"]:
            # ChromaDB can return ``None`` for drawers with missing
            # metadata (partial writes). Skip silently rather than
            # crash the whole graph build.
            if meta is None:
                continue
            room = meta.get("room", "")
            wing = meta.get("wing", "")
    ...
    # when the nook is first populated.
    if nodes:
        with _graph_cache_lock:
...
def graph_stats(col=None, config=None):
    """Summary statistics about the nook graph."""
    nodes, edges = build_graph(col, config)
...
#
# Stored as a JSON file based on SageConfig.nook_path (where the
# nook itself lives) so they persist across nook rebuilds (not in
# ChromaDB which can be recreated).


def _get_tunnel_file(config=None) -> str:
    """Return the path to the tunnels.json file, derived from SageConfig.nook_path."""
    config = config or SageConfig()
...
def _load_tunnels(config=None):
    ...
    ``SageConfig`` (which re-reads ``sage.yaml`` from disk) on
...
def _save_tunnels(tunnels, config=None):
    ...
    established by #814 for the other sensitive nook files.

    ``config`` may be passed in by the caller to avoid re-instantiating
    ``SageConfig`` on every save.
    """
...
def create_tunnel(
    ...
    """Create an explicit (symmetric) tunnel between two locations in the nook.
    ...
    Note:
        ...
        time so underscore-form data and explicit-flag hyphen data both
    ...
    # Reject self-loop: a tunnel from (wing, room) to itself adds no
    # cross-wing navigation value and inflates `_canonical_tunnel_id`
    # collisions with the diagonal of the tunnel matrix. Surface as a
    # ValueError so the MCP wrapper's existing except path produces a
    # clean error envelope.
    if source_wing == target_wing and source_room == target_room:
        raise ValueError(
            f"Tunnel source ({source_wing}/{source_room}) and target are identical; "
            "self-loops are not allowed."
        )

    # Single SageConfig() per call — reused by _get_tunnel_file /
    # _load_tunnels / _save_tunnels below. Each SageConfig() re-reads
    # sage.yaml from disk; before this change the helpers each
    ...
    with mine_lock(_get_tunnel_file(config)):
        ...
        for existing in tunnels:
            if existing.get("id") == tunnel_id:
                ...
                # still-missing fields so older records also pick up
...
def follow_tunnels(wing: str, room: str, col=None, config=None):
    ...
    # sage.yaml slug (underscore) and an explicit ``--wing`` slug
...
# ``~/.sage/known_entities.json`` under ``topics_by_wing``.
#
...
def entity_tunnels_for_wing(
    ...
    same storage path (``create_tunnel`` → ``~/.sage/tunnels.json``),
