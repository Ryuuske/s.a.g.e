#!/usr/bin/env python3
"""
entity_registry.py — Persistent personal entity registry for sage.
...
def _wikipedia_lookup(word: str) -> dict:
    ...
    **Privacy warning:** This function makes an outbound HTTPS request to
    ...
    try:
        ...
        req = urllib.request.Request(url, headers={"User-Agent": "sage/1.0"})
...
class EntityRegistry:
    ...
    Stored at ~/.sage/entity_registry.json
    Schema:
    {
    ...
    DEFAULT_PATH = Path.home() / ".sage" / "entity_registry.json"
    ...
    def save(self):
        ...
        try:
            ...
            # mid-write — unlink the .tmp sidecar so it does not litter the nook
    ...
    def learn_from_text(self, text: str, min_confidence: float = 0.75, languages=("en",)) -> list:
        ...
        configured ``SageConfig().entity_languages`` to match the
        locales used at ``sage init`` time.
        """
        from sage_mcp.entity_detector import extract_candidates, score_entity, classify_entity
    ...
    def extract_unknown_candidates(self, query: str) -> list:
        ...
        """
        from .nook import _candidate_entity_words
