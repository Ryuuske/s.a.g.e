...
Usage:
    ...
    result = sweep("/path/to/session.jsonl", "/path/to/nook")
"""
...
from .nook import get_collection
...
def get_nook_cursor(collection, session_id: str) -> Optional[str]:
...
def sweep(jsonl_path: str, nook_path: str, source_label: Optional[str] = None) -> dict:
    ...
    For each message in the jsonl:
      - If timestamp < cursor for that session, skip (strictly earlier
        than anything already in the nook — already covered).
    ...
    * ``drawers_already_present`` — rows whose deterministic ID was
      already in the nook and got rewritten idempotently.
    ...
    * ``drawers_upserted`` — total writes = added + already_present.
    """
    collection = get_collection(nook_path, create=True)
    cursors: dict = {}
    ...
    for rec in parse_claude_jsonl(jsonl_path):
        ...
        if sid not in cursors:
            cursors[sid] = get_nook_cursor(collection, sid)
...
def sweep_directory(dir_path: str, nook_path: str) -> dict:
    ...
    for f in files:
        try:
            result = sweep(str(f), nook_path, source_label=str(f))
        except Exception as exc:
