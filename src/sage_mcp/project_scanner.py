...
This module is used as the primary signal in `sage init`. The regex
...
def _merge_detected(primary: dict, secondary: dict, drop_secondary_uncertain: bool = False) -> dict:
    ...
    Dedup is case-insensitive so "sage" (manifest name) absorbs "sage"
...
def discover_entities(
    ...
    The expected shape is the dict written by ``sage init`` to
    ``<nook>/.sage/origin.json`` (see ``corpus_origin.py``).
    """
    ...
    from sage_mcp.convo_scanner import is_claude_projects_root, scan_claude_projects
    ...
    if is_claude_projects_root(root_path):
        ...
        # so a `pyproject.toml` name like `sage` and a Claude Code
        # `cwd` variant like `sage` collapse into one entry — matches
    ...
    if llm_provider is not None:
        from sage_mcp.llm_refine import collect_corpus_text, refine_entities
    ...
    # candidates contributed by every source (manifests, git authors,
    # prose, LLM refinement). Idempotent: no corpus_origin → exact the prior schema.
    if corpus_origin is not None:
        from sage_mcp.entity_detector import _apply_corpus_origin
