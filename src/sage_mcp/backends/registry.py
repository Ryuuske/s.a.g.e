"""Backend registry + entry-point discovery.

Third-party backends ship as installable packages that declare a
``sage.backends`` entry point::

    # pyproject.toml of sage-postgres
    [project.entry-points."sage.backends"]
    postgres = "sage_postgres:PostgresBackend"

sage discovers them at process start. In-tree tests and local development
...
_ENTRY_POINT_GROUP = "sage.backends"
...
def resolve_backend_for_nook(
    *,
    explicit: Optional[str] = None,
    config_value: Optional[str] = None,
    env_value: Optional[str] = None,
    nook_path: Optional[str] = None,
    default: str = "chroma",
) -> str:
    """Resolve the backend name for a nook by priority order.

    1. Explicit kwarg / CLI flag
    2. Per-nook config value
    3. ``SAGE_BACKEND`` env var
    ...
    contains backend-identifiable artifacts. For new nooks, (5) wins.
    """
