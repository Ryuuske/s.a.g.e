"""ChromaDB-backed sage storage backend."""
...
# (nook 376 MB, link_lists.bin 0 bytes, no segfault) in 2026-04.
#
...
def _validate_where(where: Optional[dict]) -> None:
    ...
    Spec: silent dropping of unknown operators is forbidden.
    """
    if not where:
        return
    stack = [where]
...
def quarantine_stale_hnsw(nook_path: str, stale_seconds: float = 300.0) -> list[str]:
    ...
    for name in entries:
        ...
        seg_dir = os.path.join(nook_path, name)
...
def _vector_segment_id(nook_path: str, collection_name: str) -> Optional[str]:
...
def _hnsw_element_count(nook_path: str, segment_id: str) -> Optional[int]:
    ...
    """
    pickle_path = os.path.join(nook_path, segment_id, "index_metadata.pickle")
...
# read the collection metadata (older nooks missing the row, sqlite
...
# fixed 2000 floor would flag every actively-written nook as DIVERGED
...
def _read_sync_threshold(nook_path: str, collection_name: str) -> int:
    ...
    explicit setting — matches what older nooks were created
    with before PR #1191.
    """
    db_path = os.path.join(nook_path, "chroma.sqlite3")
    if not os.path.isfile(db_path):
...
def hnsw_capacity_status(nook_path: str, collection_name: str = "nook_drawers") -> dict:
    ...
    * ``segment_id``       — VECTOR segment UUID, or ``None`` if no nook
    ...
    * ``message``          — human-readable summary
    ...
    try:
        seg_id = _vector_segment_id(nook_path, collection_name)
        out["segment_id"] = seg_id

        sqlite_count = _sqlite_embedding_count(nook_path, collection_name)
        out["sqlite_count"] = sqlite_count

        if seg_id is None or sqlite_count is None:
            out["message"] = "nook state unreadable; skipping HNSW capacity check"
            return out

        hnsw_count = _hnsw_element_count(nook_path, seg_id)
        out["hnsw_count"] = hnsw_count

        sync_threshold = _read_sync_threshold(nook_path, collection_name)
        ...
        if divergence > threshold:
            ...
            out["message"] = (
                ...
                "are invisible to vector search. Run `sage repair` to rebuild."
            )
        else:
            out["status"] = "ok"
            out["message"] = (
...
def _sqlite_embedding_count(nook_path: str, collection_name: str) -> Optional[int]:
    """Count rows in chroma.sqlite3.embeddings for ``collection_name``.

    Mirrors :func:`sage_mcp.repair.sqlite_drawer_count` but kept in this
...
def _pin_hnsw_threads(collection) -> None:
    ...
    Fresh collections set this via ``metadata=`` at creation. Earlier-format nooks
...
def quarantine_invalid_hnsw_metadata(nook_path: str) -> list[str]:
...
def _fix_blob_seq_ids(nook_path: str) -> None:
    ...
    owns the ``max_seq_id`` column — we leave it alone. Nooks already
    poisoned by the old behaviour can be repaired via
    ``sage repair --mode max-seq-id``.
    ...
    no-op for genuine earlier-format data.
    ...
    nooks can touch the marker manually to opt into the fast path.
    """
    db_path = os.path.join(nook_path, "chroma.sqlite3")
    if not os.path.isfile(db_path):
        return
    marker = os.path.join(nook_path, _BLOB_FIX_MARKER)
    ...
    try:
        ...
        return
    # Write marker whether or not rows needed migration — the nook is now
...
class ChromaCollection(BaseCollection):
    ...
    When ``nook_path`` is set, all write methods (``add``, ``upsert``,
    ``update``, ``delete``) acquire ``mine_nook_lock(nook_path)`` for the
    duration of the underlying chromadb call. This serializes MCP and other
    direct-backend writers against ``sage mine`` and against each other,
    ...
    ``mine_nook_lock`` — no self-deadlock.

    ``nook_path=None`` disables the wrapping, preserving the original
    no-lock behaviour for callers that construct a ``ChromaCollection``
    directly without going through ``ChromaBackend``.
    """

    def __init__(self, collection, nook_path: Optional[str] = None):
        self._collection = collection
        self._nook_path = nook_path

    @contextlib.contextmanager
    def _write_lock(self):
        """Acquire ``mine_nook_lock`` for the configured nook, if any.

        No-op (yields immediately) when ``self._nook_path`` is None.
        """
        if self._nook_path is None:
            yield
            return
        # Late import — nook.py imports ChromaBackend from this module.
        from ..nook import mine_nook_lock

        with mine_nook_lock(self._nook_path):
            yield
    ...
    @staticmethod
    def _scrub_documents(documents):
        """Apply high-confidence secret scrub to every document in the list.

        Centralised write-boundary chokepoint (ADR-0042 / WI-8): every ingest
        path (miner, convo_miner, format_miner, diary_ingest, MineContext,
        MCP tool_add_drawer) routes through ChromaCollection.add / upsert /
        update, so scrubbing here covers ALL present and future ingest paths
        without per-caller whack-a-mole. Double-scrub is idempotent — callers
        that already scrub (miner.process_file, convo_miner._mine_convos_impl,
        MCP tool_add_drawer) are unaffected.

        Only the high-confidence tier (no aggressive hex≥40 pattern) is applied
        here — git SHAs survive verbatim in stored drawers (ADR-0042 I#5).
        The aggressive tier is reserved for layers.py::assemble_tier0.
        """
        if documents is None:
            return None
        return [_scrub_secrets(doc) if isinstance(doc, str) else doc for doc in documents]
    ...
    def add(self, *, documents, ids, metadatas=None, embeddings=None):
        kwargs: dict[str, Any] = {"documents": self._scrub_documents(documents), "ids": ids}
    ...
    def update(
        ...
        *,
        ...
        if documents is not None:
            kwargs["documents"] = self._scrub_documents(documents)
    ...
    def metadata(self) -> dict:
        ...
        Used by the searcher to detect earlier-format nooks that were created
...
class ChromaBackend(BaseBackend):
    """sage's default ChromaDB backend.

    Maintains two caches:

    * ``self._clients`` — ``nook_path -> PersistentClient`` for callers
      using the ``NookRef`` / :meth:`get_collection` path.
    * An inode+mtime freshness check absorbed from ``mcp_server._get_client``
      (merged via #757) ensuring a nook rebuild on disk is detected on the
    ...
    def __init__(self):
        # nook_path -> PersistentClient
        self._clients: dict[str, Any] = {}
        # nook_path -> (inode, mtime) of chroma.sqlite3 at cache time.
    ...
    @staticmethod
    def _explain_ef_mismatch(error: Exception, nook_path: str) -> Optional[str]:
        ...
        Triggered when ``SAGE_EMBEDDING_MODEL`` is switched on an
        existing nook — ChromaDB persists the EF name on the collection
        ...
        try:
            ...
            current_model = SageConfig().embedding_model
        except Exception:
            current_model = "unknown"
        return (
            f"Embedding model mismatch reading nook at {nook_path!r}.\n"
            f"  Underlying ChromaDB error: {msg}\n"
            f"  Current SAGE_EMBEDDING_MODEL={current_model!r}.\n"
            f"  The nook was built with a different embedding model. Either:\n"
            f"    (a) revert the model: unset SAGE_EMBEDDING_MODEL (or set "
            f"the previous value), or\n"
            f"    (b) re-embed in place: run `sage repair` (mode full, the default) "
            f"with the current model set — it rewrites the index. To target this "
            f"nook specifically, set SAGE_NOOK_PATH={nook_path!r} first."
        )
    ...
    @staticmethod
    def _harden_nook_perms(nook_path: str) -> None:
        """Restrict the nook dir + ``chroma.sqlite3`` to owner-only (ADR-0077).

        ``chroma.sqlite3`` holds verbatim drawer content (which may contain
        credential-shaped text the write-boundary scrub did not recognise), but
        chromadb creates it with the process umask — typically world-readable
        0644. Match the 0700/0600 discipline ``config.py`` already applies to
        its own files so the DB is not the one content sink left readable when
        the parent dir's perms are ever loosened. Best-effort: swallow OSError
        (e.g. non-owner, exotic filesystems).
        """
        try:
            os.chmod(nook_path, 0o700)
        except OSError:
            pass
        # Harden every content-bearing file under the nook to 0600, not just
        # chroma.sqlite3: the HNSW segment binaries (vectors) and any sqlite
        # sidecars (-wal/-shm) were left 0644 (E2E round-C). Walk the tree
        # best-effort; the 0700 dir guard is the backstop if a chmod fails.
        _sensitive_suffixes = (".sqlite3", ".sqlite3-wal", ".sqlite3-shm", ".bin", ".lock")
        for root, _dirs, files in os.walk(nook_path):
            for fname in files:
                if fname.endswith(_sensitive_suffixes):
                    try:
                        os.chmod(os.path.join(root, fname), 0o600)
                    except OSError:
                        pass

    @staticmethod
    def _db_stat(nook_path: str) -> tuple[int, float]:
    ...
    def _client(self, nook_path: str):
        """Return a cached ``PersistentClient``, rebuilding on inode/mtime change.

        Handles the nook-rebuild case (repair/nuke/purge) by invalidating the
        ...
        * Mtime change uses an epsilon (0.01 s) to tolerate FS timestamp
        ...
        cached = self._clients.get(nook_path)
        cached_inode, cached_mtime = self._freshness.get(nook_path, (0, 0.0))
        current_inode, current_mtime = self._db_stat(nook_path)

        db_path = os.path.join(nook_path, "chroma.sqlite3")
        # DB was present when cache was built but is now missing → invalidate.
        if cached is not None and not os.path.isfile(db_path):
            _close_client(self._clients.pop(nook_path, None))
            self._freshness.pop(nook_path, None)
            cached = None
        ...
        if cached is None or inode_changed or mtime_changed or mtime_appeared:
            ...
            # (post-restore, fresh nook at the same path, etc.); drop the
            ...
            if inode_changed:
                ChromaBackend._quarantined_paths.discard(nook_path)
            # NOTE: do NOT _close_client() the old cached client here. A rebuild
            # is triggered by mtime change too — and mining mutates chroma.sqlite3
            # on its first upsert, so a later _get_client in the SAME run sees
            # mtime_changed and rebuilds. Live collection handles obtained earlier
            # are still bound to the old client; explicitly closing it tears down
            # its RustBindingsAPI and the next upsert fails. The old client is
            # dropped from the cache (overwritten below) and GC-collected only
            # once its last handle is released. (Reverts a harmful audit "fix".)
            ChromaBackend._prepare_nook_for_open(nook_path)
            cached = chromadb.PersistentClient(path=nook_path)
            self._clients[nook_path] = cached
            ...
            self._freshness[nook_path] = self._db_stat(nook_path)
            # chromadb wrote the DB with the process umask (often 0644);
            # restrict it to owner-only now that it exists (ADR-0077).
            self._harden_nook_perms(nook_path)
        return cached

    # ------------------------------------------------------------------
    # Public static helpers (kept for callers; prefer :meth:`get_collection`)
    # ------------------------------------------------------------------

    # Per-process record of nooks that have already had the cold-start
    ...
    # opened the nook cleanly, re-firing the stale check on every reconnect
    ...
    # corrupt.
    # Real runtime drift is still handled — nook-daemon's ``_auto_repair``
    ...
    # ``make_client()`` calls for the same nook can both pass the
    ...
    @staticmethod
    def _prepare_nook_for_open(nook_path: str) -> None:
        ...
        3. ``quarantine_stale_hnsw`` — also gated by :attr:`_quarantined_paths`
           so it fires once per nook per process. This is the SIGSEGV
           prevention path for stale HNSW segments (see #1121, #1132, #1263);
           wiring it through this helper means CLI mining, search, repair,
           and status all benefit, not just the the ``make_client`` static helper
           callers.

        Idempotent: safe to call from any code path that is about to open or
        re-open a nook. The ``_quarantined_paths`` gate prevents thrash on
        hot paths (e.g. ``_client()`` is called on every backend operation).
        """
        _fix_blob_seq_ids(nook_path)
        if nook_path not in ChromaBackend._quarantined_paths:
            quarantine_invalid_hnsw_metadata(nook_path)
            quarantine_stale_hnsw(nook_path)
            ChromaBackend._quarantined_paths.add(nook_path)

    @staticmethod
    def make_client(nook_path: str):
        """Create a fresh ``PersistentClient`` (fixes BLOB seq_ids first).

        Deprecated-ish: exposed for long-lived callers that manage their
        own client cache. New code should obtain a collection through
        :meth:`get_collection` which manages caching internally.

        Quarantines HNSW segments **once per nook per process**. See
        :attr:`_quarantined_paths` for the rationale (cold-start protection
        vs. runtime thrash on steady-write daemons).
        """
        ChromaBackend._prepare_nook_for_open(nook_path)
        return chromadb.PersistentClient(path=nook_path)
    ...
    def get_collection(
        ...
        **kwargs,
        ...
        """Obtain a collection for a nook.

        Supports two calling conventions during the backend interface transition:

        * New (preferred): ``get_collection(nook=NookRef, collection_name=...,
          create=False, options=None)``.
        * Earlier: ``get_collection(nook_path, collection_name, create=False)``
          — still used by callers not yet migrated.
        """
        nook_ref, collection_name, create, options = _normalize_get_collection_args(args, kwargs)

        nook_path = nook_ref.local_path
        if nook_path is None:
            raise NookNotFoundError("ChromaBackend requires NookRef.local_path")

        if not create and not os.path.isdir(nook_path):
            raise NookNotFoundError(nook_path)

        # State B (#1498): dir exists but no chroma.sqlite3. Constructing a
        # PersistentClient here would LAZILY CREATE chroma.sqlite3 — a read
        # mutating the filesystem. Guard before _client(), mirroring the
        # State-A check above, so read-only callers never create the db.
        if not create and not os.path.isfile(os.path.join(nook_path, "chroma.sqlite3")):
            raise CollectionNotInitializedError(nook_path)
        ...
        if create:
            try:
                ...
                explanation = self._explain_ef_mismatch(e, nook_path)
                if explanation:
            ...
            try:
                ...
                raise CollectionNotInitializedError(nook_path) from e
            except ValueError as e:
                explanation = self._explain_ef_mismatch(e, nook_path)
                if explanation:
                    raise ValueError(explanation) from e
        ...
        return ChromaCollection(collection, nook_path=nook_path)

    def close_nook(self, nook) -> None:
        """Drop cached handles for ``nook`` and release its SQLite file lock.

        Accepts ``NookRef`` or raw path str. chromadb's rust-side file
        lock is held until ``PersistentClient.close()`` is called, so plain
        dict eviction would leave the nook path unreopenable and
        unremovable in the same process.
        """
        path = nook.local_path if isinstance(nook, NookRef) else nook
        if path is None:
            return
    ...
    def health(
        ...
        """Side-effect-free health probe for ``nook_path``.

        Never constructs a PersistentClient — it reads the HNSW capacity via the
        safe sqlite/pickle path (``hnsw_capacity_status``), so it is the
        pre-open probe ops relies on (#1222). Returns:
          * ``unhealthy`` when the backend is closed;
          * ``degraded`` (ok=True, vector_disabled=True) when the index diverged;
          * ``healthy`` otherwise. A probe that raises fails open (vector path),
            matching the prior ``_default_probe`` contract.
        """
        if self._closed:
        ...
        try:
            info = hnsw_capacity_status(nook_path, collection_name)
        except Exception:
            logger.debug("HNSW capacity probe raised", exc_info=True)
            return HealthStatus.healthy(detail="capacity probe failed")
        if info.get("diverged"):
            return HealthStatus.degraded(info.get("message", "HNSW divergence"), capacity=info)
        return HealthStatus.healthy(capacity=info)

    def reset(self, nook_path: str) -> None:
        """Drop the cached client for ``nook_path`` and re-arm the stale-HNSW
        quarantine, so the next ``get_collection`` rebuilds against fresh disk
        state. This is the load-bearing reset for the #1315/#1322 self-heal.
        """
        _close_client(self._clients.pop(nook_path, None))
        self._freshness.pop(nook_path, None)
        ChromaBackend._quarantined_paths.discard(nook_path)
    ...
    def delete_collection(self, nook_path: str, collection_name: str) -> None:
        """Delete ``collection_name`` from the nook at ``nook_path``."""
        self._client(nook_path).delete_collection(collection_name)

    def create_collection(
        self, nook_path: str, collection_name: str, hnsw_space: str = "cosine"
    ) -> ChromaCollection:
        """Create (not get-or-create) ``collection_name`` with the given HNSW space."""
        ef = self._resolve_embedding_function()
        ef_kwargs = {"embedding_function": ef} if ef is not None else {}
        collection = self._client(nook_path).create_collection(
            collection_name,
            metadata={
                "hnsw:space": hnsw_space,
                "hnsw:num_threads": 1,
...
def _normalize_get_collection_args(args, kwargs):
    """Unify older positional ``(nook_path, collection_name, create)`` calls
    with the new kwargs-only ``(nook=NookRef, collection_name=..., create=...)``.

    Returns ``(NookRef, collection_name, create, options)``.
    """
    # New-style: nook= kwarg with a NookRef (spec path).
    if "nook" in kwargs:
        nook_ref = kwargs.pop("nook")
        if not isinstance(nook_ref, NookRef):
            raise TypeError("nook= must be a NookRef instance")
        ...
        if args:
            raise TypeError("positional args not allowed with nook= kwarg")
        return nook_ref, collection_name, create, options

    # First positional is a path string.
    if args:
        nook_path = args[0]
        rest = list(args[1:])
        ...
        if kwargs:
            raise TypeError(f"unexpected kwargs: {sorted(kwargs)}")
        return (
            NookRef(id=nook_path, local_path=nook_path),
            collection_name,
            bool(create),
            None,
        )

    # Earlier kwargs-only (nook_path=..., collection_name=..., create=...)
    if "nook_path" in kwargs:
        nook_path = kwargs.pop("nook_path")
        collection_name = kwargs.pop("collection_name")
        create = kwargs.pop("create", False)
        if kwargs:
            raise TypeError(f"unexpected kwargs: {sorted(kwargs)}")
    ...
    raise TypeError("get_collection requires nook= or a positional nook_path")
