...
If Alice and Bob are both mentioned in 47 drawers across the diary,
letters, and ideas rooms, there's a hallway between them. If Alice
...
The tunnel primitive in ``nook_graph.py`` connects rooms
...
Persistence mirrors ``nook_graph._TUNNEL_FILE``: a JSON file under
``~/.sage/`` so the records survive across mines and are
...
logger = logging.getLogger("sage_hallways")

# Persistence target. Mirrors ``nook_graph._TUNNEL_FILE`` so the storage
...
_HALLWAY_FILE = os.path.join(os.path.expanduser("~"), ".sage", "hallways.json")
...
def _parse_entities(value) -> list[str]:
    ...
    the deduplication semantics below: a drawer that mentions ``Alice;Alice``
    should only contribute one Alice to the entity set for that drawer.
    """
    if not value:
        return []
...
def _hallway_id(wing: str, entity_a: str, entity_b: str) -> str:
    ...
    Sorting before hashing makes the id symmetric — (Alice, Bob) and
    (Bob, Alice) produce the same record. So an idempotent re-mine
...
def compute_hallways_for_wing(
    ...
    for meta in metadatas:
        ...
        for a, b in combinations(entities, 2):
            # Canonicalize order so (Alice, Bob) and (Bob, Alice) are the
    ...
    for key in sorted(pair_counts.keys()):
        ...
        # (the new-pair case + the older-record case both land cleanly).
