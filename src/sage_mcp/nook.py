"""
nook.py — Shared nook operations.
...
from .backends import BackendClosedError, CollectionNotInitializedError, NookNotFoundError
...
def default_backend():
    """The process-wide backend instance that ``get_collection`` routes through.

    ``ops.search`` uses this for ``health()``/``reset()`` so it acts on the same
    client cache the search path actually hits.
    """
...
def get_collection(
    ...
    """Get the nook collection through the backend layer."""
...
def get_closets_collection(nook_path: str, create: bool = True):
    """Get the closets collection — the searchable index layer."""
    return get_collection(nook_path, collection_name="nook_closets", create=create)
...
def _open_collection_or_explain(
    ...
    *,
    ...
    """Open the nook collection or print a state-specific message and return ``None``.
    ...
    :class:`sage.backends.NookNotFoundError` /
    :class:`sage.backends.CollectionNotInitializedError` directly.
    ...
    nook whose collection was never bootstrapped lazily gets one on the
    ...
    State C: DB is present but the ``nook_drawers`` collection has
    ...
    if not os.path.isdir(nook_path):
        ...
        emit("  Run: sage init <dir> then sage mine <dir>")
        return None
    if not os.path.isfile(os.path.join(nook_path, "chroma.sqlite3")):
        emit(f"\n  Nook dir at {nook_path} exists but has no chroma.sqlite3 yet.")
        emit("  Run: sage mine <dir>")
        return None
    try:
        return get_collection(nook_path, collection_name=collection_name, create=False)
    except CollectionNotInitializedError:
        emit(f"\n  Nook at {nook_path} is initialized but empty (no drawers yet).")
        emit("  Run: sage mine <dir>")
        return None
    except NookNotFoundError:
        emit(f"\n  No nook found at {nook_path}")
        emit("  Run: sage init <dir> then sage mine <dir>")
        return None
    except BackendClosedError:
        # Surface this as a programmer error, not a nook-state UX message:
        # a closed backend means the caller violated the backend lifecycle,
        # not that the nook on disk is in a recoverable state.
        raise
    except Exception as e:  # noqa: BLE001 — backend exceptions vary (chromadb, OSError, lock errors)
        emit(f"\n  Error opening nook at {nook_path}: {e!r}")
        emit("  Try: sage repair-status --nook <path>")
        return None
...
def _candidate_entity_words(text: str) -> list:
    ...
    if _CANDIDATE_RX_CACHE is None:
        ...
        patterns = get_entity_patterns(SageConfig().entity_languages)
        rxs = []
...
def build_closet_lines(source_file, drawer_ids, content, wing, room, drawer_metas=None):
    ...
    Original format (3 segments): ``topic|entities|→drawer_ids``
    ...
    the 3-segment form is used — backward compat for drawers filed before
    ...
    # signals "fall back to the 3-segment format" for every emitted line.
    ...
    # pipe-separated field; otherwise emit the the 3-segment form.
...
def _build_date_line_segment(drawer_metas):
    ...
    fields is missing or unparseable — caller then falls back to the older
    ...
    # sage.miner._extract_content_date) when present. Fall back to
...
def mine_lock(source_file: str):
    ...
    """
    lock_dir = os.path.join(os.path.expanduser("~"), ".sage", "locks")
...
class MineAlreadyRunning(RuntimeError):
    """Raised when another `sage mine` already holds the per-nook lock."""


class MineValidationError(RuntimeError):
    """Raised at end of mine when PRAGMA quick_check on the nook reports errors."""

    def __init__(self, nook_path: str, errors: list[str]) -> None:
        if not errors:
            raise ValueError("MineValidationError requires at least one error string")
        if not nook_path:
            raise ValueError("MineValidationError requires a non-empty nook_path")
        super().__init__(f"FTS5/SQLite quick_check failed: {len(errors)} issue(s)")
        self.nook_path = nook_path
        # Freeze the forensic snapshot so handlers cannot mutate it.
        self.errors: tuple[str, ...] = tuple(errors)


def _validate_nook_fts5_after_mine(nook_path: str) -> None:
    ...
    _close_chroma_handles(nook_path, backend=_DEFAULT_BACKEND)

    errors = sqlite_integrity_errors(nook_path)
    if errors:
        raise MineValidationError(nook_path, errors)


# Per-thread record of nooks this thread already holds the lock for. Used by
# `mine_nook_lock` to short-circuit re-entrant acquisition from the same
...
_nook_lock_holders = threading.local()


def _holder_state():
    """Return the per-thread (pid, keys) record, refreshing after fork."""
    keys = getattr(_nook_lock_holders, "keys", None)
    pid = getattr(_nook_lock_holders, "pid", None)
    current_pid = os.getpid()
    if keys is None or pid != current_pid:
        keys = set()
        _nook_lock_holders.keys = keys
        _nook_lock_holders.pid = current_pid
    return keys


def _held_by_this_thread(lock_key: str) -> bool:
    """Return True if this thread already holds ``mine_nook_lock`` for ``lock_key``."""
...
@contextlib.contextmanager
def mine_nook_lock(nook_path: str):
    """Per-nook non-blocking lock around the full `mine` pipeline.

    The per-file `mine_lock` only protects delete+insert interleave for a
    single source; it does not prevent N copies of `sage mine <dir>`
    from being spawned concurrently by hooks. When that happens, each copy
    drives ChromaDB HNSW inserts in parallel against the same nook,
    ...
    The lock file is keyed by sha256(nook_path) so mines against
    *different* nooks can still run in parallel — we only serialize
    writes into the same nook, which is the correctness boundary.
    ...
    normcase, `C:\\Nook` and `c:\\nook` would hash to different keys
    on Windows and let two concurrent mines touch the same on-disk nook.

    Non-blocking: if another `mine` is already writing to this nook,
    ...
    nook, the context manager passes through without re-acquiring. This
    ...
    lock_dir = os.path.join(os.path.expanduser("~"), ".sage", "locks")
    os.makedirs(lock_dir, exist_ok=True)
    resolved = os.path.realpath(os.path.expanduser(nook_path))
    lock_key_source = os.path.normcase(resolved)
    nook_key = hashlib.sha256(lock_key_source.encode()).hexdigest()[:16]
    lock_path = os.path.join(lock_dir, f"mine_nook_{nook_key}.lock")

    if _held_by_this_thread(nook_key):
        # Same thread already holds the lock for this nook — pass through.
        yield
        return
        ...
        if os.name == "nt":
            ...
            try:
                ...
                raise MineAlreadyRunning(
                    f"nook {resolved} is held by {holder}; "
...
# code should use `mine_nook_lock(nook_path)` for per-nook scoping.
mine_global_lock = mine_nook_lock
...
def file_already_mined(
    ...
    """Check if a file has already been filed in the nook.
    ...
    for the same source transcript. Earlier-format drawers without extract_mode are
...
def bulk_check_mined(collection) -> dict[str, float]:
    """Pre-fetch source_file -> source_mtime for documents at the current schema.

    Returns a dict mapping source_file -> source_mtime (as float) for every
    document that has both fields AND whose stored ``normalize_version`` is at
    or above the current ``NORMALIZE_VERSION``.  Callers (the project miner)
    can then check membership and compare mtimes locally instead of issuing one
    ChromaDB query per file — the batch equivalent of
    ``file_already_mined(..., check_mtime=True)``.

    The version gate matters: a file whose drawers predate a normalization
    upgrade must re-mine, so its stale entry is deliberately excluded here (it
    will be absent from the map → not skipped → re-mined), exactly as
    ``file_already_mined`` re-mines it via the ``stored_version <
    NORMALIZE_VERSION`` branch.

    Fetches the full collection in paginated batches (like nook_graph.py)
    ...
    try:
        ...
        while offset < total:
            ...
            for meta in batch["metadatas"]:
                ...
                if not src or mtime is None:
                    continue
                # Same default as file_already_mined: a missing version == 1.
...
def prefetch_mined_set(collection, extract_mode: Optional[str] = None) -> set[str]:
    ...
    nook, making a 2000-file sweep take >1h of pure skip-checking. This
