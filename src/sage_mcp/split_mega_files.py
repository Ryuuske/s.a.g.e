...
HOME = Path.home()
SOURCE_DIR = Path(os.environ.get("SAGE_SOURCE_DIR", str(HOME / "Desktop/transcripts")))

# People we know about (for name detection in content)
# Loaded from ~/.sage/known_names.json if it exists, otherwise generic fallback.
_KNOWN_NAMES_PATH = HOME / ".sage" / "known_names.json"
...
def extract_people(lines):
    ...
    if dir_match:
        ...
        # User can map usernames to names in ~/.sage/known_names.json
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Source directory (default: SAGE_SOURCE_DIR or ~/Desktop/transcripts)",
    )
    parser.add_argument(
    ...
    src_dir = Path(args.source).expanduser().resolve() if args.source else SOURCE_DIR
