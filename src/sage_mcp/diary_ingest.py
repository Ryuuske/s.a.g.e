"""
diary_ingest.py — Ingest daily summary files into the nook.
...
- Only new entries are processed by default (tracks entry count in a state
  file under ``~/.sage/state/`` — never inside the user's diary dir).
...
Usage:
    python -m sage_mcp.diary_ingest --dir ~/daily_summaries --nook ~/.sage/nook
    python -m sage_mcp.diary_ingest --dir ~/daily_summaries --nook ~/.sage/nook --force
"""
...
def _state_file_for(nook_path: str, diary_dir: Path) -> Path:
    """Return the per-(nook, diary-dir) state-file path under ~/.sage/state.

    Keyed by sha256 of (nook_path, diary_dir) so multiple diary folders
    pointing at the same nook each get an independent state file. The
    state file is *never* written inside the user's diary directory.
    """
    state_root = Path(os.path.expanduser("~")) / ".sage" / "state"
    state_root.mkdir(parents=True, exist_ok=True)
    key = hashlib.sha256(f"{nook_path}|{diary_dir}".encode()).hexdigest()[:24]
...
def _diary_drawer_id_entry(wing: str, date_str: str, entry_idx: int, entry_chunk_idx: int) -> str:
    ...
    The ``v2_`` prefix distinguishes new IDs from the earlier file-level
    scheme (one drawer per file). Earlier-format drawers from pre-#1539 nooks
    ...
    ``delete(where=...)`` step on full rebuild collects both earlier and
...
def _write_state_atomic(state_file: Path, state: dict) -> None:
    """Write diary resume state atomically via tmp+os.replace.

    A crash mid-write leaves the prior state intact rather than a truncated
    JSON fragment. The load path at diary_ingest.py guards json.loads with
    try/except and falls back to {} on any failure, so a corrupt state file
    causes only a redundant full re-ingest — but avoiding that with atomic
    writes is cheap.
    """
...
def ingest_diaries(
    ...
    """Ingest daily summary files into the nook.
    ...
    state_file = _state_file_for(str(nook_path), diary_dir)
    ...
    drawers_col = get_collection(nook_path)
    closets_col = get_closets_collection(nook_path)
    chunk_size = SageConfig().chunk_size
    ...
    for diary_path in diary_files:
        ...
        if not force:
            if prev_hash is not None:
                ...
                # Earlier state without content_hash: keep size-based skip but
        ...
        with mine_lock(source_file):
            ...
            # step: (a) the pre-#1539 file-level drawers under the old
            ...
            if full_rebuild:
                try:
                    ...
                    logger.debug(
                        "earlier-format purge skipped for %s: %s",
                        source_file,
                        exc,
                        exc_info=True,
                    )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Ingest daily summaries into the nook")
    parser.add_argument("--dir", required=True, help="Path to daily_summaries directory")
    parser.add_argument("--nook", default=os.path.expanduser("~/.sage/nook"))
    parser.add_argument("--wing", default="diary")
    parser.add_argument("--force", action="store_true")
    args = parser.parse_args()

    ingest_diaries(args.dir, args.nook, wing=args.wing, force=args.force)
