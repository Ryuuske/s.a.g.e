"""
fact_checker.py — Verify text against known facts in the nook.
...
Usage:
    from sage_mcp.fact_checker import check_text
    issues = check_text("Bob is Alice's brother", nook_path)

    # CLI
    python -m sage_mcp.fact_checker "Bob is Alice's brother" \\
        --nook ~/.sage/nook
"""
...
# ~/.sage/known_entities.json on every check_text call.
...
def check_text(text: str, nook_path: str = None, config=None) -> list:
    ...
    issues.extend(_check_kg_contradictions(text, nook_path))
...
def _check_kg_contradictions(text: str, nook_path: str) -> list:
    ...
    try:
        ...
        # KG lives alongside the nook collection; mcp_server uses the
        # same convention (see _kg init). Pass ``db_path`` — the previous
        # code passed a nonexistent ``nook_path`` kwarg which raised
        # TypeError, silently swallowed by the outer except and rendered
        # the entire KG-check path dead.
        kg = KnowledgeGraph(db_path=os.path.join(nook_path, "knowledge_graph.sqlite3"))
    except Exception:
        # KG unavailable (brand-new nook, corrupted DB, etc.) — skip.
        return []
...
def _reconfigure_stdio_utf8_on_windows():
    ...
    Thin wrapper around the shared helper in ``sage._stdio``. Mirrors
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Check text against known facts in the sage nook.",
    ...
    parser.add_argument(
        "--nook",
        default=os.path.expanduser("~/.sage/nook"),
        help="Path to the nook directory.",
    )
    ...
    found = check_text(text_in, nook_path=args.nook)
    if found:
