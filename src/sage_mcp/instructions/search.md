...
When the user wants to search their S.A.G.E. memories, follow these steps:
...
- nook_search(query, wing, room) -- Primary search tool. Pass the semantic
  query and any wing/room filters.
- nook_list_wings -- Discover all available wings. Use when the user asks
  what categories exist or you need to resolve a wing name.
- nook_list_rooms(wing) -- List rooms within a specific wing. Use to help
  the user navigate or to resolve a room name.
- nook_get_taxonomy -- Retrieve the full wing/room/drawer tree. Use when
  the user wants an overview of their entire memory structure.
- nook_traverse(room) -- Walk the knowledge graph starting from a room.
  Use when the user wants to explore connections and related memories.
- nook_find_tunnels(wing1, wing2) -- Find cross-wing connections (tunnels)
...
If MCP tools are not available, fall back to the CLI:
    ...
    S.A.G.E. search "query" [--wing X] [--room Y]
