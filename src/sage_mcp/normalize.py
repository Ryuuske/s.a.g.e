#!/usr/bin/env python3
"""
normalize.py — Convert any chat export format to sage transcript format.
...
# Hard cap on input file size to prevent unbounded memory use on a single
# load_and_normalize() call. 500 MB is well above any sane transcript /
# export size and leaves room for chunking to dominate before this fires.
...
def _try_normalize_json(content: str) -> Optional[str]:
    ...
    for parser in (_try_claude_ai_json,):
...
def _format_tool_result(content, tool_name: str) -> str:
    ...
    # Read/Edit/Write — omit result (content is in nook or git)
...
def _messages_to_transcript(messages: list, spellcheck: bool = True) -> str:
    ...
    if spellcheck:
        try:
            from sage_mcp.spellcheck import spellcheck_user_text
