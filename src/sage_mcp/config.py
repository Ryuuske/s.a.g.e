"""
sage configuration system.

Priority: env vars > config file (~/.sage/config.json) > defaults
"""
...
def normalize_wing_name(name: str) -> str:
    ...
    and when writing `sage.yaml`, so the miner's lookup matches at
...
def sanitize_name(value: str, field_name: str = "name") -> str:
    ...
    # NFC normalize before any further validation. Without this, the same
    # visible string in NFD form (e.g. `café` typed two different ways)
    # would produce two distinct wing slugs that look identical to the
    # operator. NFC is the web/JSON canonical form; using it as the
    # storage form makes round-trips stable across editors and OS layers.
...
def sanitize_optional_name(value=None, field_name: str = "name"):
    """Validate an OPTIONAL wing/room-style filter.

    Returns ``None`` for an absent/blank value (no filter); otherwise validates
    via :func:`sanitize_name` (raising ``ValueError`` on a malformed name). The
    single shared implementation for both the CLI/ops search path and the MCP
    tool filters.
    """
...
# backward compatibility and exact UTC datetimes for sub-day precision.
#
# Accepted:
#   YYYY-MM-DD
...
DEFAULT_NOOK_PATH = os.path.expanduser("~/.sage/nook")
DEFAULT_COLLECTION_NAME = "nook_drawers"


@lru_cache(maxsize=1)
def get_configured_collection_name() -> str:
    """Return the configured drawer collection name without repeated config-file reads."""
    return SageConfig().collection_name


# Single source of truth for chunking defaults. ``sage.miner``
# imports these so the older module-level ``CHUNK_SIZE`` /
# ``CHUNK_OVERLAP`` / ``MIN_CHUNK_SIZE`` constants stay in sync with
# ``SageConfig.chunk_*``. Putting them here (not in miner.py) keeps
...
DEFAULT_HALL_KEYWORDS = {
    ...
    "memory": ["memory", "remember", "forget", "recall", "archive", "nook", "store"],
    "technical": [
        "code",
        "python",
        "script",
    ...
    # Governance hall (ADR-0068): routes ADR / decision prose to hall=decisions,
    # matching the keeper charter's decision->hall 'decisions' category routing.
    # HIGH-SIGNAL markers only (codex MED1): generic tokens (decision, finding,
    # accepted, consequences, status) appear in handoffs/bugs/specs and would
    # misroute non-governance content into hall=decisions under the no-delete
    # floor — excluded. These tokens are rare outside ADR/governance prose.
    # NOTE: detect_hall matches with raw `kw in content_lower` substring checks,
    # so short markers must be token-safe — bare "adr" would match "quadratic",
    # "cadre", "adrenaline" (codex MED2). Use "adr-" (matches ADR citations like
    # "adr-0076") and other markers with no common-word substring collisions.
    "decisions": [
        "adr-",
        "supersede",
        "supersedes",
...
class SageConfig:
    """Configuration manager for sage.
    ...
    def __init__(self, config_dir=None):
        ...
        if config_dir is not None:
            ...
            self._config_dir = Path(os.path.expanduser("~/.sage"))
        ...
        if self._config_file.exists():
            try:
                ...
                if not isinstance(self._file_config, dict):
                    self._file_config = {}
    ...
    @property
    def nook_path(self):
        """Path to the memory nook data directory."""
        env_val = os.environ.get("SAGE_NOOK_PATH")
        if env_val:
            # Normalize: expand ~ and collapse .. to match the CLI --nook
            # code path (mcp_server.py:62) and prevent surprise redirection
            # when the env var contains unresolved components.
            return os.path.abspath(os.path.expanduser(env_val))
        return self._file_config.get("nook_path", DEFAULT_NOOK_PATH)

    @property
    def tunnel_file(self):
        """Path to the tunnel file, sibling of nook_path."""
        return os.path.join(os.path.dirname(self.nook_path), "tunnels.json")
    ...
    def hooks_auto_save(self):
        ...
        """
        env_val = os.environ.get("SAGE_HOOKS_AUTO_SAVE")
    ...
    def entity_languages(self):
        ...
        Reads from env var ``SAGE_ENTITY_LANGUAGES`` (comma-separated)
        first, then the ``entity_languages`` field in ``config.json``,
        defaulting to ``["en"]``.
        """
        env_val = os.environ.get("SAGE_ENTITY_LANGUAGES")
        if env_val:
    ...
    def embedding_device(self):
        ...
        ``"dml"``. Read from env ``SAGE_EMBEDDING_DEVICE`` first, then
        ``embedding_device`` in ``config.json``, then ``"auto"``.

        ``auto`` resolves to the first available accelerator at runtime via
        :mod:`sage.embedding`; requesting an unavailable accelerator
        logs a warning and falls back to CPU.
        """
        env_val = os.environ.get("SAGE_EMBEDDING_DEVICE")
        if env_val:
    ...
    def embedding_model(self):
        ...
        ``SAGE_EMBEDDING_MODEL`` first, then ``embedding_model`` in
        ``config.json``, then ``"minilm"`` as a back-compat fallback for
        nooks created before onboarding asked the question.

        Switching models on an existing nook requires re-embedding
        (different vector space) — ChromaDB rejects reads when the persisted
        EF name doesn't match. Run ``sage repair`` (mode full, the default)
        after changing this value.
        """
        env_val = os.environ.get("SAGE_EMBEDDING_MODEL")
        if env_val:
            return env_val.strip().lower()
    ...
    def topic_tunnel_min_count(self):
        ...
        link. Reads ``SAGE_TOPIC_TUNNEL_MIN_COUNT`` env first, then the
        config-file value, then ``1``.
        """
        env_val = os.environ.get("SAGE_TOPIC_TUNNEL_MIN_COUNT")
        if env_val:
            try:
                parsed = int(env_val)
                if parsed >= 1:
    ...
    @property
    def hooks_redact_secrets(self):
        """Whether the emergency-drawer write path applies secret-redaction.

        Default OFF — see rules/memory-hygiene.md for rationale.
        Enabled by setting ``hooks.redact_secrets = true`` in config, or by
        setting the ``SAGE_REDACT_SECRETS=1`` env var (env takes precedence).
        """
        return bool(self._file_config.get("hooks", {}).get("redact_secrets", False))

    def _atomic_write_config(self) -> None:
        """Write ``_file_config`` to disk atomically via tmp+os.replace.

        Shared by ``set_hook_setting``, ``set_entity_languages``, and
        ``set_embedding_model`` so all config writers use the same hardened
        pattern (Pass 3 Cat 13 F15 rationale). Propagates ``OSError`` to the
        caller — callers that want silent failure must catch it themselves.
        """
        self._config_dir.mkdir(parents=True, exist_ok=True)
        tmp_path = self._config_file.with_suffix(self._config_file.suffix + ".tmp")
        try:
    ...
    def set_hook_setting(self, key: str, value: bool):
        ...
        Atomic temp-and-rename so a concurrent reader cannot observe a
        truncated file (which the constructor at line 297 silently swallows
        via ``except JSONDecodeError`` → ``_file_config = {}`` → next
        write wipes every other setting). (Pass 3 Cat 13 F15)
        """
    ...
    def init(self):
        ...
        # Copy the repo-root wing_config.json template into the config
        # dir on first init so the wing-registration gate has a registry
        # to read. Skip if a user-edited copy already exists.
        wing_dst = self._config_dir / "wing_config.json"
        if not wing_dst.exists():
            try:
                ...
                for parent in here.parents:
                    ...
                    if candidate.is_file():
                        wing_dst.write_text(candidate.read_text(encoding="utf-8"))
                        try:
                            wing_dst.chmod(0o600)
                        except (OSError, NotImplementedError):
                            pass
                        break
            except Exception:
                # init must not fail because the template lookup hiccuped;
                # the user can re-run wing-registry-aware commands after
                # they place the file manually.
                pass
        ...
        # init's expected output also includes identity.txt (Layer0
        # reads it), the nook data directory itself (chroma
        # collection lives there), and an empty kg.sqlite knowledge-
        # graph db. All three are created here eagerly so
        # `sage init` produces the
        # complete structure the plan documents.
        identity_path = self._config_dir / "identity.txt"
        if not identity_path.exists():
            try:
                identity_path.write_text(
                    "# sage identity\n"
                    "# Replace this stub with a short description of yourself.\n"
                    "# Layer0 reads this for wake-up context.\n",
                    encoding="utf-8",
                )
                try:
        ...
        try:
            nook_dir = Path(self.nook_path).expanduser()
            nook_dir.mkdir(parents=True, exist_ok=True)
        except OSError:
            pass

        kg_path = self._config_dir / "kg.sqlite"
        if not kg_path.exists():
            try:
                from .knowledge_graph import KnowledgeGraph

                graph = KnowledgeGraph(db_path=str(kg_path))
                graph.close()
                try:
                    kg_path.chmod(0o600)
                except (OSError, NotImplementedError):
                ...
                # KG init failure must not break `sage init`;
                # the file is created lazily on first kg_add anyway.
                pass

        return self._config_file

    def save_people_map(self, people_map):
        """Write people_map.json to config directory atomically.

        Uses tmp+os.replace so a crash mid-write leaves the prior file intact
        rather than a truncated JSON fragment.

        Args:
            people_map: Dict mapping name variants to canonical names.
        """
        self._config_dir.mkdir(parents=True, exist_ok=True)
        tmp_path = self._people_map_file.with_suffix(self._people_map_file.suffix + ".tmp")
        try:
            with open(tmp_path, "w", encoding="utf-8") as f:
            ...
            try:
                ...
                pass
            os.replace(tmp_path, self._people_map_file)
        except OSError:
            try:
                if tmp_path.exists():
                    tmp_path.unlink()
