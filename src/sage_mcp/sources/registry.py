"""Source adapter registry + entry-point discovery.

Third-party adapters ship as installable packages that declare a
``sage.sources`` entry point::

    # pyproject.toml of sage-source-cursor
    [project.entry-points."sage_mcp.sources"]
    cursor = "sage_source_cursor:CursorAdapter"

sage discovers them at process start. In-tree tests and local
development can register manually via :func:`register`. Explicit
registration wins on name conflict.

Unlike storage backends, source adapters are never auto-
detected — the user selects the adapter explicitly via ``--source NAME``
or config. The default when no adapter is named is ``filesystem``
(to preserve current ``sage mine <path>`` behavior).
"""
...
_ENTRY_POINT_GROUP = "sage_mcp.sources"
...
def resolve_adapter_for_source(
    *,
    ...
    """Resolve the adapter name by priority order.

    1. Explicit ``--source`` flag or kwarg
    2. Per-source config value
    3. Default (``filesystem``)

    Auto-detection is *intentionally* absent on the read side; a
