"""Reference implementations of the reserved content transformations.
...
``sage.sources.transforms.<adapter_name>_<transform_name>`` so the
...
# reference under
# ``sage.sources.transforms.<adapter_name>_<transform_name>`` (per the
...
def strip_tool_chrome(text: str) -> str:
    ...
    ``sage.sources.transforms.conversations_strip_tool_chrome``.
    """
    return text
