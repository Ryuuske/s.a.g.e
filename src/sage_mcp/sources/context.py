"""``NookContext`` facade passed to source adapters.

Bundles the nook-side surface an adapter needs during :meth:`ingest`:
drawer collection, closet collection, knowledge graph, nook config, and
progress hooks. Adapters receive a ``NookContext`` instance and MUST NOT
import ``sage.nook`` directly — that coupling is what the facade
exists to prevent.

This module publishes the shape third-party adapters target. Core's mine
loop will construct a concrete ``NookContext`` and pass it to adapters
...
class _CollectionLike(Protocol):
    """Minimum of :class:`sage.backends.BaseCollection` adapters rely on.
    ...
    concrete backend. See ``sage/backends/base.py`` for the full surface.
    """
...
class NookContext:
    ...
    Fields:
        drawer_collection: The nook's drawer collection.
        closet_collection: The nook's closet collection, or ``None`` if the
            nook has no closets yet. Adapters should not write to this
            directly; core builds closets post-step.
        knowledge_graph: The nook's SQLite knowledge graph. Adapters
            advertising ``supports_kg_triples`` call ``add_triple`` on it.
        nook_path: Filesystem root of the nook (convenience; same as
            ``backend.NookRef.local_path``).
        config: Nook config object (hall keywords, rooms list, privacy
            floor, etc.). Shape is the existing :class:`SageConfig`.
    ...
    def upsert_drawer(self, record: DrawerRecord) -> None:
        ...
        metadata stamps so adapters never need to populate them.
        """
...
def _build_drawer_id(record: DrawerRecord) -> str:
    ...
    negligible across corpora the size of a nook (sha1@64 bits was too
    close to the birthday bound for large ingests). Adapters that need a
    different id scheme can bypass :meth:`NookContext.upsert_drawer` and
