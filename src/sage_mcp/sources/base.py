...
Mirrors what ``sage/backends/base.py`` does for the write side: it
defines the read-side surface every source adapter must implement. A
source adapter extracts content from a specific origin (filesystem,
git, …) and yields typed records (``SourceItemMetadata`` /
``DrawerRecord``) that core routes into the nook.
"""
...
if TYPE_CHECKING:
    from .context import NookContext  # noqa: F401  (used in string annotation)
...
class TransformationViolationError(SourceAdapterError):
    ...
    an undeclared transformation (the source-adapter spec)."""
...
class DrawerRecord:
    ...
    per the backend spec — the chroma constraint. Nested data belongs on the
    knowledge graph or in a declared ``json_string`` field.
    """
...
@dataclass(frozen=True)
class FieldSpec:
    """Declared shape of a single per-adapter metadata field."""
...
class BaseSourceAdapter(ABC):
    """Long-lived adapter serving many ``SourceRef`` invocations.
    ...
    * ``default_privacy_class`` — privacy class level applied unless the
      nook config overrides it.
    """
    ...
    def is_current(
        ...
        *,
        ...
        """Return True if the nook already has an up-to-date copy of ``item``.
