...
in ``hallways.py`` and ``nook_graph.py`` invoke these functions; the
...
# SageConfig if real-nook empirical tuning calls for it.
# ─────────────────────────────────────────────────────────────────────────────

STRENGTH_FLOOR = 0.05
"""Lower bound on strength. Connections never decay below this — they
become dim but remain queryable explicitly. The nook doesn't forget;
salience just drops."""
...
def potentiate(
    ...
    *,
    ...
    if now is None:
        ...
        # Coerce naive injected `now` to UTC so subtraction against _parse_iso's
        # tz-aware datetimes doesn't raise TypeError. Mirrors consolidation.py:353-359.
...
def apply_decay(connection: dict, *, now: Optional[datetime] = None) -> dict:
    ...
    if now is None:
        ...
        # Coerce naive injected `now` to UTC. Mirrors potentiate() above and
        # consolidation.py:353-359 (the same hazard documented there).
        now = now.replace(tzinfo=timezone.utc)
