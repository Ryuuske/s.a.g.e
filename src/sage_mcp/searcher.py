...
from .nook import get_closets_collection, get_collection
...
def _open_drawers_or_error(nook_path, collection_name):
    """Open the drawers collection for read. Returns ``(collection, None)`` on
    success or ``(None, error_dict)`` with an actionable hint.

    The backend raises ``CollectionNotInitializedError`` for State B (dir exists,
    no chroma.sqlite3 — guarded before PersistentClient creation, #1498) and
    State C (db present, collection uninitialized); both map to "run sage mine"
    so the operator isn't told to re-init an already-initialized nook.
    """
    try:
        return get_collection(nook_path, collection_name=collection_name, create=False), None
    except CollectionNotInitializedError:
        return None, {"error": "Nook is initialized but empty", "hint": "Run: sage mine <dir>"}
    except Exception as e:
        logger.error("No nook found at %s: %s", nook_path, e)
        return None, {"error": "No nook found", "hint": "Run: sage init <dir> && sage mine <dir>"}


def _unsupported_metric_warning(col) -> "str | None":
    """Return a warning string if the collection was created without
    ``hnsw:space=cosine`` (older-format nooks score similarity as junk under
    L2), else ``None``. Never raises — a missing metadata table is treated as
    suspect, matching the pre-refactor behaviour.
    """
    try:
    ...
    if meta.get("hnsw:space") == "cosine":
        return None
    detail = (
        f"hnsw:space={meta.get('hnsw:space')!r}"
        if meta.get("hnsw:space")
        else "no hnsw:space metadata"
    )
    return (
        f"nook created without cosine distance ({detail}); similarity scores are "
        "not meaningful — run `sage repair` to rebuild the index with the correct metric"
    )
...
def _hybrid_rank(
    ...
    """Re-rank ``results`` by a combined score of vector similarity, BM25,
    and an additive drawer-strength tiebreaker (WI-6 decay integration).

    The base score is a convex combination of vector similarity and BM25
    (vector_weight=0.6, bm25_weight=0.4 by default — they sum to 1.0).
    Drawer strength is then an additive boost on top of that base via
    strength_weight=0.1. Because strength_weight is separate, the total
    weights sum to 1.1 rather than 1.0 — strength acts as a tiebreaker/
    boost beyond the base retrieval signal, not a replacement for it.
    ...
    * Drawer strength (WI-6): the ``strength`` metadata field written by the
      decay/consolidation pass. Values are in [DRAWER_STRENGTH_FLOOR, 1.0]
      by default (or up to MAX_STRENGTH for KG connections — drawers are
      floored at DRAWER_STRENGTH_FLOOR=0.1). Strength is min-max normalised
      within the candidate set so a drawer at the default (1.0) competes
      fairly with decayed drawers. This makes decayed drawers rank lower in
      retrieval, which is the PRD §8 goal: "recent/high-confidence/core win".
      When all candidates have the same strength (e.g. no decay has run yet),
      the normalised contribution is 0.0 — no effect on ranking, backward
      compatible.

    Candidates with ``distance=None`` are treated as vector-unknown
    (no vector signal available) and scored on BM25 + strength alone.
    ...
    # WI-6: extract strength so _hybrid_rank weights decay/consolidation signal.
    # Two shapes reach _hybrid_rank with a metadata dict:
    #   CLI search() path (line ~440): r["metadata"] = {..meta dict..} — uses
    #     the nested branch.
    #   search_memories path (line ~1030): top-level "strength" key, no
    #     r["metadata"] — uses the else branch.
    #   _bm25_only_via_sqlite candidates: no strength key at all — else branch
    #     returns None and the caller defaults to DRAWER_STRENGTH_DEFAULT.
    def _get_strength(r: dict) -> float:
        # CLI search() result shape: r["metadata"] = {..meta dict..}
        meta = r.get("metadata")
        if isinstance(meta, dict):
            raw = meta.get("strength")
        else:
            # search_memories entry shape: strength is a top-level key.
            # _bm25_only_via_sqlite candidates: key absent → raw = None.
            raw = r.get("strength")
        if raw is None:
            from .consolidation import DRAWER_STRENGTH_DEFAULT
    ...
    strengths = [_get_strength(r) for r in results]
    min_s = min(strengths) if strengths else 0.0
    max_s = max(strengths) if strengths else 1.0
    s_range = max_s - min_s
    # Min-max normalise strength: when all equal, norm = 0 (no effect).
    if s_range > 1e-9:
        strength_norm = [(s - min_s) / s_range for s in strengths]
    else:
        strength_norm = [0.0] * len(strengths)

    scored = []
    for r, raw, norm, s_norm in zip(results, bm25_raw, bm25_norm, strength_norm):
        ...
        score = vector_weight * vec_sim + bm25_weight * norm + strength_weight * s_norm
...
def _apply_agents_filter(hits: list, agents: Optional[list]) -> list:
    """Drop hits whose drawer ``agents`` list doesn't intersect ``agents``.

    Used by both ``search_memories`` (vector path) and
    ``_bm25_only_via_sqlite`` (fallback path) so the filter semantics are
    identical regardless of HNSW health. A drawer with an empty / absent
    agents list never matches when a non-empty filter is active.
    """
    if not agents:
        return hits
    filter_set = {a for a in agents if a}
    if not filter_set:
        return hits
    return [h for h in hits if filter_set.intersection(h.get("agents") or [])]
...
def _bm25_only_via_sqlite(
    ...
    try:
        ...
        if not candidate_ids and use_recency_fallback:
            ...
            # Wrapped in try/except because the schema may differ on older
            # nooks (older chromadb without ``created_at``, missing
        ...
        # Map chromadb's internal embeddings.id (rowid, integer) back to the
        # user-facing embedding_id (string drawer_id) so the BM25-fallback
        # response can include drawer_id like the vector path does — and
        # so `sage recall` shows real IDs even in HNSW-disabled mode.
        # Errors here are non-fatal: fall through to drawer_id="" for hits.
        rowid_to_drawer_id: dict[int, str] = {}
        try:
            id_rows = conn.execute(
                f"SELECT id, embedding_id FROM embeddings WHERE id IN ({placeholders})",
                candidate_ids,
            ).fetchall()
            rowid_to_drawer_id = {rid: eid for rid, eid in id_rows if eid}
        except sqlite3.Error:
            logger.debug("embeddings.embedding_id lookup failed", exc_info=True)
    finally:
        conn.close()
    ...
    for emb_id, key, sval, ival in meta_rows:
        d = drawers.setdefault(
            ...
            {
                "_id": emb_id,
                "drawer_id": rowid_to_drawer_id.get(emb_id, ""),
                "metadata": {},
                "text": "",
            },
        )
    ...
    # Agents post-filter — shared with the vector path via the helper so
    # filter semantics stay identical across HNSW health states.
    hits = _apply_agents_filter(hits, agents)

    return {
        "query": query,
        "filters": {"wing": wing, "room": room, "agents": list(agents) if agents else None},
...
def _merge_bm25_union_candidates(
    ...
    def _dedup_key(entry: dict):
        ...
        # avoids silently dropping candidates on earlier-format data while still
...
def _apply_candidate_strategy(
    ...
    if merger is not None:
        merger(hits, query, nook_path, wing, room, n_results, max_distance=max_distance)


def _enrich_closet_hits(
    hits: list, drawers_col, query: str, max_hydration_chars: int = 10000
) -> None:
    """Expand closet-boosted hits to the keyword-best drawer chunk + neighbors.

    Mutates ``hits`` in place. Extracted from ``search_memories`` to keep that
    function under the C901 complexity ceiling. Memoizes per-source fetches so
    two hits from the same source_file share one ``drawers_col.get()`` call.
    """
    source_fetch_cache: dict = {}
    for h in hits:
        ...
        if not full_source:
            continue
        try:
            if full_source in source_fetch_cache:
                source_drawers = source_fetch_cache[full_source]
            else:
                source_drawers = drawers_col.get(
                    where={"source_file": full_source},
                    include=["documents", "metadatas"],
                )
                source_fetch_cache[full_source] = source_drawers
        except Exception:
        ...
        if len(expanded) > max_hydration_chars:
            expanded = (
                ...
                "Use nook_get_drawer for full content.]"
            )
        h["text"] = expanded
...
def search_memories(
    ...
    Args:
        ...
        nook_path: Path to the ChromaDB nook directory.
        wing: Optional wing filter.
        room: Optional room filter.
        n_results: Max results to return.
        max_distance: Max cosine distance threshold. The nook collection uses
    ...
    drawers_col, open_error = _open_drawers_or_error(nook_path, collection_name)
    if open_error is not None:
        return open_error

    where = build_where_filter(wing, room)

    # Legacy nooks created before ``hnsw:space=cosine`` was pinned produce
    # meaningless similarity scores under the default L2 metric (distances
    # exceed 1.0, flooring every result to 0.0). Surface a one-line warning +
    # repair pointer so the operator isn't left thinking search is broken.
    metric_warning = _unsupported_metric_warning(drawers_col)
    ...
    try:
        closets_col = get_closets_collection(nook_path, create=False)
        ckwargs = {
            "query_texts": [query],
    ...
    # Capture total before filter once here (O(1) accessor); reuse in the
    # return dict to avoid a second call.
    _total_before_filter = len(_first_or_empty(drawer_results, "documents"))
    ...
    for did, doc, meta, dist in zip(
        ...
        entry = {
            ...
            # WI-6: carry strength through so _hybrid_rank can use the
            # decay/consolidation signal written by consolidation_pass.
            # This entry has no "metadata" sub-dict; _get_strength falls
            # through to the top-level r.get("strength") branch.  The CLI
            # search() path (line ~440) wraps meta in r["metadata"] and hits
            # the nested-dict branch; _bm25_only_via_sqlite candidates carry
            # no strength key at all and fall back to DRAWER_STRENGTH_DEFAULT.
    ...
    # wrong chunk within it; grep picks the right one. Extracted to
    # _enrich_closet_hits to keep this function under the C901 ceiling.
    ...
    # Agents post-filter (sage extension). ChromaDB's where clause
    # can't do array-contains against a JSON-encoded list, so filtering
    # runs in Python against the in-memory hit list right before return.
    # Extracted to ``_apply_agents_filter`` to keep ``search_memories``
    # under the ruff C901 complexity ceiling.
    hits = _apply_agents_filter(hits, agents)

    return {
        "query": query,
        "filters": {"wing": wing, "room": room, "agents": list(agents) if agents else None},
        "total_before_filter": _total_before_filter,
        "results": hits,
        # None unless the nook uses a non-cosine metric (legacy); consumers
        # falsy-check it (render banner / MCP transparency).
...
# See docs/specs/virtual-line-numbering.md for the full design rationale.
# ─────────────────────────────────────────────────────────────────────────────
