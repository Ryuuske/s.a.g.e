...
terabytes on large nooks — eventually causing segfaults.

This module provides four operations:

  status  — compare sqlite vs HNSW element counts (read-only health check)
  scan    — find every corrupt/unfetchable ID in the nook
...
full nook directory — so it works even when link_lists.bin is bloated.

Usage (standalone):
    python -m sage_mcp.repair status
    python -m sage_mcp.repair scan [--wing X]
    python -m sage_mcp.repair prune --confirm
    python -m sage_mcp.repair rebuild

Usage (from CLI):
    sage repair
    sage repair-scan [--wing X]
    sage repair-prune --confirm
"""
...
# The closets collection (the topic/summary index layer) is intentionally
# fixed — closets reference drawer IDs by string and live alongside drawers
# in the same nook; renaming the closets collection per-deployment would
# break cross-nook closet lookups. Drawer collection name comes from
# config (see ``_recoverable_collections``).
CLOSETS_COLLECTION_NAME = "nook_closets"
...
def _drawers_collection_name() -> str:
    ...
    Recovery flows must honor ``SageConfig().collection_name`` so a
    user with a non-default drawer collection (e.g. multi-nook setups)
    ...
    try:
        ...
        return SageConfig().collection_name or COLLECTION_NAME
    except Exception:
...
def _recoverable_collections() -> tuple[str, ...]:
    ...
    Drawers first (bulk data), then closets (topic/summary index layer
    that references drawer IDs by string in their documents — no
...
def _get_nook_path():
    ...
    try:
        ...
        default = os.path.join(os.path.expanduser("~"), ".sage", "nook")
        return default
...
def _delete_collection_if_exists(backend, nook_path: str, collection_name: str) -> None:
    try:
        backend.delete_collection(nook_path, collection_name)
...
def _rebuild_collection_via_temp(
    ...
    try:
        _delete_collection_if_exists(backend, nook_path, temp_name)

        progress(f"  Building temporary collection: {temp_name}")
        temp_col = backend.create_collection(nook_path, temp_name)
        staged = 0
        ...
        backend.delete_collection(nook_path, collection_name)
        live_replaced = True
        new_col = backend.create_collection(nook_path, collection_name)
...
def scan_nook(nook_path=None, only_wing=None, collection_name: Optional[str] = None):
    """Scan the nook for corrupt/unfetchable IDs.

    Probes in batches of 100, falls back to per-ID on failure.
    Writes corrupt_ids.txt to the nook directory for the prune step.
    ...
    bad_file = os.path.join(nook_path, "corrupt_ids.txt")
...
def prune_corrupt(nook_path=None, confirm=False, collection_name: Optional[str] = None):
...
# at exactly 10 000 even on nooks with many more rows. Refusing to
...
def check_extraction_safety(
    ...
    collection_name = collection_name or _drawers_collection_name()
    sqlite_count = sqlite_drawer_count(nook_path, collection_name)
    ...
    if sqlite_count is not None and sqlite_count > extracted:
        ...
        message = (
            ...
            "\n"
            "  Recovery options:\n"
            "    1. Restore from your most recent nook backup, then re-mine.\n"
            "    2. Direct-extract from chroma.sqlite3 (rows are still on disk) and\n"
            "       rebuild the nook from source files.\n"
            "    3. If you have independently confirmed the nook really contains only\n"
    ...
    if cap_signal and sqlite_count is None:
        message = (
            ...
            f"  nook genuinely holds {CHROMADB_DEFAULT_GET_LIMIT:,} rows or whether extraction was\n"
            "  silently capped. Refusing to overwrite the nook.\n"
            "\n"
            "  If you have independently confirmed (e.g. via direct sqlite3 query) that\n"
            f"  the nook really contains exactly {CHROMADB_DEFAULT_GET_LIMIT:,} drawers, re-run with\n"
            "  --confirm-truncation-ok.\n"
        )
        raise TruncationDetected(message, sqlite_count, extracted)


def sqlite_drawer_count(nook_path: str, collection_name: Optional[str] = None) -> "int | None":
    ...
    """
    collection_name = collection_name or _drawers_collection_name()
    sqlite_path = os.path.join(nook_path, "chroma.sqlite3")
...
def sqlite_integrity_errors(nook_path: str) -> list[str]:
...
def print_sqlite_integrity_abort(nook_path: str, errors: list[str]) -> None:
    ...
    print("  `sage repair` will not call Chroma delete_collection() because")
    ...
    print("    1. Stop all sage writers / MCP clients.")
    print("    2. Back up the entire nook directory.")
    ...
    print("    6. Re-run `sage repair --yes`.")
...
def maybe_repair_poisoned_max_seq_id_before_rebuild(
    ...
    *,
    ...
    stop instead of continuing into the original rebuild path. The rebuild path
...
def _vacuum_and_rebuild_fts5(nook_path: str, progress=print) -> None:
    """VACUUM the nook SQLite file and rebuild the FTS5 index if present.
...
def rebuild_index(
    ...
    nook genuinely contains exactly the extracted number of drawers
    (typically only a concern for nooks sized at exactly 10 000 rows).
    ...
    progress(f"\n{'=' * 55}")
    progress("  sage Repair — Index Rebuild")
    progress(f"{'=' * 55}\n")
    progress(f" Nook: {nook_path}")
    ...
    try:
        ...
        progress(f"  Error reading nook: {e}")
        progress("  Nook may need to be re-mined from source files.")
        return
    ...
    try:
        ...
        if e.live_replaced and os.path.exists(backup_path):
            ...
            try:
                ...
                _delete_collection_if_exists(backend, nook_path, collection_name)
                shutil.copy2(backup_path, sqlite_path)
                progress("  Backup restored. Nook is back to pre-repair state.")
            ...
            print("  Live collection was not replaced; leaving the original nook untouched.")
        raise

    _close_chroma_handles(nook_path, backend=backend)
    _vacuum_and_rebuild_fts5(nook_path, progress=progress)
...
class RebuildPartialError(Exception):
    ...
    the dest path holding the partial nook, and the archive path
...
def _rebuild_one_collection(
    *,
    ...
    freshly-created collection at ``dest_nook``. Returns rows
    ...
    silently shipping a partial nook.
    """
    ids: list[str] = []
    docs: list[str] = []
    metas: list[dict] = []
    ...
    try:
        ...
        col = backend.create_collection(dest_nook, collection_name)

        for emb_id, doc, meta in extract_via_sqlite(source_nook, collection_name):
            ids.append(emb_id)
            docs.append(doc or "")
            # chromadb 1.5.x rejects both None and empty-dict entries in
            # the metadatas list (ValueError: Expected metadata to be a
            # non-empty dict). sage drawers always carry at least
        ...
        msg_parts = [
            ...
            f"Partial nook left at: {dest_nook}",
        ]
        if archive_path is not None:
            msg_parts.append(f"Original nook archived at: {archive_path}")
            msg_parts.append(
                "  Recover by removing the partial dest and re-running with "
                f"--source {archive_path}"
            )
        else:
            msg_parts.append("  Source nook is unchanged. Remove the partial dest and re-run.")
        message = "\n  ".join(msg_parts)
        print(f"\n  ERROR: {message}")
...
def extract_via_sqlite(nook_path: str, collection_name: str) -> Iterator[tuple[str, str, dict]]:
    ...
    HNSW segment writer. This is the recovery path for nooks where
    ...
    Silent on missing nook, missing ``chroma.sqlite3``, or unknown
...
def rebuild_from_sqlite(
    ...
    *,
    ...
    """Rebuild a nook by reading drawers from ``source_nook``'s
    ``chroma.sqlite3`` and upserting them into a fresh nook at
    ``dest_nook``.
    ...
    the older ``rebuild_index`` and the inline ``cli.cmd_repair`` path call
    ...
    ``archive_existing_dest`` controls behavior when ``dest_nook``
    already exists:

    * ``False`` (default) — refuse with a clear message. Callers must
      manually move the existing nook aside first.
    * ``True`` — rename ``dest_nook`` to
      ``<dest_nook>.pre-rebuild-<timestamp>`` and read from there
      instead. Used by the in-place CLI flow where ``--source`` defaults
      to the same path as ``--nook``.
    ...
    chromadb upsert fails partway through; the dest nook is left in
    ...
    .. warning::
       ...
       In-place mode (``source_nook == dest_nook`` with
       ...
       an existing cached System built against the original nook will
       ...
       *any* nook, not just this one. Do not call this function from
       inside a long-running sage process (MCP server, daemon)
       while other callers hold live ``PersistentClient`` references —
       use the CLI in a separate process instead. Cross-nook use
       (``source != dest``) does not touch the cache.

    Note on metadata fidelity: the resolution rule
    (``string_value`` → ``int_value`` → ``float_value`` → ``bool_value``)
    matches the precedent in :mod:`sage.migrate`. ChromaDB 0.4.x
    ...
    behavior.
    """
    source_nook = os.path.abspath(os.path.expanduser(source_nook))
    dest_nook = os.path.abspath(os.path.expanduser(dest_nook))

    src_db = os.path.join(source_nook, "chroma.sqlite3")

    in_place = source_nook == dest_nook

    print(f"\n{'=' * 55}")
    print("  sage Repair — Rebuild from SQLite")
    print(f"{'=' * 55}\n")
    print(f"  Source: {source_nook}")
    print(f"  Dest:   {dest_nook}")
    ...
    # (--source pointing at a non-nook dir) bails cleanly.
    if in_place:
        if not archive_existing_dest:
            print(
                ...
                "the existing nook aside, or pass a different source_nook= "
                "(CLI: --source)."
            )
            return {}
        if not os.path.isfile(src_db):
            print(f"\n  Source nook has no chroma.sqlite3 at {src_db}")
            return {}
    else:
        if not os.path.isfile(src_db):
            print(f"\n  Source nook has no chroma.sqlite3 at {src_db}")
            return {}
        if os.path.exists(dest_nook):
            print(
                f"\n  Refusing to rebuild into existing path: {dest_nook}\n"
                "  Move it aside, pass a different dest, or set "
                "archive_existing_dest=True if rebuilding in place "
                "(source_nook == dest_nook)."
            )
            return {}
    ...
    if in_place:
        ...
        archive_path = f"{dest_nook}.pre-rebuild-{ts}"
        print(f"  Archiving {dest_nook} → {archive_path}")
        shutil.move(dest_nook, archive_path)
        source_nook = archive_path
        src_db = os.path.join(source_nook, "chroma.sqlite3")

        # In-place only: drop chromadb's process-wide System registry so
        # the new client at dest_nook builds a fresh System. Without
        # this, ``create_collection`` raises "Collection already exists"
        # because the cached System still holds the pre-rename schema.
        # Cross-nook mode does not need this and would needlessly
    ...
    os.makedirs(dest_nook, exist_ok=True)

    # Backend lifetime is wrapped in try/finally so the dest nook's
    ...
    # (cf. #1285's lifecycle hardening for the original rebuild path).
    backend = ChromaBackend()
    counts: dict[str, int] = {}
    try:
        for cname in _recoverable_collections():
            ...
            if upserted == 0:
                print(f"    no rows found for {cname} in source nook")
            else:
                print(f"    done: {upserted} rows in {cname}")

        print(f"\n  Rebuild complete. {sum(counts.values())} total rows.")
        if archive_path is not None:
            print(f"  Original nook archived at: {archive_path}")
        print(f"{'=' * 55}\n")
        return counts
    finally:
        backend.close()
...
def status(nook_path=None, collection_name: Optional[str] = None) -> dict:
    ...
    chromadb tries to load the undersized HNSW. Running ``sage
    ...
    directly via :func:`sage.backends.chroma.hnsw_capacity_status`.

    Returns the capacity-status dict (also printed). Returns a dict with
    ``status="unknown"`` when no nook exists at the given path.
    """
    nook_path = nook_path or _get_nook_path()
    collection_name = collection_name or _drawers_collection_name()
    print(f"\n{'=' * 55}")
    print("  sage Repair — Status")
    print(f"{'=' * 55}\n")
    print(f"  Nook: {nook_path}")

    if not os.path.isdir(nook_path):
        print("  No nook found.\n")
        return {"status": "unknown", "message": "no nook at path"}

    db_path = os.path.join(nook_path, "chroma.sqlite3")
    if not os.path.isfile(db_path):
        print(f"  Nook dir at {nook_path} exists but has no chroma.sqlite3 yet.\n")
        return {"status": "uninitialized", "message": "nook has no chroma.sqlite3 yet"}
    ...
    drawer_row_count = sqlite_drawer_count(nook_path, collection_name)
    if isinstance(drawer_row_count, int) and drawer_row_count == 0:
        print("  Nook is initialized but empty (no drawers yet).\n")
        return {"status": "empty", "message": "nook has no drawers yet"}

    drawers = hnsw_capacity_status(nook_path, collection_name)
    closets = hnsw_capacity_status(nook_path, CLOSETS_COLLECTION_NAME)
    ...
    if drawers["diverged"] or closets["diverged"]:
        print("\n  Recommended: run `sage repair` to rebuild the index.")
    print()
...
def _close_chroma_handles(nook_path: str, backend: "ChromaBackend | None" = None) -> None:
...
#: Any ``max_seq_id.seq_id`` above this is unreachable by a real nook.
...
def _compute_heuristic_seq_id(cur: sqlite3.Cursor, segment_id: str) -> int:
    ...
    ``embeddings.seq_id`` rows can be BLOB-typed on nooks where
...
def repair_max_seq_id(
    ...
    *,
    ...
    print(f"\n{'=' * 55}")
    print("  sage Repair — max_seq_id Un-poison")
    print(f"{'=' * 55}\n")
    print(f"  Nook:  {nook_path}")
    if segment:
    ...
    if not os.path.isdir(nook_path):
        print(f"  No nook found at {nook_path}")
        result["aborted"] = True
        result["reason"] = "nook-missing"
        return result
    if not contains_nook_database(nook_path):
        print(f"  No nook database at {nook_path}")
        result["aborted"] = True
    ...
    if not confirm_destructive_action("Repair max_seq_id", nook_path, assume_yes=assume_yes):
        result["aborted"] = True
    ...
    if backup:
        ...
        backup_path = os.path.join(nook_path, f"chroma.sqlite3.max-seq-id-backup-{timestamp}")
        shutil.copy2(db_path, backup_path)
        result["backup"] = backup_path
        print(f"  Backup:  {backup_path}")

    _close_chroma_handles(nook_path)
...
if __name__ == "__main__":
    p = argparse.ArgumentParser(description="sage repair tools")
    p.add_argument("command", choices=["status", "scan", "prune", "rebuild"])
    p.add_argument("--nook", default=None, help="Nook directory path")
    ...
    if args.command == "status":
        status(nook_path=path)
    elif args.command == "scan":
        scan_nook(nook_path=path, only_wing=args.wing)
    elif args.command == "prune":
        prune_corrupt(nook_path=path, confirm=args.confirm)
    elif args.command == "rebuild":
        rebuild_index(nook_path=path)
