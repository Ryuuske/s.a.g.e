#!/usr/bin/env python3
"""
convo_miner.py — Mine conversations into the nook.

Ingests chat exports (Claude Code, Claude.ai, Codex CLI, plain text transcripts).
Normalizes format, chunks by exchange pair (Q+A = one unit), files to nook.

Same nook as project mining. Different ingest strategy.
"""
...
from .miner import is_gitignored, load_sageignore_matcher
...
def _source_file_delete_ids(collection, source_file: str, extract_mode: str) -> list[str]:
    ...
    Earlier conversation drawers did not carry extract_mode; treat those as
...
def scan_convos(convo_dir: str) -> list:
    ...
    Also skips files matched by ``.sageignore`` in any ancestor
    directory (WI-8 defense-in-depth: secret-file patterns such as ``*.pem``
    and ``.env`` are excluded before any content is read).
    """
    ...
    for root, dirs, filenames in os.walk(convo_path):
        ...
        # Load .sageignore matchers from root down to current_dir.
        # Mirrors scan_project's nookignore logic in miner.py.
        nookignore_matchers = []
        try:
            rel_parts = current_dir.relative_to(convo_path).parts
        except ValueError:
            rel_parts = ()
        for depth in range(len(rel_parts) + 1):
            ancestor = convo_path.joinpath(*rel_parts[:depth]) if depth > 0 else convo_path
        ...
        for filename in filenames:
            ...
            if filepath.suffix.lower() in CONVO_EXTENSIONS:
                ...
                if nookignore_matchers and is_gitignored(
                    filepath, nookignore_matchers, is_dir=False
                ):
                    continue
...
def _is_ai_tool_path(path: Path) -> bool:
    ...
    Detected paths (exact-segment match — substrings like `.codex-archive`
    do NOT match):
...
def mine_convos(
    ...
    """Mine a directory of conversation files into the nook.
    ...
    per-nook flock around it so two concurrent ``sage mine --mode
    convos`` invocations against the same nook can't pile up. This
    mirrors the pattern in :func:`sage.miner.mine`. The lock is
    ...
    Dry-run skips the lock — it never writes to the nook and so cannot
    ...
    SageConfig inside :func:`_mine_convos_impl` so `config.json`
...
def _mine_convos_impl(
    ...
    explicit_min = nook_config.min_chunk_size_explicit
    cfg_min_chunk_size = explicit_min if explicit_min is not None else MIN_CHUNK_SIZE

    convo_path = Path(convo_dir).expanduser().resolve()
    wing = _resolve_wing(convo_path, wing)
    # Phase 4 wing-registration gate. _resolve_wing may produce a
    # path-derived wing name; reject unregistered wings up front so
    # search isn't poisoned by a typo'd or auto-inferred wrong wing.
    ...
    # nook each per-file query costs ~2s, so a 2000-file sweep used to
    ...
    for i, filepath in enumerate(files, 1):
        ...
        try:
            ...
            if not dry_run:
                _register_file(collection, source_file, wing, agent, extract_mode, agents=agents)
            continue
        ...
        # Apply the high-confidence tier (not aggressive) after normalize and
        # before chunking so secrets in transcripts never reach stored drawers.
        ...
        drawers_added, room_delta, skipped = _file_chunks_locked(
            collection, source_file, chunks, wing, room, agent, extract_mode, agents=agents
        )
        if skipped:
            files_skipped += 1
            continue
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python convo_miner.py <convo_dir> [--nook PATH] [--limit N] [--dry-run]")
        sys.exit(1)
    from .config import SageConfig

    mine_convos(sys.argv[1], nook_path=SageConfig().nook_path)
