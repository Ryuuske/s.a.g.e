...
PROMPT_TEMPLATE = """You are reading content filed in a memory nook. Generate a
...
def regenerate_closets(
    ...
    drawers_col = get_collection(nook_path, create=False)
    # When dry_run=True, pass create=False so a nook that has no closets
    # collection yet is not mutated on disk by a read-only listing operation.
    closets_col = get_closets_collection(nook_path, create=not dry_run)

    total = drawers_col.count()
    if total == 0:
        print("No drawers in nook.")
        return {"processed": 0}

    # Paginate the fetch — a single get(limit=total, ...) blows through
    # SQLite's SQLITE_MAX_VARIABLE_NUMBER (32766) on large nooks and
...
if __name__ == "__main__":
    ...
    regenerate_closets(args.nook, wing=args.wing, sample=args.sample, dry_run=args.dry_run, cfg=cfg)
