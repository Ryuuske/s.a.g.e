"""
Hook logic for sage — Python implementation of session-start, stop, and precompact hooks.
...
SAGE_ROOT = Path.home() / ".sage"

# ── SAGE_HOOK_PROFILE dial ──────────────────────────────────────────────────
# Controls how much of the hook side-effect surface is active.
#
#   off      — all sage hook side-effects no-op (fail-open: empty {} returned).
#              Equivalent to the ~/.sage kill-switch but scoped to hook behavior
#              only, leaving the nook directory intact.
#   minimal  — emergency-drawer / Keeper-fallback writes only.
#              Diary auto-save (hooks_auto_save) and desktop toasts are
#              suppressed.  Transcript ingest and SAGE_DIR background mining
#              are also suppressed.
#   standard — full current behavior (DEFAULT when env var is unset or
#              contains an unrecognised value).
#
# SAGE_HOOK_PROFILE is env-only — there is no corresponding SageConfig property
# for this dial.  The existing per-feature toggles (hooks_auto_save,
# hook_silent_save, hook_desktop_toast) remain honoured when the profile is
# "standard" — they provide fine-grained control within the standard profile.
# Under "minimal" and "off" the profile short-circuits before those toggles are
# read, so their values have no effect.
_HOOK_PROFILE_ENV = "SAGE_HOOK_PROFILE"
_HOOK_PROFILE_OFF = "off"
_HOOK_PROFILE_MINIMAL = "minimal"
_HOOK_PROFILE_STANDARD = "standard"
_HOOK_PROFILE_VALUES = frozenset({_HOOK_PROFILE_OFF, _HOOK_PROFILE_MINIMAL, _HOOK_PROFILE_STANDARD})


def _hook_profile() -> str:
    """Return the active SAGE_HOOK_PROFILE value.

    Reads SAGE_HOOK_PROFILE from the environment.  Unset or unknown values
    resolve to 'standard' so the dial is truly opt-in for any change from the
    default behavior.
    """
    raw = os.environ.get(_HOOK_PROFILE_ENV, "").strip().lower()
    if raw in _HOOK_PROFILE_VALUES:
        return raw
    return _HOOK_PROFILE_STANDARD
...
def _nook_root_exists() -> bool:
    """User-removable kill-switch.

    If ~/.sage/ does not exist, the user has explicitly cleared it.
    ...
    ``~/.sage`` (or a broken symlink) is treated as absent — otherwise
    the kill-switch would be bypassed and ``STATE_DIR.mkdir()`` would later
    crash on ``NotADirectoryError``.
    """
    return SAGE_ROOT.is_dir()


def _sage_python() -> str:
    """Return the python interpreter that has sage installed.
    ...
    1. SAGE_PYTHON env var (explicit override)
    2. Venv python from package install path
    3. Editable install: venv/ sibling to sage/
    4. sys.executable fallback
    """
    # Honor explicit override (used by shell hook wrappers)
    env_python = os.environ.get("SAGE_PYTHON", "")
    if env_python and os.path.isfile(env_python) and os.access(env_python, os.X_OK):
        return env_python
    # This file lives at <venv>/lib/pythonX.Y/site-packages/sage/hooks_cli.py
    # or <project>/sage/hooks_cli.py (editable install).
    #
    ...
    if len(parents) > 3:
        ...
        if venv_bin.is_file():
            return str(venv_bin)
    # Editable install: assumes project root has a venv/ sibling to sage/
    if len(parents) > 1:
...
# Mirrors the redaction gate in hooks/scripts/_session_hook.py so the diary
# auto-save path applies the same scrubbing policy as the emergency drawer.
# Enabled by SAGE_REDACT_SECRETS=1 env var OR hooks.redact_secrets config key.
# Default is OFF; existing behavior is unchanged unless the operator opts in.
...
def _redaction_enabled() -> bool:
    ...
    Default is False when neither is set — existing behavior unchanged.
    Fails open (returns False) on any import or config error.
    """
...
def _maybe_scrub(content: str) -> str:
    ...
    Fails open: returns ``content`` unchanged if the scrubber import fails.
    """
...
STOP_BLOCK_REASON = (
    "sage auto-save checkpoint. "
    "Use nook_diary_write (session summary) and nook_add_drawer "
    "(quotes, decisions, code) to save session content. "
    "Do NOT use native auto-memory files."
)

PRECOMPACT_BLOCK_REASON = (
    "sage emergency save — compaction imminent. "
    "Use nook_diary_write (thorough summary) and nook_add_drawer "
...
def _log(message: str):
    ...
    if not _nook_root_exists():
        return  # User removed the nook; do not recreate by logging
...
def _output(data: dict):
    ...
    If sage_mcp.mcp_server is already loaded, reuse its saved real stdout fd.
    ...
    mcp_mod = sys.modules.get("sage_mcp.mcp_server") or sys.modules.get(
...
def _get_mine_targets() -> list[tuple[str, str]]:
    ...
    SAGE_DIR (when set and resolvable) contributes a ``"projects"``
    ...
    An empty list means no SAGE_DIR ingest should run.
    """
    targets: list[tuple[str, str]] = []
    sage_dir = os.environ.get("SAGE_DIR", "")
    if sage_dir:
        resolved = Path(sage_dir).expanduser().resolve()
        if resolved.is_dir():
...
_MINE_PID_FILE_ENV = "SAGE_MINE_PID_FILE"
...
# its slot forever.  Set SAGE_MINE_TIMEOUT_HOURS=0 to disable the
# timeout (slots are reclaimed only when the PID is dead).
_MINE_TIMEOUT_HOURS_ENV = "SAGE_MINE_TIMEOUT_HOURS"
...
def _mine_slot_timeout_secs() -> float:
    ...
    Reads ``SAGE_MINE_TIMEOUT_HOURS`` from the environment (float).
...
def _spawn_mine(cmd: list) -> None:
    ...
    ``SAGE_MINE_PID_FILE`` env var so its cleanup hook can remove
...
def _maybe_auto_ingest():
    """Background-mine SAGE_DIR (project files) if set.
    ...
    for mine_dir, mode in targets:
        try:
            _spawn_mine([_sage_python(), "-m", "sage_mcp", "mine", mine_dir, "--mode", mode])
        except OSError:
            pass


def _mine_sync():
    """Synchronously mine SAGE_DIR (precompact path).
...
def _desktop_toast(body: str, title: str = "sage"):
    """Send a desktop notification via notify-send. Fails silently."""
    try:
        subprocess.Popen(
            ["notify-send", "--app-name=sage", "--icon=brain", title, body],
...
def _save_diary_direct(
    ...
    # Scrub before writing to nook, consistent with the emergency drawer path.
    entry = _maybe_scrub(entry)
...
def _ingest_transcript(transcript_path: str):
    """Mine a Claude Code session transcript into the nook as a conversation."""
    path = Path(transcript_path).expanduser()
    if not path.is_file() or path.stat().st_size < 100:
        return

    try:
        SageConfig()  # validate config loads
    except Exception:
        return
...
def _wing_from_transcript_path(transcript_path: str) -> str:
    ...
    3. EARLIER FORM — Match an explicit ``-Projects-<name>`` segment for
    ...
    # 3. Earlier-form — explicit -Projects-<name> segment
...
def hook_stop(data: dict, harness: str):
    ...
    # SAGE_HOOK_PROFILE dial — checked before any config reads.
    _profile = _hook_profile()
    if _profile == _HOOK_PROFILE_OFF:
        # All sage hook side-effects no-op (fail-open).
        _output({})
        return
    if _profile == _HOOK_PROFILE_MINIMAL:
        # Diary auto-save, transcript ingest, and background mining suppressed.
        # Only emergency-drawer / Keeper-fallback writes are active (handled by
        # the _session_hook body, which runs independently of hooks_cli.py).
        _output({})
        return
    ...
    if not SageConfig().hooks_auto_save:
        _output({})
        return
    ...
    if str(stop_hook_active).lower() in ("true", "1", "yes"):
        ...
        # (the older version), so if we can't read config, behave as if it's still on.
        silent_guard = True
        try:
            silent_guard = SageConfig().hook_silent_save
    ...
    try:
        ...
        _log(f"WARNING: could not create state dir {STATE_DIR}: {exc}; hook proceeding")
        _output({})
        return
    ...
    if since_last >= SAVE_INTERVAL and exchange_count > 0:
        ...
        if silent:
            ...
            if count > 0:
                ...
                _output(
                    {
                        "systemMessage": f"\u2726 {count} memories woven into the nook{tag}",
                    }
                )
            else:
                _output({})
        else:
            # Original: block and ask Claude to save via MCP tools.
...
def hook_session_start(data: dict, harness: str):
    ...
    # SAGE_HOOK_PROFILE dial — off suppresses all side-effects.
    # minimal and standard both allow session tracking initialization.
    if _hook_profile() == _HOOK_PROFILE_OFF:
        _output({})
        return
    ...
    try:
        ...
        _log(f"WARNING: could not create state dir {STATE_DIR}: {exc}")
...
def hook_precompact(data: dict, harness: str):
    ...
    if _profile == _HOOK_PROFILE_OFF:
        ...
        return
    if _profile == _HOOK_PROFILE_MINIMAL:
        # Transcript ingest and background mining suppressed under minimal profile.
        _output({})
        return
    ...
    # Mine SAGE_DIR synchronously so project data lands before
...
def run_hook(hook_name: str, harness: str):
    ...
    try:
        ...
        # Fail-open: an unhandled exception must never propagate out of a
        # hook handler and crash the harness integration (mirrors the outer
        # guard in _session_hook.py::run_session_hook and ADR-0011's
        # 'PreCompact must never cancel compaction' invariant).
        import traceback

        _log(f"ERROR: {hook_name} hook raised unexpectedly:\n{traceback.format_exc()}")
        _output({})
